 ...
 * ─────────────────────────────────────────────────────
 * Slack Socket Mode 整合閘道 (OmniAgent-compatible)
 *
...
export async function push5TReport(payload: Slack5TReportPayload): Promise<void> {
  ...
  const blocks: SlackBlock[] = [
    ...
    { type: 'divider' },
    {
      ...
      fields: [
        ...
        {
          type: 'mrkdwn',
          text: `*T2 善 (Transparent / 可透明)*\n${_scoreBar(payload.t2Transparent)} ${
            payload.t2Transparent
          }分`,
        },
        {
          type: 'mrkdwn',
          text: `*T3 美 (Tangible / 可感知)*\n${_scoreBar(payload.t3Tangible)} ${
            payload.t3Tangible
          }分`,
        },
        {
          type: 'mrkdwn',
          text: `*T4 信 (Trustworthy / 不可篡改)*\n${_scoreBar(payload.t4Trustworthy)} ${
            payload.t4Trustworthy
          }分`,
        },
        {
          type: 'mrkdwn',
          text: `*T5 通 (Trackable / 可追蹤)*\n${_scoreBar(payload.t5Trackable)} ${
            payload.t5Trackable
          }分`,
        },
  ...
  await callSlackAPI('chat.postMessage', {
    channel,
    blocks,
    text: `5T 協議報告：${payload.companyName} (${score}/100)`,
  });
}
...
export async function pushAgentSummary(payload: SlackAgentSummaryPayload): Promise<void> {
  ...
  const statusIcon =
    payload.status === 'completed' ? '✅' : payload.status === 'failed' ? '❌' : '⚠️';
  ...
  await callSlackAPI('chat.postMessage', {
    channel,
    blocks,
    text: `${statusIcon} ${agentLabel}: ${payload.taskTitle}`,
  });
}
