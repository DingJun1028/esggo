...
export { getOmniNotesAction, semanticCreateTaskAction } from './OmniNotesActions';
export type { TaskRecord } from './OmniNotesActions';
import { TaskRecord, handleRefresh, markTaskAsDone, assignTaskToAgent } from './OmniNotesActions';
 ...
 */
export default async function OmniNotesTracker() {
  ...
  try {
    ...
    const rawRecords = await (client.getRecords
      ? client.getRecords(datasheetId)
      : (client as any).listRecords(datasheetId));
    ...
    <div className="w-full max-w-4xl mx-auto rounded-2xl border border-[var(--theme-border)] bg-[var(--theme-surface)]/80 p-6 shadow-glass backdrop-blur-xl transition-all duration-slower">
      ...
      <div className="mb-6 flex items-center justify-between border-b border-[var(--theme-border)]/30 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold tracking-wider text-[var(--theme-text)]">
              全通之心顯化：萬能筆記追蹤
            </h2>
            <p className="text-sm text-[var(--theme-text-muted)]">
              OmniCore 自主衍生的治癒與治理足跡
            </p>
          </div>
        </div>
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                {!task.assignee && task.status !== 'Done' && (
                  <form
                    action={assignTaskToAgent.bind(null, task.id)}
                    className="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                  >
                    <select
                      name="agentId"
                      required
                      className="appearance-none bg-black/40 border border-[var(--theme-border)] text-[10px] font-mono text-slate-300 py-1 pl-2 pr-5 rounded focus:border-[var(--theme-primary)]/50 focus:outline-none cursor-pointer transition-colors"
                    >
                      ...
                      <option value="OmniAgent">OmniAgent Email</option>
                ...
                {/* 狀態標籤 */}
                <span
                  className={`flex items-center gap-2 rounded-full px-3 py-1 text-xs font-bold font-mono border ${
                    task.status === 'Done'
