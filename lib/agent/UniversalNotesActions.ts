...
export async function getUniversalNotesAction(): Promise<TaskRecord[]> {
    try {
        ...
        console.error('[UniversalNotes Action] 實時連動獲取失敗:', err);
        return [];
    }
}
...
export async function semanticCreateTaskAction(prompt: string) {
    ...
    try {
        ...
        console.error('[UniversalNotes Action] 語義建立任務失敗:', err);
    }
}
...
export async function assignTaskToAgent(taskId: string, formData: FormData) {
    ...
    try {
        ...
        console.error(`[UniversalNotes] 無法指派任務 ${taskId}:`, err);
    }
}
