...
// Write logic
parts.push(sections.map(s => {
  const c = s.content.replace(/`/g, '\\`').replace(/\$/g, '\\$');
  return "  {\n" +
         "    id: '" + s.id + "',\n" +
         "    title: '" + s.title + "',\n" +
         "    chapter: " + s.chapter + ",\n" +
         "    wordCount: " + s.wordCount + ",\n" +
         "    griAlignment: " + JSON.stringify(s.griAlignment) + ",\n" +
         "    hasChart: " + s.hasChart + ",\n" +
...
parts.push("export const FULL_TEMPLATE: ReportTemplate = {");
parts.push("  id: 'full-esg-report',");
parts.push("  name: '全面合規永續報告',");
parts.push("  theme: 'comprehensive',");
parts.push("  industry: ['technology', 'manufacturing', 'general'],");
parts.push("  totalSections: SECTIONS.length,");
parts.push("  estimatedWords: SECTIONS.reduce((acc, curr) => acc + curr.wordCount, 0),");
parts.push("  sections: SECTIONS");
parts.push("};");

fs.writeFileSync(outputPath, parts.join('\n'));
console.log('Successfully generated full-template.ts');