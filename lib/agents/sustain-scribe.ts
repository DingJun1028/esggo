...
export class SustainWriteScribe {
  ...
  public async expandChapter(task: ExpansionTask): Promise<string> {
    ...
    omniAgentBus.publish('AGENT_TASK', {
      agent: 'SustainScribe',
      task: `Expanding ${title} to depth ${depth}`,
    });

    const outline = await this.generateDeepOutline(title, griReference, depth);
    console.log(
      `[SustainWrite] 深層大綱已生成，共 ${outline.length} 個主章節，預計展開 ${outline.reduce(
        (acc, curr) => acc + curr.subsections.length,
        0
      )} 個子維度。`
    );
    ...
    for (const section of outline) {
      ...
      for (const subsection of section.subsections) {
        if (depth === 4) {
          const microTopics = await this.generateGranularOutline(
            subsection.title,
            subsection.focus
          );
          for (const topic of microTopics) {
            tasksToRun.push(() =>
              this.limit(async () => {
                omniAgentBus.publish('AGENT_TASK', {
                  agent: 'SustainScribe',
                  task: `Drafting Micro: ${topic.title}`,
                });
                const content = await this.generateExpertParagraph(
                  title,
                  section.title,
                  subsection.title + ' - ' + topic.title,
                  topic.focus,
                  context,
                  depth
                );
          ...
          tasksToRun.push(() =>
            this.limit(async () => {
              omniAgentBus.publish('AGENT_TASK', {
                agent: 'SustainScribe',
                task: `Drafting: ${subsection.title}`,
              });
              const content = await this.generateExpertParagraph(
                title,
                section.title,
    ...
    console.log(
      `[SustainWrite] 開始並發執行 ${tasksToRun.length} 個擴充任務 (Max Concurrency: ${CONCURRENCY_LIMIT})`
    );
  ...
  private async generateDeepOutline(title: string, gri: string, depth: number) {
    ...
    const standardStructure = [
      {
        ...
        subsections: [
          {
            title: '1.1 政策聲明與最高指導原則',
            focus: '詳述企業對此議題的承諾、董事會層級的參與以及核心價值觀的結合。',
          },
          {
            title: '1.2 治理架構與權責分配',
            focus: '說明專責單位的組成、跨部門協調機制以及向董事會的匯報流程。',
          },
          {
            title: '1.3 利害關係人議合與重大性鑑別',
            focus: '分析此議題對各利害關係人的影響，以及為何被列為重大主題的評估過程。',
          },
        ],
      },
      {
        ...
        subsections: [
          {
            title: '2.1 轉型風險與實體風險評估',
            focus: '依據 TCFD/TNFD 框架，詳細展開短中長期面臨的具體風險情境。',
          },
          {
            title: '2.2 氣候與社會變遷帶來的商業機會',
            focus: '探討永續轉型如何帶來新市場、新技術與產品創新的可能性。',
          },
          {
            title: '2.3 財務衝擊與韌性壓力測試',
            focus: '量化風險對營運成本與資本支出的影響，並說明企業的承受能力。',
          },
        ],
      },
      {
        ...
        subsections: [
          {
            title: '3.1 短中長期量化目標 (KPIs)',
            focus: '列出明確的基期、目標年份與預期達成率（如 SBTi 減碳路徑）。',
          },
          {
            title: '3.2 關鍵資源投入與技術升級計畫',
            focus: '說明為達成目標所規劃的資本支出 (CAPEX)、技術引進與研發 (R&D) 策略。',
          },
          {
            title: '3.3 供應鏈與價值鏈協同方案',
            focus: '探討如何帶動上下游夥伴共同參與，解決 Scope 3 或價值鏈社會責任。',
          },
        ],
      },
      {
        ...
        subsections: [
          {
            title: '4.1 年度績效數據總覽與趨勢分析',
            focus: '以數據佐證當年度的執行成效，並與過往三年進行趨勢對比。',
          },
          {
            title: '4.2 挑戰、阻礙與應對策略',
            focus: '誠實揭露執行過程中遇到的困難，並提出修正後的行動方針。',
          },
          {
            title: '4.3 未來展望與下一階段規劃',
            focus: '總結本章，並勾勒出下一個年度的永續重點與升級方向。',
          },
        ],
      },
    ];
    ...
    if (depth === 1) {
      return standardStructure
        .slice(0, 1)
        .map((s) => ({ ...s, subsections: s.subsections.slice(0, 1) }));
    } else if (depth === 2) {
      return standardStructure
        .slice(0, 3)
        .map((s) => ({ ...s, subsections: s.subsections.slice(0, 2) }));
    }
  ...
  private async generateGranularOutline(subsectionTitle: string, focus: string) {
    ...
    return [
      {
        title: '背景脈絡與現狀診斷',
        focus: `基於「${focus}」，詳細分析當前的全球趨勢與企業面臨的具體現狀。`,
      },
      {
        title: '關鍵痛點與根本原因',
        focus: `深入探討在執行「${focus}」時遭遇的結構性阻礙與技術瓶頸。`,
      },
      {
        title: '解決方案與執行路徑',
        focus: `提出具體、可量化、分階段的創新解決方案與資源配置藍圖。`,
      },
      {
        title: '預期效益與風險控制',
        focus: `評估方案實施後的財務與非財務效益，以及可能的副作用與備援機制。`,
      },
...
5. 融入「觀因循果」的邏輯：說明動機（因）、執行過程（循）、以及預期或實際效益（果）。
      ...
      if (!ai) {
        throw new Error('AI not initialized - missing API key');
      }
  ...
  public getDepthConfig(depth: number): {
    ...
    const configs: Record<
      number,
      { wordCountTarget: number; paragraphCount: number; subsectionDepth: number }
    > = {
