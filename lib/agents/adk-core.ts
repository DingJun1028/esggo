...
import { omniAgentBus } from './oa-agent-bus.ts';
...
Consider synthesizing a temporary tool if the existing tools are insufficient.
    ...
    const hasMatchingTool = existingTools.some(
      (t: unknown) =>
        (t as { name: string })?.name &&
        task.toLowerCase().includes((t as { name: string })?.name.toLowerCase())
    );
      ...
      try {
        ...
        const memoryRecord: Omit<MemoryRecord, 'id' | 'timestamp'> = {
          ...
          tags: [
            ...
            `error:${(error instanceof Error ? error.message : 'Unknown error').slice(0, 20)}`,
          ],
        };
        this._memoryStore.add(memoryRecord);
        ...
        if (
          ...
          console.warn(
            `[ADK Agent - ${this.config.name}] ⚠️ API Key Error. Entering Resilient Simulation Mode...`
          );
