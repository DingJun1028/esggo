...
export class JESMonitor {
  ...
  private targetEmissions: Map<string, number>;
  private carbonHistory: CarbonHistoryEntry[] = [];
  ...
  async fetchEnergyDataFromSupabase(maxRetries = 3): Promise<EnergyData[]> {
  ...
  suggestOptimizations(conflicts: Conflict[]) {
  ...
  getCarbonHistory(limit = 50): CarbonHistoryEntry[] {
  ...
  generateChart(): string {
    ...
    if (history.length === 0) return '無歷史數據';
