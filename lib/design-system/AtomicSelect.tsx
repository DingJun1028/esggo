    ...
    <div className={`flex flex-col gap-1.5 w-full ${className}`}>
      ...
      <div className="relative">
        <select
          ...
          disabled={disabled}
          className={`w-full h-10 px-3 py-2 rounded-md border text-sm bg-black/40 text-slate-200 transition-all duration-200 focus:outline-none appearance-none cursor-pointer
            ${error ? 'border-rose-500/50 focus:ring-1 focus:ring-rose-500/50' : 'border-white/10 focus:ring-1 focus:ring-[#06b6d4]/50 focus:border-[#06b6d4]/50'}
      ...
      {helperText && (
        <span className={`text-[10px] font-mono ${error ? 'text-rose-400' : 'text-slate-500'}`}>
          {helperText}
        </span>
      )}
    </div>
