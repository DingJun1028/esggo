...
export interface AtomicTextareaProps extends React.TextareaHTMLAttributes<HTMLTextAreaElement> {
  label?: string;
  error?: boolean;
  helperText?: string;
}
...
export const AtomicTextarea: React.FC<AtomicTextareaProps> = ({
  ...
  return (
    <div className={cn('flex flex-col gap-1.5 w-full', className)}>
      {label && (
        <label htmlFor={textareaId} className="text-xs font-mono text-slate-400">
          {label}
        </label>
      )}
      <textarea
        ...
        className={cn(
          'w-full min-h-[100px] px-3 py-2 rounded-lg border text-[13px] font-medium bg-[#020617]/60 backdrop-blur-md text-slate-200',
          'transition-all duration-300 focus:outline-none placeholder:text-slate-500 resize-y custom-scrollbar',
          error 
