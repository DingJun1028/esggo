...
function Start-Platform {
    ...
    $nextProcess = Start-Process npm.cmd -ArgumentList "run dev" -NoNewWindow -PassThru -RedirectStandardOutput "./omni_trace.log" -RedirectStandardError "./omni_error.log" -WorkingDirectory $APP_DIR
    
    # Start OmniAgent Gateway Server in background
    $gatewayProcess = Start-Process node -ArgumentList "omniagent-gateway/omni-server.mjs" -NoNewWindow -PassThru -RedirectStandardOutput "./gateway_trace.log" -RedirectStandardError "./gateway_error.log" -WorkingDirectory $APP_DIR
...
# 自動權限檢查與提權
if ($action -match "start|stop|restart" -and -not (Test-IsAdmin)) {
    Write-Host "[!] Administrative privileges are required for: $action" -ForegroundColor Yellow
