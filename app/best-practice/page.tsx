...
export default function BestPracticeHubPage() {
  const [activeTab, setActiveTab] = useState<'benchmarks' | 'standards' | 'templates'>(
    'benchmarks'
  );
  ...
  const applyPractice = async (practice: AiRecommendation) => {
    ...
    try {
      ...
      await integrityService.sealData(
    ...
    primaryActions: [
      {
        id: 'ai-suggest',
        label: 'AI 推薦實踐',
        icon: loadingAi ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />,
        ...
        component: (
          <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="relative group">
              <Search
                size={20}
                className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#003262] transition-colors"
              />
              <input
            ...
            {aiRecommendations.length > 0 && activeTab === 'benchmarks' && (
              <div className="p-8 bg-blue-50/50 rounded-[2.5rem] border border-blue-200/30 space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setAiRecommendations([])}>
                    清除
                  </Button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {aiRecommendations.map((rec, i) => (
                    <Card
                      ...
                      hover
                      padding="md"
                      className="bg-white border-blue-100/50 border-2"
                    >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {BEST_PRACTICES.filter((p) => p.title.includes(searchQuery)).map((p) => (
                  <Card
                    ...
                    hover
                    padding="lg"
                    className="flex flex-col h-full border-none shadow-premium relative overflow-hidden"
                  ...
                  >
                    ...
                    <h4 className="text-lg font-black text-[#003262] mb-2 leading-tight">
                      {p.title}
                    </h4>
                    ...
                    <div className="mt-8 pt-6 border-t border-slate-50 flex items-center justify-between">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        className="p-0 h-auto text-slate-400 hover:text-[#003262]"
                      >
              ...
              <div className="grid grid-cols-1 gap-4">
                {STANDARDS.slice(0, 5).map((s) => (
                  <Card
                    key={s.id}
                    padding="md"
                    className="flex items-center justify-between border-slate-100 hover:border-blue-200 transition-all group"
                  >
                ...
                <Link href="/standards">
                  <Button
                    variant="ghost"
                    fullWidth
                    className="py-8 border-dashed border-2 border-slate-100 rounded-3xl text-slate-400 hover:text-blue-600 transition-all"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {EXPERT_TEMPLATES.map((t) => (
                  <Card
                    ...
                    hover
                    padding="lg"
                    className="border-none shadow-sm flex items-center gap-6 group"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {t.t5ready && (
                          <Badge variant="success" size="xs" className="scale-75 origin-left">
                            5T Ready
                          </Badge>
                        )}
                      </div>
                    ...
                    <Button
                      variant="primary"
                      size="sm"
                      className="rounded-xl px-6 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      下載
                    </Button>
                  </Card>
        ...
        >
          <div className="space-y-8 p-2">
            <div className="p-8 bg-blue-50/30 rounded-[2.5rem] border border-blue-100/50">
              ...
              <h3 className="text-2xl font-black text-[#003262] mb-4 leading-tight">
                ...
                <div className="relative z-10 space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-xs font-black uppercase tracking-tight">
                      OmniAgent Governance AI
                    </p>
                  </div>
                  <p className="text-sm text-blue-100/90 leading-relaxed font-medium italic">
                    「偵測到您的企業在 ${selectedPractice.industry} 中具備相似的組織結構。建議導入其
                    5T 自動化驗算模型，可大幅降低合規缺口風險。」
                  </p>
                  <Button
                    variant="primary"
                    fullWidth
                    className="bg-blue-500 hover:bg-blue-400 h-12 rounded-2xl font-black"
