...
export default function OmniFactoryBasePage() {
  const [activeTab, setActiveTab] = useState<'factory' | 'bookshelf' | 'oneness' | 'tools'>(
    'factory'
  );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setSwarmLogs((prev) =>
        [...prev, messages[Math.floor(Math.random() * messages.length)]].slice(-6)
      );
    }, 3500);
  ...
  const handleSubmitDsl = () => {
    ...
    setPipelineLogs([
      '[SYS] Parsing OmniFactory DSL...',
      '[SYS] Validating schema against OMNIFACTORY_DSL.md...',
    ]);
    ...
    setTimeout(
      () =>
        setPipelineLogs((prev) => [
          ...prev,
          '[ZKP] Checking 5T compliance (Traceable, Trackable)... Passed!',
        ]),
      800
    );
    setTimeout(() => setPipelineLogs((prev) => [...prev, '[BUILD] Compiling React AST...']), 1600);
    setTimeout(
      () => setPipelineLogs((prev) => [...prev, '[SEAL] Applying Cryptographic Hash Lock...']),
      2400
    );
    setTimeout(() => {
      setPipelineLogs((prev) => [
        ...prev,
        '[SUCCESS] Component published to OmniBookCaseRegistry.',
      ]);
      ...
      <div className="relative z-10 flex-1 flex flex-col p-4 md:p-8 max-w-7xl mx-auto w-full gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs px-2 py-0.5 rounded bg-cyan-950 border border-cyan-500/30 text-cyan-400 font-bold uppercase tracking-widest">
                  OMNI CORE
                </span>
        ...
        <div className="flex gap-2 border-b border-white/10 pb-1 overflow-x-auto">
          <button
            ...
            className={`px-6 py-3 rounded-t-lg font-bold text-sm tracking-widest flex items-center gap-2 transition-all whitespace-nowrap ${
              activeTab === 'factory'
                ? 'bg-cyan-950/50 text-cyan-400 border-t border-l border-r border-cyan-500/30'
          ...
          <button
            ...
            className={`px-6 py-3 rounded-t-lg font-bold text-sm tracking-widest flex items-center gap-2 transition-all whitespace-nowrap ${
              activeTab === 'bookshelf'
                ? 'bg-emerald-950/50 text-emerald-400 border-t border-l border-r border-emerald-500/30'
                : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'
            }`}
          >
          ...
          <button
            ...
            className={`px-6 py-3 rounded-t-lg font-bold text-sm tracking-widest flex items-center gap-2 transition-all whitespace-nowrap ${
              activeTab === 'oneness'
                ? 'bg-indigo-950/50 text-indigo-400 border-t border-l border-r border-indigo-500/30'
                : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'
            }`}
          >
          ...
          <button
            ...
            className={`px-6 py-3 rounded-t-lg font-bold text-sm tracking-widest flex items-center gap-2 transition-all whitespace-nowrap ${
              activeTab === 'tools'
                ? 'bg-purple-950/50 text-purple-400 border-t border-l border-r border-purple-500/30'
                : 'text-slate-500 hover:text-slate-300 hover:bg-white/5'
            }`}
          >
          ...
          {activeTab === 'factory' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col gap-6">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h2 className="text-lg font-bold text-cyan-100 tracking-wider">
                  萬能工廠 DSL 提交器
                </h2>
              </div>
                ...
                <div className="flex flex-col gap-4">
                  <div className="border border-cyan-500/20 rounded-xl p-4 flex-1 flex flex-col">
                    ...
                    <div className="mt-4 flex justify-between items-center">
                      ...
                      >
                        {isSubmitting ? (
                          <span className="animate-pulse">編譯中...</span>
                  ...
                  <div className="border border-white/5 rounded-xl p-4 flex-1 flex flex-col">
                    ...
                    <div className="space-y-3 mt-4">
                      ...
                      <div className="bg-cyan-950/20 border border-cyan-500/20 p-3 rounded-lg flex justify-between items-center">
                        ...
                        <span className="px-2 py-1 bg-cyan-500/20 text-cyan-400 text-xs font-bold rounded border border-cyan-500/30">
                          TESTING
                        </span>
                      </div>
                    </div>
          ...
          {activeTab === 'bookshelf' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full w-full">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-lg font-bold text-emerald-100 tracking-wider">
                  萬能書櫃總表 (BookCase Registry)
                </h2>
              </div>
          ...
          {activeTab === 'oneness' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col items-center justify-center relative min-h-[600px]">
              <div className="absolute top-0 left-0 w-full flex items-center gap-2 mb-4 p-2">
                ...
                <h2 className="text-lg font-bold text-indigo-100 tracking-wider">
                  全通之心：無作妙德，圓通無礙
                </h2>
              </div>

              {/* Concentric Circles Visualization using new Organism component */}
              ...
              <div className="mt-12 text-center max-w-lg z-10">
                <h3
                  className="text-xl font-bold text-white mb-2 tracking-widest"
                  style={{ textShadow: '0 0 10px rgba(255,255,255,0.5)' }}
                >
                  全通之心 (The Heart of Omni Connectivity)
                </h3>
                <p className="text-sm text-indigo-200 leading-relaxed opacity-80">
                  當數據在全知、全能、全域、全境、全息間流轉時，不存在任何技術瓶頸與延遲。
                  <br />
          ...
          {activeTab === 'tools' && (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col gap-6">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-purple-100 tracking-wider">
                  萬能工具樞紐 (Omni Tools Hub)
                </h2>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">
                      內建 Agnes AI 專屬文法工具、萬能智庫佐證對接，以及 5T
                      全息編織引擎的專業永續報告編輯器。
                    </p>
                  </div>
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">
                      管理 ESG 領域知識庫、RAG 檢索向量數據庫，與外部政策文檔的同步中心。
                    </p>
                  </div>
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <h3 className="text-lg font-bold text-emerald-100 mb-1">
                      元件設計市集 (Design Library)
                    </h3>
                    <p className="text-xs text-slate-400 leading-relaxed">
                      瀏覽所有已發布至市集的 Trinity UIUX 元件，支援 Light/Dark 主題熱切換展示。
                    </p>
                  </div>
              ...
              <div className="mt-6 bg-black border border-indigo-500/20 rounded-xl flex flex-col flex-1 max-h-[300px]">
                <div className="bg-indigo-950/40 px-4 py-2 border-b border-indigo-500/20 flex items-center gap-2">
                  ...
                  <span className="text-xs text-indigo-300 font-bold tracking-widest">
                    OMNI-AGENT SWARM CONSOLE
                  </span>
                </div>
