...
const mobileBottomItems = [
  ...
  { href: '/sustain-write', label: '撰寫', icon: <PenTool size={24} /> },
...
export default function AppShellV2({ children }: { children: React.ReactNode }) {
  ...
  const [favorites, setFavorites] = useState<
    { id: string; title: string; path: string; icon: string; sub?: string }[]
  >([]);
              ...
              {!sidebarCollapsed && (
                <p className="px-4 text-[10px] font-black opacity-40 uppercase tracking-[0.3em] whitespace-nowrap flex items-center justify-between">
                  ...
                  {group.groupId === 'super-admin' && (
                    <span className="text-[9px] bg-purple-500/10 text-purple-400 font-bold px-1.5 py-0.5 rounded-full uppercase tracking-normal">
                      SYS
                    </span>
                  )}
                  {group.groupId === 'favorites' && (
                    <span className="text-[9px] bg-amber-500/10 text-amber-400 font-bold px-1.5 py-0.5 rounded-full uppercase tracking-normal">
                      FAV
                    </span>
                  )}
                </p>
      ...
      <div className="flex-1 flex flex-col min-w-0 relative h-full">
        ...
        >
          {topbarCollapsed ? (
            <ChevronDown
              size={14}
              className="text-[#FDB515] group-hover:scale-125 transition-transform"
            />
          ) : (
            <ChevronUp
              size={14}
              className="text-slate-400 group-hover:scale-125 transition-transform"
            />
          )}
        </button>
          ...
          <div className="flex items-center gap-3 overflow-x-scroll no-scrollbar py-2 w-full pr-4">
            ...
            <BrandButton
              ...
              icon={
                <Star
                  size={18}
                  className={cn(isFavorited ? 'fill-yellow-400 text-yellow-400' : 'text-slate-400')}
                />
              }
              className={cn(
