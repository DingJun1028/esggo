...
export default function ThoughtStreamPanel({
  ...
  useEffect(() => {
    ...
    const es = new EventSource(`/api/agent/${encodeURIComponent(agentId)}/thought/stream${qs}`);
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-3 pr-1">
        ...
        {events.map((ev, i) =>
          ev.type === 'connected' ? (
            <div
              key={i}
              className="text-xs font-mono text-textSecondary border-b border-borderColor pb-2"
            >
              ⚡ 已連線 · {ev.topic}
            </div>
          ) : (
