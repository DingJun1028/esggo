# 萬能筆記 MCP 服務器註冊腳本 (Windows)
# Universal Notes MCP Server Registration Script (Windows)
...
Write-ColorOutput Cyan "萬能筆記 MCP 服務器註冊工具"
Write-ColorOutput Cyan "Universal Notes MCP Server Registration"
Write-ColorOutput Cyan "========================================"
Write-Output ""
...
if (-not (Test-Path $ConfigFile)) {
    ...
    # 檢查是否已經註冊
    if ($config.mcpServers.PSObject.Properties.Name -contains "universal-notes") {
        Write-ColorOutput Yellow "萬能筆記 MCP 服務器已經註冊"
    ...
    $config.mcpServers | Add-Member -Type NoteProperty -Name "universal-notes" -Value @{
        command = "node"
        args = @($ServerPath)
    } -Force
...
Write-Output "2. 在 Claude 中使用萬能筆記工具"
Write-Output ""
