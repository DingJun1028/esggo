...
interface OtherIndirectEmissions {
  ...
  productProcessing: number;
  productUse: number;
  productEndOfLife: number;
...
export class SSOTCarbonInventoryService {
  ...
  constructor() {
    ...
    omniLogger.info('ESG', '????SSOT Carbon Inventory Service initialized', {
  ...
  async calculateInventory(
    ...
    omniLogger.info('ESG', '????Starting SSOT carbon inventory calculation', {
      traceId,
      organizationId,
    ...
    try {
      ...
      omniLogger.info('ESG', '??SSOT carbon inventory calculation completed', {
        traceId,
      ...
      omniLogger.error('ESG', '??SSOT carbon inventory calculation failed', {
        traceId,
  ...
  private async calculateScope3(evidenceData: any): Promise<OtherIndirectEmissions> {
    ...
    scope3.total = Object.keys(scope3)
      ...
      .reduce((sum: number, key: string) => sum + ((scope3 as any)[key] as number), 0);
