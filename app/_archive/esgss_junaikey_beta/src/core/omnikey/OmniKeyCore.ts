...
import type {
  ...
  AwarenessData,
  SemanticData,
  StrategyGuidanceData,
  FlowExecutionData,
  EchoFeedbackData,
  AuthorityKeyReference,
  RuneReference,
  ToolReference,
...
export class OmniKeyCore {
  ...
  public get diagnostics(): SystemDiagnostics {
    return this.systemDiagnostics;
  }

  public get goals(): GoalManager {
  ...
  async initiateCycle(
    ...
    omniLogger.info(LogCategory.SYSTEM, `🌟 奧秘元鑰循環啟動: ${cycle.id}`, { initialTrigger });

    // 異步執行循環 (不阻塞)
    this.executeCycle(cycle).catch(error => {
      omniLogger.error(LogCategory.SYSTEM, '奧秘元鑰循環執行失敗', { error });
      cycle.status = 'failed';
    });
  ...
  private async executeCycle(cycle: MysticCycle): Promise<CycleResult> {
    ...
    try {
      for (const form of forms) {
        ...
        omniLogger.debug(LogCategory.SYSTEM, `⚡ 執行奧義: ${form}`);
      ...
      omniLogger.info(LogCategory.SYSTEM, `✨ 奧秘元鑰循環完成: ${cycle.id}`, { duration });
   ...
   * 解析輸入內容，提取關鍵信息和意圖
   */
  private async executeSemanticDecoding(cycle: MysticCycle): Promise<SemanticData> {
    const awarenessData = cycle.data.awareness_activation_result;

    // 使用符文進行語義分析 (如果有AI模型符文)
    const aiRunes =
      awarenessData?.availableRunes?.filter(
        (r): r is RuneReference => (r as RuneReference)?.baseRune?.type === 'ai_model'
      ) || [];
   ...
   * 基於解構結果，制定執行策略
   */
  private async executeStrategicGuidance(cycle: MysticCycle): Promise<StrategyGuidanceData> {
    const decodingData = cycle.data.semantic_decoding_result;
    const awarenessData = cycle.data.awareness_activation_result;

    // 空值檢查
    if (!decodingData || !awarenessData) {
      throw new Error('Missing required data for strategic guidance');
    }
   ...
   * 執行制定的計劃，調度各種資源
   */
  private async executeFlowExecution(cycle: MysticCycle): Promise<FlowExecutionData> {
    const guidanceData = cycle.data.strategic_guidance_result;

    // 空值檢查
    if (!guidanceData) {
      throw new Error('Strategic guidance data is required');
    }
    ...
    for (const keyId of relevantKeys) {
      try {
        const result = await this.authorityForge.useAuthorityKey(keyId, {
          cycleId: cycle.id,
          context: (guidanceData.task as any)?.context || '',
        });
        executionResults.push(result);
      } catch (error) {
        omniLogger.warn(LogCategory.SYSTEM, `權能鑰匙執行失敗 ${keyId}`, { error });
      }
    }
    ...
    for (const runeId of relevantRunes) {
      try {
        const result = await this.runeEngrafter.executeRune(runeId, {
          cycleId: cycle.id,
          task: guidanceData.task as any, // Task is unknown from StrategyGuidanceData
        });
        executionResults.push(result);
      } catch (error) {
        omniLogger.warn(LogCategory.SYSTEM, `符文執行失敗 ${runeId}`, { error });
      }
    }
   ...
   * 收集執行結果，反饋系統效能
   */
  private async executeEchoFeedback(cycle: MysticCycle): Promise<EchoFeedbackData> {
    const executionData = cycle.data.flow_execution_result;

    // 空值檢查
    if (!executionData) {
      throw new Error('Flow execution data is required for feedback');
    }

    // 分析執行結果
    const feedback: EchoFeedbackData = {
      cyclePerformance: {
        totalTime: executionData.executionTime,
        toolsExecuted: executionData.executedTools,
        successRate:
          executionData.results.filter(r => r.success !== false).length /
  ...
  private async executeKnowledgeRefinement(cycle: MysticCycle): Promise<any> {
    ...
    // 空值檢查
    if (!feedbackData) {
      throw new Error('Echo feedback data is required for knowledge refinement');
    }
  ...
  // 私有工具方法
  private calculateAwarenessLevel(memories: unknown[], keys: unknown[], runes: unknown[]): number {
  ...
  private selectExecutionApproach(decoding: SemanticData, awareness: AwarenessData): string {
  ...
  private selectRequiredTools(decoding: SemanticData, awareness: AwarenessData): ToolReference[] {
    const tools: ToolReference[] = [];

    // 添加相關權能鑰匙
    awareness.authorityKeys.forEach(key => {
      tools.push({ type: 'authority_key' as const, id: key.id, name: key.name });
    });

    // 添加相關符文
    awareness.availableRunes.forEach(rune => {
      tools.push({ type: 'rune' as const, id: rune.id, name: rune.baseRune.name });
    });

    return tools.slice(0, 3); // 限制工具數量
  }

  private assessExecutionRisk(decoding: SemanticData): string {
    if (decoding.processingComplexity >= 3) return 'high';
    if (decoding.keyEntities.length > 2) return 'medium';
    return 'low';
  }

  private shouldTerminateCycle(cycle: MysticCycle, form: MysticForm, result: unknown): boolean {
    // 檢查錯誤條件
    if (form === MysticForm.SEMANTIC_DECODING && !(result as any)?.semanticAnalysis) {
      return true;
    }

    if (form === MysticForm.FLOW_EXECUTION && (result as any)?.flowStatus === 'failed') {
      return true;
    }
  ...
  private synthesizeFinalOutput(cycle: MysticCycle): CycleOutput {
  ...
  private calculateEfficiency(executionData: FlowExecutionData): number {
  ...
  private analyzeCycleImprovements(cycle: MysticCycle, feedback: EchoFeedbackData): string[] {
    const improvements = [];
  ...
  private async applySystemOptimizations(improvements: string[]): Promise<void> {
    ...
    omniLogger.info(LogCategory.SYSTEM, '應用系統優化', { improvements });
  }

  private extractKnowledgeGains(cycle: MysticCycle): {
    patternsLearned: number;
    executionInsights: CycleDataContainer;
    performanceMetrics: { totalTime: number; formsCompleted: number };
  } {
    return {
