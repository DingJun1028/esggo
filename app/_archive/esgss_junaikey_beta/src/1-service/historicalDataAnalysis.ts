...
import { AnalysisType as GlobalAnalysisType } from '@/types';
...
export class HistoricalDataAnalysis {
  ...
  ): Promise<DataOperationResult<HistoricalTrendAnalysis>> {
    const startTime = Date.now();
    const {
  ...
  generateAnalysisReport(taskId: string): string {
    ...
    switch (task.type) {
      case DataAnalysisType.TREND:
        const trend = task.result as HistoricalTrendAnalysis;
        report += `## 趨勢分析結果\n\n`;
    ...
    if (subscribers) {
      subscribers.forEach(callback => {
        try {
          ...
          omniLogger.error(LogCategory.SYSTEM, '[historicalDataAnalysis] 事件訂閱者回調失敗:', { error })
        }
      });
    }
  }
