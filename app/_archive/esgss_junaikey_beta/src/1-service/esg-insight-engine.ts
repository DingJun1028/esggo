...
export class ESGInsightEngine {
   ...
   * 趨勢分析
   */
  async analyzeTrends(metricId: string, periodMonths: number = 12): Promise<EsgTrendAnalysis> {
    try {
      // 獲取歷史數據
      ...
      // 調用 JunAiKey 的趨勢分析
      const trendResult: any = await this.junAiKey.analyzeTrend(dataPoints);
  ...
  async detectBatchAnomalies(days: number = 7): Promise<AnomalyDetection[]> {
    ...
    try {
      ...
      // 並發檢測異常
      const anomalyPromises = recentReadings.map(reading => this.detectAnomalies(reading.id));
   ...
   * 生成趨勢描述
   */
  private generateTrendDescription(trend: EsgTrendAnalysis, metricName: string): string {
    const direction =
      trend.trend === 'increasing' ? '上升' : trend.trend === 'decreasing' ? '下降' : '穩定';
   ...
   * 計算影響程度
   */
  private calculateImpact(trend: EsgTrendAnalysis): 'high' | 'medium' | 'low' {
