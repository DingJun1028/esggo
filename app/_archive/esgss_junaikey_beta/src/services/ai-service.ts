...
import { Language, McpRunActionOutput } from '@/types.js';
import { logKernelEvent, omniLogger, LogCategory } from '@/omni/infrastructure/logging/OmniLogger.js';
...
async function fetchWithRetry(
  ...
  // If all retries fail, return empty object
  return {};
}
...
export async function runMcpAction(
  ...
  try {
    ...
    onLog?.(
      isZh ? `[SYSTEM] Executing protocol: ${action}` : `[SYSTEM] Executing protocol: ${action}`,
      'info'
    );
