...
export interface NexusEvent {
  id: string;
  source: 'system' | 'legion' | 'knowledge' | 'security' | 'governance' | 'ui';
  priority: 'low' | 'normal' | 'high' | 'critical';
  message: string;
  timestamp: number;
  metadata?: any;
  evidence?: IEvidenceMap; // 5T Logic Gate Evidence
  read: boolean;
}
...
class OmniNexusService {
   ...
   */
  public subscribe(typeOrListener: string | NexusListener, listener?: NexusListener): () => void {
    const actualListener = typeof typeOrListener === 'function' ? typeOrListener : listener;
    if (!actualListener) return () => { };

    this.listeners.push(actualListener);
    return () => {
      this.listeners = this.listeners.filter(l => l !== actualListener);
    };
  }
