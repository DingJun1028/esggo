...
import { OMNI_AGENTS, OmniAgentProfile } from '../data/omni-agents.js';
import { WorldEvent } from './EventEngine.js';
...
// The "4+1" Protocol Logic Gate for Training
...
export class OmniTrainingCore {
  ...
  /**
   * [SENTIENT] High-level AI training via Gemini
   */
  static async processSentientTraining(
    agent: any,
    input: TrainingInput,
    activeEvents: WorldEvent[] = []
  ): Promise<TrainingResult> {
    const aiResult = await GeminiService.processSentientTraining({
      agent,
      input: {
        type: input.type,
        content: input.content,
        category: input.category || 'Environmental', // Default category
    ...
    if (!aiResult) {
      ...
      return this.processTraining(agent, input, activeEvents);
    }

    return {
      success: true,
      xpGained: aiResult.xpGained,
      drift: aiResult.drift,
      feedback: aiResult.feedback,
      newTraits: aiResult.insights,
      nexusSynergy: true, // AI calls are inherently high synergy
    };
  }
