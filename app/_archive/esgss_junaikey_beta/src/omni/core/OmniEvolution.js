import { GeminiService, TaskComplexity } from '../../services/geminiService.js';
...
Output JSON format: { "intent": "string (e.g., SECURITY_SCAN, REPORT_GEN, GENERAL_QUERY)", "confidence": number (0-1), "complexity": "simple|moderate|complex" }`;
    ...
    static heuristicAnalysis(input) {
    ...
    static calculateResonance(intent, memoryWeights) {
        return memoryWeights[intent] || 0.1;
    }
    // 4. Strategy - Strategy Generation (Powered by Gemini)
    static async strategize(intent, input, complexity) {
    ...
    static async execute(intent, strategies) {
    ...
    static calculateExperience(currentLevel, taskComplexity) {
        const baseXP = 10;
