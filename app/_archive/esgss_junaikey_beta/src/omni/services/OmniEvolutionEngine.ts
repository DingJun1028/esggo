...
import { OmniEsgManager } from './OmniEsgManager.ts';
...
import { IStakeholder, OmniValueDistribution } from './OmniValueDistribution.ts';
import { IClaim, truthEngine } from './OmniTruthEngine.ts';
    ...
    try {
      ...
      for (const soul of soulComponents) {
        ...
        if (soul.confidence === 'high' && soul.metadata.verified) {
          ...
          const ethicalAdherenceScore = await this.evaluateEthicalAdherence(agentId); // Placeholder for specific agent ID, will use generic for now
          const xpMultiplier = ethicalAdherenceScore > 0.5 ? ethicalAdherenceScore : 0.5; // Minimum 0.5x XP even if adherence is low

          // Grant XP (Evolution) - dynamically adjusted by ethical adherence
          const baseXP = 50;
          ...
          if (result.leveledUp) {
            ...
            awakeningBroadcaster.shareInsight({
              category: 'achievement', // Changed from enlightenment
              title: 'Digital Soul Evolved',
              message: `Agent ${soul.label} has evolved to Level ${result.newLevel} through high ESG confidence.`,
              priority: 'high',
              actionable: false,
