...
export const ImpactProject: React.FC<ImpactProjectProps> = ({ project, compact = false }) => {
  ...
  return (
    <div className="bg-slate-900/50 border border-cyan-500/20 rounded-2xl p-6 flex flex-col h-full backdrop-blur-sm shadow-[0_0_20px_rgba(34,211,238,0.1)]">
        ...
        <div className="flex flex-wrap gap-2">
          {currentProject.sdgs.map(sdgId => {
            ...
            return sdg ? (
              <div
                key={sdgId}
                className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold text-white ${sdg.color}`}
              >
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        ...
        <div className="bg-slate-800/50 border border-white/5 rounded-lg p-4">
          ...
          <div className="text-xs text-slate-500">
            每投入 1 元創造 {currentProject.sroi} 元社會價值
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-3">
          {currentProject.impactMetrics.map((metric, index) => (
            <div key={index} className="bg-slate-800/30 border border-white/5 rounded-lg p-3">
              <div className="flex justify-between items-center mb-2">
              ...
              <div className="w-full bg-slate-700/50 rounded-full h-2">
                <div
                  className={`h-2 rounded-full transition-all duration-500 ${
                    metric.proxy_value >= 80
                      ? 'bg-emerald-500 shadow-[0_0_5px_rgba(16,185,129,0.5)]'
                      : metric.proxy_value >= 60
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <div className="text-white font-bold">
              ${(currentProject.financials.budget / 1000000).toFixed(1)}M / $
              {(currentProject.financials.spent / 1000000).toFixed(1)}M
            </div>
          </div>
          <div>
            <div className="text-slate-400">預計收益 / ROI</div>
            <div className="text-emerald-400 font-bold">
              ${(currentProject.financials.revenue_projected / 1000000).toFixed(1)}M /{' '}
              {currentProject.financials.roi_projected}x
            </div>
          </div>
        </div>
      </div>
