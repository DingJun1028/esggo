/**
 * 🔐 用戶認證與誠信護照系統 (User Authentication & Passport System)
 * --------------------------------------------------
 * [功能] 處理用戶登入、認證狀態維護及誠信護照詳細資訊。
 * [語言] 雙語支援 (繁體中文 / 英文)
 */

import React, { useState, createContext, useContext } from 'react';
import { motion } from 'framer-motion';
import { User as UserIcon, Lock, Mail, Phone, Building, MapPin } from 'lucide-react';
...
interface AuthContextType {
  ...
  login: (email: string, passface: string) => Promise<boolean>;
  logout: () => void;
  ...
  const login = async (email: string, passphrase: string): Promise<boolean> => {
    ...
    if (email && passphrase) {
      const mockUser: UserPassport = {
        ...
        company: '鼎俊永續科技股份有限公司',
        position: 'ESG 首席架構師',
        address: '台北市信義區忠孝東路',
        avatar: '',
        esgProfile: {
          knowledgeLevel: 85,
          interestTags: ['環境', '社會性', '循環經濟'],
          learningPreferences: ['閱讀', '視覺化'],
        },
...
export const LoginPage: React.FC<{
...
}> = ({ onLoginSuccess, language, onToggleLanguage }) => {
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    setIsLoading(true);
    const success = await login(email || 'demo@esgss.com', password || 'demo1234');
    setIsLoading(false);
    if (success) {
      if (onLoginSuccess) onLoginSuccess({ name: 'Demo User', role: 'Developer' });
    } else {
      alert('登入失敗，請檢查您的電子郵件與密碼。');
    }
  };

  const quickEntry = async () => {
    setIsLoading(true);
    // Simulate delay for ritualistic feel
        ...
        <div className="bg-slate-900/40 backdrop-blur-2xl border border-slate-800 p-8 rounded-[32px] shadow-2xl relative overflow-hidden">
          ...
          <div className="mt-8 pt-6 border-t border-slate-800">
            ...
            >
              ...
              <span className="text-sm tracking-widest">快速進入展示模式 (DEMO)</span>
            </button>
...
export const UserPassportPage: React.FC = () => {
  ...
  return (
    <div className="p-6 bg-gradient-to-br from-slate-50 to-slate-100 min-h-screen">
      ...
      >
        <h1 className="text-3xl font-bold text-slate-800 mb-8">誠信護照詳細資訊 / User Passport</h1>
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8">
          <h3 className="text-xl font-bold text-slate-800 mb-4">
            ESG 知識能力分析 / ESG Learning Profile
          </h3>
          <div className="space-y-4">
            <div>
              <div className="flex items-center justify-between mb-2">
                <span className="text-slate-700">知識評估百分比 / Knowledge Level</span>
            ...
            <div>
              <span className="text-slate-700 block mb-2">感興趣的領域 / Interest Tags</span>
              <div className="flex flex-wrap gap-2">
                {user.esgProfile.interestTags.map((tag, index) => (
                  <span
                    key={index}
