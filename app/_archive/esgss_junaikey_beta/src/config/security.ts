...
export const securityConfig = {
  ...
  helmet: {
    contentSecurityPolicy: {
      directives: {
        ...
        scriptSrc: ["'self'", "'unsafe-inline'"], // Removed 'unsafe-eval' for hardening
        imgSrc: ["'self'", 'data:', 'https:', 'blob:'],
        connectSrc: [
          "'self'",
          'https://api.openai.com',
          'https://*.firebaseio.com',
          'https://generativelanguage.googleapis.com' // Added for OmniGemini
        ],
        frameSrc: ["'none'"],
        objectSrc: ["'none'"],
    ...
    noSniff: true,
    xssFilter: true,
    referrerPolicy: { policy: 'same-origin' }, // Hardened from strict-origin-when-cross-origin
  },
  ...
  sensitiveData: {
    ...
    maskPatterns: [/password/i, /secret/i, /token/i, /key/i, /ssn/i, /social.?security/i],
  },
...
export function loadSecurityConfig(): typeof securityConfig {
  ...
  if (process.env.NODE_ENV === 'production') {
    // 這裡可以從安全的配置服務加載
    omniLogger.info(LogCategory.SYSTEM, '加載生產環境安全配置');
  }
...
export class SecurityLogger {
  static logSecurityEvent(event: {
    ...
    // 在生產環境，這裡會發送到安全日誌服務
    omniLogger.warn(LogCategory.SECURITY, `[SECURITY] ${event.type}`, logEntry);
  ...
  static triggerSecurityAlert(event: any): void {
    // 在生產環境，這裡會觸發安全警報
    omniLogger.error(LogCategory.SECURITY, '[SECURITY ALERT] Critical Event', event);
