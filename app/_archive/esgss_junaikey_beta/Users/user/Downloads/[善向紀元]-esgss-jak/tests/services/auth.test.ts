...
import { AuthService, UserRole, Permission, RolePermissions, authService } from '../../services/auth';
...
describe('AuthService', () => {
  ...
  describe('Permission Checking', () => {
    ...
    it('should check any permission correctly', () => {
      expect(authServiceInstance.hasAnyPermission([Permission.VIEW_ANALYTICS, Permission.ADMIN_ACCESS])).toBe(true);
      expect(authServiceInstance.hasAnyPermission([Permission.ADMIN_ACCESS, Permission.USER_MANAGEMENT])).toBe(false);
    });

    it('should check all permissions correctly', () => {
      expect(authServiceInstance.hasAllPermissions([Permission.VIEW_ANALYTICS, Permission.RUN_ANALYSIS])).toBe(true);
      expect(authServiceInstance.hasAllPermissions([Permission.VIEW_ANALYTICS, Permission.ADMIN_ACCESS])).toBe(false);
    });
  });
  ...
  describe('Token Management', () => {
    ...
    it('should store tokens in correct storage', () => {
      ...
      expect(sessionStorageMock.setItem).toHaveBeenCalledWith('esg_auth_token', JSON.stringify(token));

      // Test localStorage (rememberMe = true)
      (authServiceInstance as any).storeTokens(token, true);
      expect(localStorageMock.setItem).toHaveBeenCalledWith('esg_auth_token', JSON.stringify(token));
    });
  });
