...
router.get('/logs', async (req, res) => {
  try {
    ...
    const logs = await getLogs(service, level, parseInt(limit), startTime, endTime);
...
router.get('/alerts', async (req, res) => {
  try {
    ...
    const alerts = await getAlerts(status, severity, parseInt(limit));
...
async function getLogs(service, level, limit, startTime, endTime) {
...
async function createAlert(type, severity, message, service, metadata) {
  const alert = {
    id: `alert-${Date.now()}`,
    type,
    severity,
