...
const mockAnalytics = {
  dashboard: {
    ...
    topContent: [
      { title: 'ESG 基礎課程', views: 450, engagement: 85 },
      { title: '碳足跡計算指南', views: 380, engagement: 78 },
      { title: '永續投資策略', views: 320, engagement: 82 }
    ],
    userDemographics: {
      roles: {
        admin: 15,
  ...
  try {
    ...
    const esgAnalytics = {
      ...
      benchmarks: {
        ...
        peerComparison: '優於 68% 同業',
        globalAverage: 65.2
      },
      topIssues: [
        { issue: '碳排放管理', companies: 89, severity: 'high' },
        { issue: '供應鏈透明度', companies: 76, severity: 'medium' },
        { issue: '員工多元性', companies: 64, severity: 'medium' }
      ],
      companyId,
      period
    };
...
router.get('/learning', async (req, res) => {
  try {
    ...
    const learningAnalytics = {
      ...
      topCourses: mockAnalytics.dashboard.topContent.map(item => ({
        title: item.title,
...
router.get('/trends', async (req, res) => {
  try {
    ...
    if (metric) {
      trendsData = {
        [metric]: mockAnalytics.trends[metric] || []
      };
    }
