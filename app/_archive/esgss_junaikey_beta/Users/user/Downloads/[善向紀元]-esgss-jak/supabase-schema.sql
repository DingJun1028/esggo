...
-- B2. 佐證資料表 (連結 Storage)
create table public.esg_evidence (
    id uuid primary key default uuid_generate_v4(),
...
returns table (current_role text, current_org_id uuid)
security definer
as $$
begin
    return query
...
for select using (
    exists (
        ...
        where current_role in ('admin', 'auditor')
           or (current_role = 'editor' and current_org_id = esg_readings.org_unit_id)
    )
);
...
for update using (
    exists (
        ...
        where current_role = 'admin'
           or (
               current_role = 'editor'
               and current_org_id = esg_readings.org_unit_id
