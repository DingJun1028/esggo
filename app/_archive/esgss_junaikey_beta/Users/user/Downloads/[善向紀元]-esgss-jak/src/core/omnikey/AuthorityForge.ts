...
export class AuthorityForge {
  ...
  async observeBehavior(
    ...
    // 更新模式統計
    pattern.frequency++;
    pattern.timeSpent = (pattern.timeSpent * (pattern.frequency - 1) + duration) / pattern.frequency;
  ...
  private async forgeAuthorityKey(pattern: BehaviorPattern): Promise<AuthorityKey> {
    console.log(`🔨 開始冶煉權能鑰匙: ${pattern.pattern}`);
    ...
    console.log(`✨ 權能鑰匙冶煉完成: ${authorityKey.name}`);
    return authorityKey;
  }
  ...
  private async evolveAuthority(
    ...
    console.log(`🧬 權能進化: ${key.name} (${oldPower} -> ${key.power})`);
  }
  ...
  private shouldForgeAuthority(pattern: BehaviorPattern): boolean {
    ...
    // 檢查是否已有相似權能
    const existingKeys = Array.from(this.authorityKeys.values())
      .filter(key => key.userId === pattern.userId);

    const similarKey = existingKeys.find(key =>
      this.calculateSimilarity(key.pattern, pattern) > 0.8
    );
...
def generate_report(data, template_path):
  ...
  private calculateComplexity(action: string, context: string, metadata: Record<string, any>): number {
    let complexity = 1;
  ...
  private calculateSimilarity(pattern1: BehaviorPattern, pattern2: BehaviorPattern): number {
    // 簡化的相似度計算
    const tagSimilarity = pattern1.tags.filter(tag => pattern2.tags.includes(tag)).length /
                         Math.max(pattern1.tags.length, pattern2.tags.length);

    const actionSimilarity = pattern1.pattern.includes(pattern2.pattern.split(' ')[0]) ? 0.5 : 0;
