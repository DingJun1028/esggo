...
export { NavigationAgent, type Task, type ExecutionResult } from './NavigationAgent';
export { AuthorityForge, type AuthorityKey, type BehaviorObservation } from './AuthorityForge';
export { RuneEngrafter, type Rune, type EngraftedRune } from './RuneEngrafter';
export {
  OmniKeyCore,
  type MysticCycle,
  type CycleResult,
...
export class JunAIKey {
  ...
  constructor(userId: string) {
    ...
    this.navigationAgent = new NavigationAgent(userId);
    this.authorityForge = new AuthorityForge(userId);
    ...
    console.log('🔑 Jun.AI.Key 系統初始化完成');
  }
   ...
   * 記憶存儲
   */
  async storeMemory(content: string, type: string, context: string, tags: string[] = [], userId?: string) {
   ...
   * 執行任務
   */
  async executeTask(description: string, type: 'analysis' | 'creation' | 'automation' | 'learning' | 'execution' = 'execution') {
  ...
  async forgeAuthorityKey(pattern: string, userId?: string) {
    const targetUserId = userId || 'default-user';
    return await this.authorityForge.forgeAuthorityKey(targetUserId, pattern);
  }

  /**
   * 註冊符文
   */
  registerRune(rune: Omit<import('./RuneEngrafter').Rune, 'id' | 'engraveDate' | 'usageCount' | 'successRate'>) {
 ...
 * 快速啟動函數
 */
export async function quickActivate(trigger: string, context?: Record<string, any>): Promise<string> {
