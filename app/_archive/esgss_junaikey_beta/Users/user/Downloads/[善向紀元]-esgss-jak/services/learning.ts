...
export class LearningService {
  ...
  private userProgress: Map<string, { [itemId: string]: { completed: boolean; score?: number; completedAt?: Date } }> = new Map();

  // Knowledge management
  addKnowledgeItem(item: Omit<KnowledgeItem, 'id' | 'createdAt' | 'updatedAt' | 'views' | 'likes'>): string {
  ...
  searchKnowledge(query: string, filters?: { category?: string; type?: string; difficulty?: string; tags?: string[] }): KnowledgeItem[] {
  ...
  getUserProgress(userId: string): { [itemId: string]: { completed: boolean; score?: number; completedAt?: Date } } {
  ...
  takeQuiz(userId: string, quizId: string, answers: number[]): { score: number; passed: boolean; feedback: string[] } {
  ...
  getLearningAnalytics(userId: string): {
    ...
    return {
      ...
      averageScore: completedItems.reduce((sum, p) => sum + (p.score || 0), 0) / completedItems.length || 0,
