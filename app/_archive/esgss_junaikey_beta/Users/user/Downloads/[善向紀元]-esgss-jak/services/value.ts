...
export class ValueService {
  ...
  calculatePropositionROI(proposition: ValueProposition): number {
    ...
    return ((proposition.expectedRevenue - proposition.implementationCost) / proposition.implementationCost) * 100;
  }
  ...
  getValuePortfolioSummary(): {
    ...
    const averageROI = metrics.length > 0 ? metrics.reduce((sum, m) => sum + this.calculateROI(m), 0) / metrics.length : 0;

    const topPerformingMetrics = metrics
      .sort((a, b) => this.calculateROI(b) - this.calculateROI(a))
      .slice(0, 5);

    const riskDistribution = investments.reduce((dist, i) => {
      dist[i.riskAssessment] = (dist[i.riskAssessment] || 0) + 1;
      return dist;
    }, {} as { [risk: string]: number });

    const categoryBreakdown = investments.reduce((breakdown, i) => {
  ...
  forecastValueGrowth(metricId: string, periods: number): { period: number; projectedValue: number; confidence: number }[] {
    const metric = this.metrics.get(metricId);
    if (!metric) return [];

    const growthRate = metric.trend === 'increasing' ? 0.05 : metric.trend === 'decreasing' ? -0.02 : 0;
    const forecast = [];
  ...
  getIndustryBenchmarks(category: string): {
    ...
    const benchmarks = {
      ...
      strategic: [
        { averageROI: 35, topQuartile: 60, median: 32, industry: 'Consulting' }
      ],
      reputational: [
        { averageROI: 18, topQuartile: 28, median: 15, industry: 'Consumer Goods' }
      ]
    };
  ...
  getValueOptimizationRecommendations(): string[] {
    ...
    if (portfolio.riskDistribution.high > portfolio.riskDistribution.low * 2) {
      recommendations.push('Portfolio risk is skewed towards high-risk investments - diversify risk levels');
    }

    const lowPerformingMetrics = Array.from(this.metrics.values())
      .filter(m => this.calculateROI(m) < 0);

    if (lowPerformingMetrics.length > 0) {
      recommendations.push(`Review ${lowPerformingMetrics.length} underperforming value metrics for optimization or termination`);
    }
