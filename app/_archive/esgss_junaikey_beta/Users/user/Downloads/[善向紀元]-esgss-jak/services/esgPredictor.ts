...
export class ESGPredictor {
  ...
  async predict(input: PredictionInput): Promise<PredictionResult> {
    const { metric, data, scenario, horizon, confidenceThreshold } = input;
    ...
    if (confidence < confidenceThreshold) {
      console.warn(`預測信心度不足: ${confidence.toFixed(2)} < ${confidenceThreshold}`);
    }

    return {
      metric,
      currentValue: processedData[processedData.length - 1],
  ...
  private movingAverage(
    ...
    const window = Math.min(data.length, 3);
    ...
    return {
      value: average,
      uncertainty,
      details: { method: 'moving_average', window }
    };
  }
  ...
  private simpleExtrapolation(
    ...
    const extrapolatedValue = recent[recent.length - 1] + trend * horizon;
  ...
  private calculateConfidence(prediction: any, data: number[]): number {
    // 基於預測不確定性和數據品質計算信心度
    const uncertaintyRatio = prediction.uncertainty / Math.abs(prediction.value);
    ...
    // 情境調整
    if (prediction.details.adjustment !== 1.0) {
  ...
  private generateDataPoints(
    ...
    historical.forEach((value, index) => {
      points.push({
        date: new Date(Date.now() - (historical.length - index) * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        value,
        predicted: false
      });
    });
    ...
    for (let i = 1; i <= horizon; i++) {
      const predictedDate = new Date(Date.now() + i * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
  ...
  async isHealthy(): Promise<boolean> {
    try {
      ...
      await this.predict({
        metric: 'test',
        data: testData.map((v, i) => ({ date: `2024-${i + 1}-01`, value: v })),
        scenario: 'baseline',
        horizon: 1,
      ...
      console.error('ESG預測器健康檢查失敗:', error);
      return false;
    }
  }
}