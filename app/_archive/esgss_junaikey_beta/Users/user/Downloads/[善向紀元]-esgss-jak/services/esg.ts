...
export class ESGCalculator {
  ...
  // 計算永續發展目標 (SDGs) 貢獻度
  static calculateSDGContribution(actions: Array<{
    sdg: number;
    impact: number;
    confidence: number;
    ...
    actions.forEach(action => {
      if (!contributions[action.sdg]) {
        contributions[action.sdg] = 0;
      }
      contributions[action.sdg] += action.impact * action.confidence;
    });
  ...
  static calculateDiversityIndex(workforce: {
    ...
    const ethnicityEntropy = -ethnicityDistribution.reduce((sum, p) => sum + (p > 0 ? p * Math.log2(p) : 0), 0);
  ...
  // 計算供應鏈風險評分
  static calculateSupplyChainRisk(suppliers: Array<{
    name: string;
    country: string;
    riskFactors: {
  ...
  static assessClimateRisk(exposure: {
    ...
    const transitionRisk = (exposure.transition.policy + exposure.transition.technology + exposure.transition.market) / 3;
    const opportunities = (exposure.opportunities.resourceEfficiency + exposure.opportunities.energyTransition + exposure.opportunities.productInnovation) / 3;
  ...
  // 計算利益相關者參與度
  static calculateStakeholderEngagement(stakeholders: Array<{
...
export class ESGDataValidator {
  ...
  // 驗證治理評分
  static validateGovernanceData(data: Partial<GovernanceScore>): { isValid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    const scoreFields = ['boardComposition', 'executiveCompensation', 'shareholderRights', 'auditQuality', 'riskManagement'];
  ...
  static checkDataConsistency(reports: ESGReport[]): {
    ...
    for (let i = 1; i < sortedReports.length; i++) {
      ...
      // 社會指標連續性檢查
      if (Math.abs(current.social.employeeSatisfaction - previous.social.employeeSatisfaction) > 30) {
        inconsistencies.push({
          reportId: current.id,
...
export class ESGReportGenerator {
  static generateAnnualReport(
    ...
    const social = {
      employeeSatisfaction: socialMetrics.find(m => m.category === 'employees' && m.metric === 'satisfaction')?.value || 0,
      diversityIndex: socialMetrics.find(m => m.category === 'diversity' && m.metric === 'index')?.value || 0,
      communityInvestment: socialMetrics.find(m => m.category === 'community' && m.metric === 'investment')?.value || 0,
      supplyChainEthics: socialMetrics.find(m => m.category === 'supply_chain' && m.metric === 'ethics')?.value || 0,
      humanRights: socialMetrics.find(m => m.category === 'supply_chain' && m.metric === 'human_rights')?.value || 0
    };

    // 治理評分
    const governance = {
      transparencyScore: data.governanceData.boardComposition * 0.8 + data.governanceData.auditQuality * 0.2,
      stakeholderEngagement: data.governanceData.shareholderRights * 0.7 + data.governanceData.executiveCompensation * 0.3,
      ethicalConduct: data.governanceData.riskManagement,
      regulatoryCompliance: (data.governanceData.auditQuality + data.governanceData.riskManagement) / 2
    };
    ...
    const overallScore = ESGCalculator.calculateOverallScore(environmentalScore, socialScore, governanceScore);
  ...
  private static calculateEnvironmentalScore(data: typeof environmental): number {
    // 簡化的環境評分計算
  ...
  private static calculateSocialScore(data: typeof social): number {
    return (
