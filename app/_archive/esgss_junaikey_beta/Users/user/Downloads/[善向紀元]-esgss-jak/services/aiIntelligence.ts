...
export class AIIntelligenceService {
  ...
  async predictESGMetrics(
    ...
    for (const metric of metrics) {
      ...
      if (!data || data.length < 12) { // 需要至少12個月數據
        console.warn(`數據不足，跳過 ${metric} 預測`);
        continue;
      }
      ...
      for (const scenario of scenarios) {
        try {
          ...
          console.error(`預測失敗 ${metric} - ${scenario}:`, error);
        }
      }
    }
  ...
  async analyzeText(
    ...
    return await this.nlpAnalyzer.analyze(text, {
      ...context,
  ...
  async analyzeMultipleTexts(
    ...
    for (const item of texts) {
      try {
        ...
        console.error(`分析失敗 ${item.id}:`, error);
  ...
  async identifyOpportunities(
    ...
    analysisScope: ('environmental' | 'social' | 'governance')[] = ['environmental', 'social', 'governance']
