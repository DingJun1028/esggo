...
exec('curl -s -o /dev/null -w "%{http_code}" http://localhost:3000', (error, stdout) => {
  ...
  console.log('\n2. 檢查JunAiKey服務狀態...');
  exec(
    'curl -s -H "x-celestial-token: celestial-access-2024" -X POST http://localhost:3003/api/manifest',
    (error, stdout) => {
      try {
      ...
      exec(
        `curl -s -H "x-celestial-token: celestial-access-2024" "http://localhost:3003/api/interact?message=${testMessage}&sessionId=test-session"`,
        (error, stdout) => {
