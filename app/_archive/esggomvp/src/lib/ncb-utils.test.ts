...
import { extractAuthCookies, getRlsPolicies } from "./ncb-utils";
...
describe("getRlsPolicies", () => {
  it("returns expected RLS policies configuration", async () => {
    const policies = await getRlsPolicies("some-instance");

    expect(policies).toBeDefined();
    expect(typeof policies).toBe("object");

    // Test specific policies returned
    expect(policies["suppliers"]).toEqual({ public_read: true, public_write: false });
    expect(policies["village_supply"]).toEqual({ public_read: true, public_write: false });
    expect(policies["carbon_data"]).toEqual({ public_read: true, public_write: true, owner_scoped: true });
    expect(policies["test_table"]).toEqual({ public_read: true, public_write: true });
  });
});
