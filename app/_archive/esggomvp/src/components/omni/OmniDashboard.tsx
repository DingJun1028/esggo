...
export const OmniDashboard: React.FC = () => {
    ...
    useEffect(() => {
        async function loadData() {
            try {
                const s = await analyticsEngine.getDashboardSummary();
                const m = await analyticsEngine.getKeyMetrics();
                setSummary(s);
                setMetrics(m);
            } catch (err) {
    ...
    if (loading) return (
        <div className="min-h-screen bg-omni-bg flex items-center justify-center">
            <div className="text-omni-primary font-black animate-pulse tracking-[0.5em] text-2xl uppercase">LOADING QUANTUM MATRIX...</div>
        </div>
    );
        ...
        <div className="min-h-screen bg-omni-bg p-8 font-sans text-omni-text-main">
            <div className="max-w-7xl mx-auto mb-12">
                <div className="flex items-center gap-4 mb-2">
                    <div className="w-12 h-12 rounded-2xl bg-omni-primary/10 flex items-center justify-center border border-omni-primary/30 shadow-[0_0_20px_rgba(var(--color-omni-primary),0.2)]">
                    ...
                    <div className="flex-grow max-w-xs space-y-2 relative z-10">
                        ...
                        <div className="h-1.5 w-full bg-omni-surface-2 rounded-full overflow-hidden border border-omni-glass-border">
                            <motion.div
                                ...
                                className="h-full bg-omni-primary shadow-[0_0_10px_rgba(var(--color-omni-primary),0.5)]"
                            />
                        </div>
                    </div>
                    <div className="flex-grow max-w-xs space-y-2 relative z-10">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-stretch">
                        <div className="space-y-10">
                            <LiquidGlassContainer className="p-8">
                                <GnosisHeatmap data={mockHeatmapData} title="Global Gnosis Impact Matrix" />
                            </LiquidGlassContainer>
                            <LiquidGlassContainer className="p-8">
