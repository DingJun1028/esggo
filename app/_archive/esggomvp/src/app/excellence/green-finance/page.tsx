    ...
    return (
        <div className="max-w-7xl mx-auto space-y-12 pb-24 text-[var(--foreground)]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 p-8 md:p-12 rounded-[3rem] md:rounded-[3.5rem] bg-gradient-to-br from-amber-400/10 via-[var(--background)] to-emerald-500/5 border border-amber-400/30 liquid-glass relative overflow-hidden min-h-[400px] md:h-[450px] shadow-2xl">
                    <div className="absolute top-0 right-0 p-8 opacity-5">
                        <TrendingUp size={300} className="text-amber-400" />
                    </div>

                    <div className="relative z-10 flex flex-col h-full justify-between">
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-amber-500">Current ESG Alpha Coefficient</p>
                            <motion.h2
                                animate={{ opacity: [1, 0.7, 1] }}
                                transition={{ duration: 3, repeat: Infinity }}
                                className="text-5xl md:text-7xl font-black italic text-[var(--foreground)] tracking-tighter"
                            >
                                {alphaRatio}x
                            </motion.h2>
                            <p className="text-xs text-emerald-400 font-bold mt-2 uppercase">Outperforming Benchmark by 18.2%</p>
                        </div>
                    ...
                    <div className="space-y-4">
                        {instruments.map((ins, i) => (
                            <motion.div
                                ...
                                className="p-6 rounded-[2rem] bg-[var(--card-bg)] border border-[var(--card-border)] group hover:border-amber-400/30 transition-all cursor-pointer shadow-md hover:shadow-lg"
                            >
                                <div className="flex justify-between items-center mb-4">
                                    <div className="p-3 bg-amber-400/10 text-amber-500 rounded-2xl group-hover:bg-amber-400 group-hover:text-[var(--background)] transition-all shadow-inner">
                                        {ins.icon}
                                    </div>
                                    <ArrowUpRight size={20} className="text-[var(--sidebar-text)] group-hover:text-amber-400 transition-colors" />
                                </div>
                                <h5 className="text-[10px] font-black text-[var(--sidebar-text)] uppercase tracking-widest mb-1">{ins.t}</h5>
                                <p className="text-base font-bold text-[var(--foreground)] uppercase italic">{ins.v}</p>
                                <p className="text-[10px] text-emerald-400 font-bold mt-2">{ins.rate}</p>
                            </motion.div>
                        ))}
                    </div>
                </div>
