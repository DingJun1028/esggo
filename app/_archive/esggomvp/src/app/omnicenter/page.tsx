...
export default function OmniCenter() {
  ...
  return (
    <div className="min-h-screen p-4 lg:p-8">
      ...
      >
        <div>
          <h1 className="text-3xl md:text-4xl font-black tracking-tighter text-[var(--primary)]">OmniCenter</h1>
          <p className="text-[var(--sidebar-text)] text-sm mt-1">{t.nav.dashboard === '儀表板' ? '萬能中心：全域服務導航' : 'Universal Center: Global Service Navigation'}</p>
        </div>
        ...
        <div className="xl:col-span-2 flex flex-col gap-6">
          ...
          >
            ...
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden">
              <div className="w-64 h-64 rounded-full border border-[var(--primary)] opacity-20 animate-spin-slow absolute" />
              <div className="w-48 h-48 rounded-full border border-[var(--accent)] opacity-10 animate-spin-slow absolute" style={{ animationDirection: 'reverse', animationDuration: '15s' }} />
              <div className="absolute w-full h-full bg-[radial-gradient(circle_at_center,var(--primary)_0%,transparent_50%)] opacity-5" />
            </div>
            ...
            <div className="z-10 text-center flex flex-col items-center">
              ...
              <h2 className="text-3xl font-black text-[var(--foreground)] mb-6">5T Protocol Core</h2>

              <div className="flex flex-wrap justify-center gap-3 max-w-lg">
                {['Traceable', 'Trackable', 'Transparent', 'Trustworthy', 'Tangible'].map(p => (
                  <div key={p} className="px-4 py-2 rounded-xl bg-[var(--card-bg)] border border-[var(--card-border)] shadow-sm text-xs font-bold text-[var(--sidebar-text)] hover:text-[var(--primary)] hover:border-[var(--primary)] transition-all cursor-default">
                    {p}
                  </div>
                ))}
              </div>
              ...
              >
                <Link href={cat.href} className="block h-full">
                  ...
                  >
                    ...
                    <h2 className="text-xl font-black mb-3 text-omni-text-main group-hover:text-omni-primary transition-colors">
                      {cat.title}
                    </h2>
                    <p className="text-omni-text-muted text-sm leading-relaxed mb-8 flex-grow">
                      {cat.description}
                    </p>
