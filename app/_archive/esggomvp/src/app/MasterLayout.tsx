...
const mainNavIds = [
    ...
    // { id: 'omni_space', icon: Globe, href: '/omni/space', fallbackLabel: 'OmniSpace 時空座標' },
    // { id: 'impact', icon: Globe, href: '/impact/village-square', fallbackLabel: '善向永續村落' },
];
...
export default function MasterLayout({ children }: { children: React.ReactNode }) {
    ...
    // 🌊 Get resonant color based on path
    const getResonantColor = () => {
        if (pathname.includes('governance')) return 'rgba(147, 51, 234, 0.05)';
        if (pathname.includes('impact')) return 'var(--theme-primary-muted)';
    ...
    return (
        <div className="flex h-[100dvh] font-sans overflow-hidden transition-colors duration-500 bg-[var(--theme-bg)] text-[var(--theme-text-main)]">
            {/* 🌌 Global Resonance Background - Apple Style Subtlety */}
            <div
                className="fixed inset-0 pointer-events-none transition-colors duration-1000 z-0"
                style={{
                    background: `radial-gradient(circle at 50% 10%, ${getResonantColor()} 0%, transparent 80%)`
                }}
            />
                        ...
                        <div className="p-6 mb-4 flex items-center justify-between">
                            <AnimatePresence mode="wait">
                                {(!isCollapsed || isMobileMenuOpen) && (
                                    ...
                                    >
                                        <div className="size-10 bg-[var(--theme-primary)] rounded-xl flex items-center justify-center text-white shadow-lg shadow-blue-500/20">
                                            <LayoutDashboard size={22} />
                                        </div>
                                        <div>
                                            <h1 className="text-omni-text-main text-lg font-black tracking-tight leading-none">ESG Go</h1>
                                            <div className="flex items-center gap-2 mt-1">
                                                <p className="text-omni-primary text-[10px] uppercase tracking-widest font-bold">OmniCenter</p>
                                            </div>
                                        </div>
                                    </motion.div>
                                )}
                            ...
                            {navItems.map((item) => {
                                ...
                                return (
                                    <Link
                                        key={item.id}
                                        href={item.href}
                                        className={`flex items-center p-3 rounded-2xl group transition-all ${isActive ? 'bg-omni-primary text-white shadow-lg shadow-blue-500/25' : 'text-omni-text-sub hover:bg-omni-primary-muted hover:text-omni-primary'} ${isCollapsed && !isMobileMenuOpen ? 'justify-center' : 'gap-4'}`}
                                    >
                                        <Icon size={20} className={isActive ? 'text-white' : 'group-hover:text-omni-primary'} />
                                        {(!isCollapsed || isMobileMenuOpen) && (
                                            <span className={`font-bold tracking-tight text-sm truncate whitespace-nowrap ${isActive ? 'text-white' : 'text-current'}`}>
                                                {item.label}
                                            </span>
                                        )}
                                    </Link>
                        ...
                        <div className="p-4 mt-auto border-t border-omni-glass-border">
                            {status === 'authenticated' && session?.user ? (
                                <div className={`flex items-center p-2 rounded-2xl bg-omni-surface-2 ${isCollapsed && !isMobileMenuOpen ? 'justify-center' : 'gap-3'}`}>
                                    {session.user.image ? (
                                        <img src={session.user.image} alt={session.user.name || ''} className="w-9 h-9 rounded-full ring-2 ring-omni-primary/10" />
                                    ) : (
                                        <div className="w-9 h-9 rounded-full bg-omni-primary-muted flex items-center justify-center text-xs font-bold text-omni-primary">
