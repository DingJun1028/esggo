...
import { CardArena } from "@/components/omni/cards/CardArena";
import { CardForge } from "@/components/omni/cards/CardForge";
import { Marketplace } from "@/components/omni/cards/Marketplace";

type TabId = "rules" | "arena" | "forge" | "marketplace" | "starter" | "partners" | "binder";
...
export default function OmniCardsPage() {
    ...
    const tabs = [
        { id: "arena" as TabId, icon: "⚔️", label: "競技大廳", labelEn: "RPG Arena" },
        { id: "forge" as TabId, icon: "🔥", label: "知識熔爐", labelEn: "Card Forge" },
        { id: "marketplace" as TabId, icon: "⚖️", label: "交易所", labelEn: "Orbital Exchange" },
        { id: "binder" as TabId, icon: "📔", label: "我的收藏", labelEn: "Collection" },
        { id: "rules" as TabId, icon: "📖", label: "遊戲說明", labelEn: "Game Guide" },
        { id: "starter" as TabId, icon: "🃏", label: "基本套牌", labelEn: "Starter Deck" },
        { id: "partners" as TabId, icon: "✨", label: "企業牌", labelEn: "Partners" },
    ];

    return (
        <div className="flex flex-col gap-6 w-full animate-in fade-in duration-700 pb-20">
            ...
            <section className="text-center py-4 sm:py-6">
                ...
                <h1 className={`text-3xl md:text-4xl font-light tracking-wide mb-2 ${T.main}`}>
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-amber-500 font-bold">萬能卡牌</span>
                    {" "}遊戲生態系
                </h1>
                <p className={`text-sm font-mono ${T.muted}`}>
                    Forge · Trade · Battle · Collect | ESG GO Omni System
                </p>
            </section>
                    ...
                    {tabs.map(tab => (
                        <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                            className={`flex-none flex items-center justify-center gap-2 py-2.5 sm:py-3 px-6 rounded-xl text-sm font-medium transition-all duration-300 ${activeTab === tab.id
                                ? "bg-gradient-to-r from-cyan-500/20 to-amber-500/20 border border-cyan-500/30 text-omni-text-main shadow-lg shadow-cyan-500/10"
            ...
            {/* ══ NEW TABS ══ */}
            {activeTab === "arena" && <CardArena />}
            {activeTab === "forge" && <CardForge />}
