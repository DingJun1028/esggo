    ...
    private setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (req) => {
            if (req.params.name === "forge_celestial_artifact") {
                ...
                const artifact = CelestialLifecycleManager.forgeInit(payload, originator);
                this.library.set(artifact._core.uuid, artifact);
                return {
                    content: [{ type: "text", text: JSON.stringify({ status: "FORGED", uuid: artifact._core.uuid }) }]
                };
            }
