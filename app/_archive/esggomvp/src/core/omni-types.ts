...
/**
 * 💡 萬能元件心核：UCC (Universal Component Core)
 * 貫徹「知識即資產」：所有 IComponentCore 實體皆為可交易/證明的資產單元
 */
export interface IComponentCore<T = Record<string, unknown>> {
  // 1. 唯一識別心核 (Identity)
  readonly uuid: string;           // [可溯源] 來自萬能永憶主體
  readonly version: string | number; // 語義化版本 (e.g., "1.0.0-verified")
  readonly timestamp: number;      // [可追蹤] 學習刻印時間戳

  // 2. 證據左證庫 (Traceable - 可溯源)
  readonly evidence: {
    origin_id?: string;          // 原始憑證 ID
    origin_hash?: string;        // 原始憑證的 SHA-256 指紋
    extraction_method?: 'OCR' | 'IoT' | 'Manual'; // 數據是如何產生的？
    verifier_signature?: string; // 第三方確信者的數位簽章
    [key: string]: string | number | boolean | object | undefined; // Fallback
  };

  // 3. 實作生命週期 Hook (Trackable - 可追蹤)
  lifecycle: Array<ILifecycleEvent<T>>;

  // 4. 數據本體
  data?: T;

  // 5. 不可篡改鎖 (Trust)
  isFrozen: boolean;            // 是否已執行 Object.freeze()

  // --- Legacy Compatibility ---
  readonly impactMetric?: string;
  readonly status?: OmniStatus;
