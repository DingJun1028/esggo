...
async function verifyOmniOne() {
    ...
    console.log('\n  📤 manifest_asset...');
    const manifestResult = await OmniOne.dispatch('manifest_asset', {
        intent: 'OmniOne Verification Test',
        payload: { test: 'verify-omni-one', status: 'active' }
    });
    console.log(`  ✅ manifest_asset: ${manifestResult.success ? 'PASS' : 'FAIL'}`);
    if (manifestResult.metadata?.uuid) {
        console.log(`     UUID: ${manifestResult.metadata.uuid}`);
    }
...
verifyOmniOne().catch(console.error);
