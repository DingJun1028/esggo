...
export class ReportService {
    ...
    public static async generateEliteReport(
        ...
        // Step 3: PERSISTENT INTEGRATION - Save to specific reports table for dashboard visibility
        try {
            await OmniNcbService.saveReport({
                title: title,
                company_name: "DingJun Hong",
                reporting_year: new Date().getFullYear(),
                status: 'draft',
                payload: { indicators, auditResult },
                complianceScore: auditResult?.overallScore || 0
            });
            omniLogger.info(LogCategory.SYSTEM, `ReportService: Integrated report [${title}] into NCB Sustainability Repository.`);
        } catch (err) {
            omniLogger.error(LogCategory.SYSTEM, "ReportService: Failed to bridge to NCB Repository", err);
        }
