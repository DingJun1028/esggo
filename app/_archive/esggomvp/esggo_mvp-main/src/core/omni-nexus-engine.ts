...
export class OmniNexusEngine {
    ...
    public forgeCardFromAtom(atom: IOmniAtom<any>): INexusCard {
        ...
        const card: INexusCard = {
            ...
            evidence: [{ origin_id: atom.uuid, origin_hash: atom.hash_lock, extraction_method: 'Manual' }],
            status: 'Potential',
            lifecycle_events: [
                { id: 'nexus-forge-init', action: 'CREATED', source_module: 'Nexus_Forge', timestamp: Date.now(), metadata: { reason: 'Forged from knowledge atom' } }
            ]
        };
    ...
    public convertDbCardToNexusCard(dbCard: OmniCard): INexusCard {
        ...
        return {
            ...
            isEquipped: false,
            isFrozen: true,
            hash_lock: (dbCard as any).hash_lock || 'INITIAL_DB_HASH',
            evidence: [{ db_id: dbCard.id.toString(), source: 'Supabase', framework: dbCard.framework_ref }],
            status: 'Potential',
            lifecycle_events: [
                { id: 'nexus-init', action: 'CREATED', source_module: 'Nexus_DB_Loader', timestamp: Date.now(), metadata: { reason: 'Loaded from prototype database' } }
            ]
        };
    }
