...
export class OmniNcbService {
    ...
    private static getFullUrl(path: string): string {
        return `${this.CONFIG.baseUrl}${this.CONFIG.apiUrl}${path}`;
    }
    ...
    public static async saveReport(report: any): Promise<any> {
        const title = report.title || report.intent || report.payload?.title || 'Untitled Report';
        omniLogger.info(LogCategory.SYSTEM, `NCB: Saving report [${title}] to instance ${this.CONFIG.instance}`);

        try {
            const url = this.getFullUrl(`/create/sustainability_reports?Instance=${this.CONFIG.instance}`);
            const res = await axios.post(url, {
                title: title,
                company_name: report.company_name || report.payload?.company_name || 'InfoOne Corp',
                reporting_year: report.reporting_year || report.payload?.reporting_year || new Date().getFullYear(),
                status: report.status || report.payload?.status || 'draft',
                report_data: report.payload || report,
                compliance_score: report.complianceScore || report.payload?.complianceScore || 0,
    ...
    public static async anchorEvidence(evidence: any): Promise<any> {
        ...
        try {
            const url = this.getFullUrl(`/create/evidence_vault?Instance=${this.CONFIG.instance}`);
    ...
    public static async listReports(): Promise<any[]> {
        try {
            const url = this.getFullUrl(`/list/sustainability_reports?Instance=${this.CONFIG.instance}`);
            const res = await fetch(url, {
                headers: this.getHeaders()
            });
            if (!res.ok) return [];
    ...
    public static async saveOmniDraft(draftData: any): Promise<any> {
        ...
        try {
            const url = this.getFullUrl(`/create/omni_drafts?Instance=${this.CONFIG.instance}`);
            const res = await fetch(url, {
                method: 'POST',
                headers: this.getHeaders(),
                body: JSON.stringify({
    ...
    public static async fetchOmniDraft(): Promise<any> {
        try {
            const url = this.getFullUrl(`/list/omni_drafts?Instance=${this.CONFIG.instance}`);
            const res = await fetch(url, {
                headers: this.getHeaders()
            });
    ...
    public static async fetchSuppliers(): Promise<any[]> {
        try {
            const url = this.getFullUrl(`/list/impact_suppliers?Instance=${this.CONFIG.instance}`);
            const res = await fetch(url, {
                headers: this.getHeaders()
            });
            if (!res.ok) return [];
    ...
    public static async fetchImpactMetrics(): Promise<any> {
        try {
            const url = this.getFullUrl(`/list/impact_metrics?Instance=${this.CONFIG.instance}`);
            const res = await fetch(url, {
                headers: this.getHeaders()
            });
            if (!res.ok) return null;
    ...
    public static async fetchCommunityPosts(): Promise<any[]> {
        try {
            const url = this.getFullUrl(`/list/community_posts?Instance=${this.CONFIG.instance}`);
            const res = await fetch(url, {
                headers: this.getHeaders()
            });
            if (!res.ok) return [];
    ...
    public static async saveComment(comment: {
        ...
        try {
            ...
            const url = this.getFullUrl(`/create/community_comments?Instance=${this.CONFIG.instance}`);
            const res = await fetch(url, {
                method: 'POST',
                headers: this.getHeaders(),
                body: JSON.stringify({
    ...
    public static async saveSupplier(supplier: any): Promise<any> {
        try {
            ...
            // 5. 發送到 NCB
            const url = this.getFullUrl(`/create/impact_suppliers?Instance=${this.CONFIG.instance}`);
            const res = await fetch(url, {
                method: 'POST',
                headers: this.getHeaders(),
    ...
    public static async saveImpactMetrics(metrics: any): Promise<any> {
        try {
            ...
            // 5. 發送到 NCB
            const url = this.getFullUrl(`/create/impact_metrics?Instance=${this.CONFIG.instance}`);
            const res = await fetch(url, {
                method: 'POST',
                headers: this.getHeaders(),
                body: JSON.stringify(lockedData)
    ...
    public static async saveCarbonRecord(record: { scope: number, value: number, description: string }): Promise<any> {
        try {
            const url = this.getFullUrl(`/create/carbon_records?Instance=${this.CONFIG.instance}`);
            const res = await axios.post(url, {
                scope: record.scope,
                value: record.value,
    ...
    public static async listCarbonRecords(): Promise<any[]> {
        try {
            const url = this.getFullUrl(`/list/carbon_records?Instance=${this.CONFIG.instance}`);
    ...
    public static async saveAlchemyProgress(progress: { user_id: string, level: number, points: number, rank: string }): Promise<any> {
        try {
            const url = this.getFullUrl(`/upsert/alchemy_progress?Instance=${this.CONFIG.instance}`);
    ...
    public static async getAlchemyProgress(userId: string): Promise<any> {
        try {
            const url = this.getFullUrl(`/list/alchemy_progress?Instance=${this.CONFIG.instance}&user_id=${userId}`);
            const res = await axios.get(url, {
                headers: this.getHeaders()
            });
    ...
    public static async saveGameScore(score: { player_name: string, score: number, wins: number }): Promise<any> {
        try {
            const url = this.getFullUrl(`/create/impact_nexus_leaderboard?Instance=${this.CONFIG.instance}`);
    ...
    public static async listLeaderboard(): Promise<any[]> {
        try {
            const url = this.getFullUrl(`/list/impact_nexus_leaderboard?Instance=${this.CONFIG.instance}`);
            const res = await axios.get(url, {
                headers: this.getHeaders()
            });
            return res.data.items || [];
    ...
    public static async saveAtom(atom: any): Promise<any> {
        ...
        try {
            // 1. Save to esg_atoms
            const url = this.getFullUrl(`/create/esg_atoms?Instance=${this.CONFIG.instance}`);
            const res = await axios.post(url, {
                uuid: atom.uuid,
    ...
    public static async saveNote(atom: any): Promise<boolean> {
        try {
            const url = this.getFullUrl('/create/notes_atom');
    ...
    public static async listNotes(): Promise<any[]> {
        try {
            const url = this.getFullUrl('/read/notes_atom');
            const response = await fetch(url, {
                method: 'GET',
                headers: this.getHeaders(),
            });
