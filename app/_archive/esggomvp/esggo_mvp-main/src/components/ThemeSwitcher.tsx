...
const themes = [
    { id: 'aqua', name: '善向永續 (Classic)', icon: <Droplets size={16} />, color: '#63a6b0' },
...
export default function ThemeSwitcher() {
    const [currentTheme, setCurrentTheme] = useState('aqua');
    const [isOpen, setIsOpen] = useState(false);

    useEffect(() => {
        // 優先讀取保存的，預設改為 aqua (善向永續經典版) 確保符合系統基調
        const savedTheme = localStorage.getItem('omni-theme') || 'aqua';
        setTheme(savedTheme);
    }, []);
