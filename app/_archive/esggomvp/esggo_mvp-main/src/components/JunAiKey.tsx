...
import { AutoKarmaRepair } from '@/core/auto-karma-repair';
...
export default function JunAiKey({ hideOrb = false }: { hideOrb?: boolean }) {
    ...
    React.useEffect(() => {
        const toggleHandler = () => setIsMenuExpanded((prev: boolean) => !prev);
        window.addEventListener('toggle-junaikey', toggleHandler);

        const handleKeyDown = (e: KeyboardEvent) => {
            if (e.ctrlKey && e.altKey && e.key.toLowerCase() === 'm') {
                e.preventDefault();
                setIsMenuExpanded((prev: boolean) => !prev);
            }
        };
    ...
    const handleRepair = async () => {
        setIsTyping(true);
        setMood('thinking');
        addMessage(locale === 'zh-TW' ? '啟動「果因修復協議」 (Jules 9 Steps)...' : 'Initiating Jules 9-Step Karma Repair...', 'sprite');
        
        try {
            const success = await AutoKarmaRepair.repair('SYSTEM_ENTROPY_DETECTED', { UI_DUPLICATION_FIXED: true });
            if (success) {
                addMessage(locale === 'zh-TW' ? '✨ 果因修復完成！系統已進入 TRANSCENDED 狀態。' : '✨ Karma Repair Complete! System transcended.', 'sprite');
                setMood('happy');
            } else {
                addMessage(locale === 'zh-TW' ? '修復過程中斷，請檢查 API 金鑰。' : 'Repair failed. Check configuration.', 'sprite');
                setMood('sad');
            }
        } catch (error) {
            addMessage('Critical Error during repair.', 'sprite');
        } finally {
            setIsTyping(false);
        }
    };
                            ...
                            {isMenuExpanded && !activeTab && menuItems.map((item, index) => {
                                ...
                                return (
                                    <motion.button
                                        ...
                                        onClick={(e: React.MouseEvent) => { e.stopPropagation(); setActiveTab(item.id); }}
                                        ...
                                        onPointerDownCapture={(e: React.PointerEvent) => e.stopPropagation()} // prevent drag conflict
                                    >
                                        <item.icon size={20} />
                                    </motion.button>
                                );
                                ...
                                {activeTab === 'chat' && (
                                    <div className="space-y-5">
                                        ...
                                        <div className="flex flex-wrap gap-2 mb-6">
                                            {['Analyze ESG', 'Root Cause Fix', 'Scan Risks'].map((action: string) => (
                                                <button
                                                    key={action}
                                                    onClick={() => action === 'Root Cause Fix' ? handleRepair() : handleQuickAction(action)}
                                                    className="px-3 py-1.5 rounded-full bg-omni-primary/10 border border-omni-primary/20 text-[10px] font-black text-omni-primary uppercase tracking-widest hover:bg-omni-primary hover:text-white transition-all"
                                                >
                                                    {action === 'Root Cause Fix' && locale === 'zh-TW' ? '果因修復' : action}
                                                </button>
                                            ))}
                                        </div>
                                        {isTyping && (
                                ...
                                {activeTab === 'gnosis' && (
                                    <div className="space-y-4">
                                        <div className="flex items-center gap-2 bg-omni-surface-2 border border-omni-glass-border rounded-full px-4 py-3 shadow-inner">
                                            ...
                                            <input
                                                value={gnosisQuery}
                                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setGnosisQuery(e.target.value)}
                                    ...
                                    <div className="space-y-6 pt-2">
                                        <div className="space-y-3">
                                            ...
                                            <input
                                                type="password"
                                                value={julesKeyInput}
                                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setJulesKeyInput(e.target.value)}
                                            ...
                                            />
                                            <p className="text-[10px] text-omni-text-muted font-bold ml-1">
                                                {locale === 'zh-TW' ? '配置 Jules 萬能代理果因修復能力。' : 'Configures Jules omni-agent cause-effect repair.'}
                                            </p>
                                        </div>
                                        <div className="space-y-3">
                                            <label className="text-xs font-black text-omni-primary uppercase tracking-widest flex items-center gap-2">
                                            ...
                                            <input
                                                ...
                                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => setGeminiKeyInput(e.target.value)}
