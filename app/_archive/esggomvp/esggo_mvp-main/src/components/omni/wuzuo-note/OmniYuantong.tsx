            ...
            <div className="p-5 border-b border-white/5 flex items-center justify-between bg-gradient-to-r from-cyan-500/10 to-transparent relative z-10">
                <div className="flex items-center gap-3">
                    <div className="relative group cursor-help">
                        <div className={`w-10 h-10 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 blur-[2px] ${isAnalyzing ? 'animate-spin-slow scale-110 border border-cyan-400/30' : ''}`} />
                ...
                <div className="flex bg-white/5 rounded-full p-1 border border-white/5">
                    <button
                        ...
                        className={`px-3 py-1 rounded-full text-[9px] font-bold transition-all ${activeTab === 'insight' ? 'bg-cyan-500 text-black border border-cyan-400/20' : 'text-white/40 hover:text-white'}`}
                    >
                        萬能洞悉
                    </button>
                    <button
                        onClick={() => setActiveTab('universal')}
                        className={`px-3 py-1 rounded-full text-[9px] font-bold transition-all ${activeTab === 'universal' ? 'bg-cyan-500 text-black border border-cyan-400/20' : 'text-white/40 hover:text-white'}`}
                    >
                        全域連結
                    </button>
                    <button
                        onClick={() => setActiveTab('bridge')}
                        className={`px-3 py-1 rounded-full text-[9px] font-bold transition-all ${activeTab === 'bridge' ? 'bg-cyan-500 text-black border border-cyan-400/20' : 'text-white/40 hover:text-white'}`}
                    >
                        知識橋接
                    </button>
                </div>
...
const UniversalItem = React.memo(({ icon, label, status, color, description, onClick }: UniversalItemProps) => (
    ...
    >
        ...
        <div className="mt-4 flex items-center gap-2 relative z-10">
            <div className="w-1.5 h-1.5 rounded-full bg-green-500 border border-green-400/20" />
