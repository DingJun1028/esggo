    ...
    return (
        <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <LiquidGlassContainer
                ...
                className={`transition-all duration-300 overflow-hidden rounded-md border border-[var(--theme-glass-border)] ${mastery.level === 3 ? 'border-[var(--theme-accent)]/50' : ''}`}
            >
                <div className="flex flex-col gap-3">
                    {/* Mastery Progress Bar */}
                    <div className="flex gap-1 h-1 w-full bg-[var(--theme-text-main)]/10 rounded-full overflow-hidden">
                        {[1, 2, 3].map((step) => (
                            <div
                                key={step}
                                className={`flex-1 transition-all duration-300 ${mastery.level >= step
                                    ? step === 3 ? 'bg-[var(--theme-accent)]' : step === 2 ? 'bg-purple-400' : 'bg-[var(--theme-primary)]'
                                    : 'bg-transparent'
                                    }`}
                            />
                        ))}
                    ...
                    <div onClick={() => setExpanded(!expanded)} className="cursor-pointer group">
                        <h3 className="font-black italic text-base leading-tight text-[var(--theme-text-main)] group-hover:text-cyan-500 transition-colors">
                            {knowledge.title_zh}
                        </h3>
                        <p className="text-xs text-[var(--theme-text-sub)] leading-relaxed font-['Outfit'] mt-1">
                    ...
                    <AnimatePresence>
                        {expanded && (
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="pt-3 border-t border-[var(--theme-glass-border)]/10 flex flex-col gap-3">
                                {knowledge.formula && (
                                    <div className="p-3 rounded-xl bg-[var(--theme-surface-2)] border border-[var(--theme-glass-border)]">
                                        <p className="text-[8px] font-black text-[var(--theme-text-muted)] uppercase tracking-widest mb-1">因果驗算公式</p>
                                        <code className="text-[11px] text-cyan-500 font-mono">{knowledge.formula}</code>
                                    </div>
                                )}
                                {julesResponse && (
                                ...
                                <div className="flex flex-wrap gap-1">
                                    {knowledge.tags.map(t => <span key={t} className="text-[8px] px-2 py-0.5 rounded-full bg-[var(--theme-surface-2)] border border-[var(--theme-glass-border)] text-[var(--theme-text-muted)]">#{t}</span>)}
                                </div>
                            </motion.div>
                        )}
                    </AnimatePresence>
                        ...
                        <div className="flex gap-2">
                            ...
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${mastery.level >= 1
                                    ? `bg-${color}-500/20 border border-${color}-500/30 text-${color}-500`
                                    : 'bg-[var(--theme-surface-2)] border border-[var(--theme-glass-border)] text-[var(--theme-text-muted)] hover:bg-[var(--theme-surface)]'
                                    }`}
                            >
