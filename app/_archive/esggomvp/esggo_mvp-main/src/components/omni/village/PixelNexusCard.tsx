...
export const PixelNexusCard: React.FC<PixelNexusCardProps> = ({ card, onClick, isHovered }) => {
    ...
    const getElementColor = (element: INexusCard['element']) => {
        switch (element) {
            case 'Environment': return 'text-emerald-400 border-emerald-500/50';
            case 'Social': return 'text-sky-400 border-sky-500/50';
            case 'Governance': return 'text-indigo-400 border-indigo-500/50';
            default: return 'text-cyan-400 border-cyan-500/50';
        }
    };

    const getRarityStyle = (rarity: INexusCard['rarity']) => {
        switch (rarity) {
            case 'Omni': return 'bg-gradient-to-br from-yellow-400 via-orange-500 to-red-600';
            ...
            <div className={`absolute inset-0 border-4 ${getElementColor(card.element).split(' ')[1]} bg-black/90 p-1`}>
                ...
                <div className="relative z-10 flex justify-between items-center p-2 border-b-2 border-white/5">
                    ...
                    <div className="flex items-center gap-1">
                        {isSynced && <Heart size={10} className="text-red-500 animate-pulse mr-1" fill="currentColor" />}
