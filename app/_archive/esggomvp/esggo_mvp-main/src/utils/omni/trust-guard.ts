...
export const appendLifecycleEvent = <T>(
    ...
    eventDetails: Omit<ILifecycleEvent, 'timestamp'>
): IComponentCore<T> => {
    if (coreData.isFrozen) {
    ...
    const updatedEvents = [
        ...
        { id: `hook-${uuidv4().slice(0, 8)}`, action: (eventDetails as any).event || 'UPDATED', source_module: (eventDetails as any).actor_id || 'System', timestamp: Date.now() }
    ];
...
export const createGenesisNode = <T>(
    ...
    return {
        ...
        lifecycle_events: [{
            id: `genesis-${uuidv4().slice(0, 8)}`,
            action: 'CREATED',
            source_module: 'Genesis_Inception',
            timestamp: Date.now(),
            metadata: { creatorId, originId }
        }],
        data: initialData,
        isFrozen: false,
        hash_lock: generateSignature(initialData),
        status: 'Trustworthy'
    };
};
