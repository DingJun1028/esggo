    ...
    const vitals = [
        {
            ...
            icon: <ShieldCheck size={16} />, color: 'text-emerald-600', bg: 'bg-emerald-500/10'
        },
        {
            label: locale === 'zh-TW' ? '風險韌性' : 'Risk Resilience',
            val: Math.round(resilience || dnaScores.S),
            icon: <Activity size={16} />, color: 'text-blue-600', bg: 'bg-blue-500/10'
        },
        {
            label: locale === 'zh-TW' ? '環境表現' : 'Environmental',
            val: dnaScores.E || 0,
            icon: <TrendingUp size={16} />, color: 'text-amber-600', bg: 'bg-amber-500/10'
        },
        {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 p-8 rounded-[3rem] bg-[var(--card-bg)] border border-[var(--card-border)] liquid-glass shadow-lg">
                    ...
                    <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        {diagnoses.map((d) => (
                            <div key={d.id} className="p-4 md:p-6 rounded-3xl bg-[var(--theme-surface)]/40 border border-[var(--theme-glass-border)] flex flex-col md:flex-row items-start md:items-center justify-between gap-4 group hover:bg-[var(--theme-card-bg)] hover:border-[var(--primary)]/30 transition-all cursor-pointer shadow-sm">
                                <div className="flex items-start md:items-center gap-4">
                                    <div className={`shrink-0 p-3 rounded-2xl ${d.type === 'Success' ? 'bg-emerald-500/10 text-emerald-600' :
                                        d.type === 'Warning' ? 'bg-amber-500/10 text-amber-600' :
                                    ...
                                    <div>
                                        <div className="flex items-center gap-3 mb-1">
                                            <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${d.type === 'Success' ? 'bg-emerald-500/20 text-emerald-700' :
                                                d.type === 'Warning' ? 'bg-amber-500/20 text-amber-700' :
                                                    'bg-[var(--primary)]/20 text-[var(--primary)]'
                                                }`}>
                                                {d.type}
                                            </span>
                                            <span className="text-[9px] text-[var(--theme-text-muted)] font-mono">{d.date}</span>
                                        </div>
                                        <p className="text-sm font-bold text-[var(--theme-text-main)]">{d.msg}</p>
                                        <p className="text-[10px] text-[var(--theme-text-sub)] mt-1 uppercase tracking-widest">{d.detail}</p>
                                    </div>
                                </div>
                                <div className="w-10 h-10 rounded-full bg-[var(--theme-card-bg)] border border-[var(--theme-glass-border)] flex items-center justify-center group-hover:bg-[var(--primary)] group-hover:text-black transition-colors shrink-0 hidden md:flex shadow-inner">
