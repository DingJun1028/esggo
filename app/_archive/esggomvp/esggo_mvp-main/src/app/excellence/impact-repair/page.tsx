            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6 flex flex-col">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        <button className="text-[10px] uppercase font-bold text-[var(--theme-text-muted)] hover:text-aqua transition-colors flex items-center gap-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1">
                        {projects.map((p, i) => (
                            <motion.div
                                ...
                                className="p-6 rounded-[2rem] bg-[var(--theme-card-bg)] border border-[var(--theme-glass-border)] liquid-glass group hover:border-blue-500/30 hover:bg-[var(--theme-surface)] transition-all cursor-pointer flex flex-col justify-between shadow-lg hover:shadow-xl"
                            >
                                <div className="flex justify-between items-start mb-6">
                                    <div className={`p-4 rounded-2xl ${p.status === 'active' ? 'bg-blue-500/10 text-blue-600' : 'bg-[var(--theme-surface-2)] text-[var(--theme-text-muted)]'} group-hover:scale-110 transition-transform shadow-inner`}>
                                        {p.icon}
                                    </div>
                                    <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded ${p.status === 'active' ? 'bg-blue-500/20 text-blue-700 border border-blue-500/30' : 'bg-[var(--theme-surface-2)] text-[var(--theme-text-muted)]'}`}>
                                        {p.status}
                                    </span>
                                </div>

                                <div>
                                    <h5 className="text-xs font-black text-[var(--theme-text-main)] uppercase tracking-tight group-hover:text-blue-600 transition-colors mb-1">
                                        {locale === 'zh-TW' ? p.zh : p.title}
                                    </h5>
                                    <p className="text-[10px] text-[var(--theme-text-muted)] font-mono mb-4">{p.timeline}</p>

                                    <div className="flex justify-between items-center pt-4 border-t border-[var(--theme-glass-border)]">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-[var(--theme-text-muted)]">Proj. Impact</span>
                        ...
                        <div className="p-6 rounded-[2rem] border border-dashed border-[var(--theme-glass-border)] flex flex-col items-center justify-center text-center opacity-50 hover:opacity-100 hover:border-[var(--primary)] transition-all cursor-pointer group shadow-inner">
                            <div className="size-12 rounded-full bg-[var(--theme-surface-2)] flex items-center justify-center mb-4 group-hover:text-[var(--primary)] transition-colors">
                                <Sparkles size={20} />
                            </div>
                            <span className="text-[10px] font-black uppercase tracking-widest text-[var(--theme-text-main)]">Synthesize New Paradigm</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="p-10 rounded-[3rem] bg-[var(--card-bg)] border border-[var(--card-border)] liquid-glass">
                <div className="flex flex-col lg:flex-row justify-between items-center text-center lg:text-left gap-8 lg:gap-12">
                    <div className="flex-1 space-y-4">
                        ...
                        <p className="text-xs text-[var(--theme-text-sub)] leading-relaxed max-w-2xl">
                            Beyond environmental restoration, the AI actively monitors <span className="text-[var(--theme-text-main)] font-black">Social Tissue Resilience</span>.
                    ...
                    <div className="shrink-0 flex flex-col sm:flex-row gap-4 w-full lg:w-auto mt-6 lg:mt-0">
                        <button className="px-8 py-4 bg-[var(--theme-surface)] border border-[var(--theme-glass-border)] text-[var(--theme-text-main)] rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-[var(--theme-surface-2)] transition-all w-full sm:w-auto shadow-md">
                            Access Healing Logs
                        </button>
                        <button className="px-8 py-4 bg-blue-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:scale-105 transition-all shadow-[0_0_30px_rgba(59,130,246,0.3)] w-full sm:w-auto flex items-center justify-center gap-2">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {[
                    ...
                    <motion.div
                        ...
                        key={n.l}
                        className="p-8 rounded-[2rem] bg-[var(--theme-card-bg)] border border-[var(--theme-glass-border)] group hover:border-blue-500/30 hover:shadow-[0_0_20px_rgba(59,130,246,0.1)] transition-all text-center relative overflow-hidden shadow-lg"
                    >
                        ...
                        <div className="mb-4 text-[var(--theme-text-muted)] group-hover:text-blue-600 transition-colors flex justify-center relative z-10">
                            {n.i}
                        </div>
                        <p className="text-[9px] font-bold text-[var(--theme-text-muted)] uppercase tracking-widest mb-1 relative z-10">{n.l}</p>
                        <p className="text-xl font-black text-[var(--theme-text-main)] italic uppercase relative z-10 group-hover:text-blue-800">{n.v}</p>
                    </motion.div>
                ))}
            </div>
        </div>
