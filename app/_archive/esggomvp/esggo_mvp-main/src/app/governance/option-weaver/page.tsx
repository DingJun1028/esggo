...
export default function OptionWeaverPage() {
    ...
    const [metadata5T, setMetadata5T] = useState<any>(null);
    const [atomUuid, setAtomUuid] = useState<string | null>(null);

    const handleStartWeave = async () => {
        if (!selectedPath || !userInput.trim()) return;
        ...
        try {
            const response = await fetch('/api/governance/weave', {
                method: 'POST',
            ...
            if (result.success) {
                ...
                setMetadata5T(result.data.metadata5T);
                setAtomUuid(result.data.atomUuid);
            } else {
                console.error('Weaving failed:', result.error);
                setGeneratedDraft(`Weaving encountered an error: ${result.error}`);
            }
        } catch (error) {
            console.error('API Error:', error);
            setGeneratedDraft('An unexpected error occurred during weaving.');
        } finally {
            setIsWeaving(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#0b1120] flex font-sans text-slate-300 relative overflow-hidden selection:bg-indigo-500/30">
            ...
            {/* 全局環境光 */}
            <div className={`absolute top-[-10%] left-[-10%] w-[50%] h-[50%] blur-[150px] opacity-15 rounded-full pointer-events-none transition-colors duration-1000 ${
                selectedPath === 'visionary' ? 'bg-indigo-600' : 
                selectedPath === 'progressive' ? 'bg-[#63a6b0]' : 'bg-amber-600'
            }`}></div>
            <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-[#63a6b0] blur-[200px] opacity-10 rounded-full pointer-events-none"></div>
            ...
            <aside className="w-80 border-r border-slate-800/50 flex flex-col p-6 z-10 bg-slate-900/40 backdrop-blur-xl shrink-0">
                <div className="flex items-center gap-4 mb-10">
                    <div className="p-3 bg-[#63a6b0]/10 rounded-2xl text-[#63a6b0] border border-[#63a6b0]/20 shadow-[0_0_15px_rgba(99,166,176,0.2)]">
                        <Sparkles className="w-6 h-6" />
                    </div>
                    <div>
                        <h2 className="text-xl font-bold text-white tracking-wide">Option Weaver</h2>
                        <p className="text-xs text-[#63a6b0]/80 font-mono tracking-widest uppercase mt-1">Sentient Core Active</p>
                    </div>
                </div>
                    ...
                    >
                        <div className="flex flex-col h-full bg-slate-900/40 rounded-[2.5rem]">
                            ...
                            <div className="flex-1 p-8 md:p-14 overflow-y-auto custom-scrollbar relative">
                                {isWeaving && (
                                    <div className="absolute inset-0 z-20 bg-[#0b1120]/60 backdrop-blur-sm rounded-b-[2.5rem] flex flex-col items-center justify-center">
                                        <RefreshCw className="w-10 h-10 text-aqua animate-spin mb-4" />
                                        <p className="text-aqua font-mono tracking-widest uppercase text-sm animate-pulse">Weaving Sentient Draft...</p>
                                    </div>
                                )}
                                ...
                                <div className="relative group min-h-[400px]">
                                    {!generatedDraft ? (
                                        <div className="flex flex-col gap-6">
                                            <textarea
                                                className="w-full h-40 text-xl text-slate-300 leading-loose resize-none focus:outline-none bg-white/5 border border-white/10 rounded-2xl p-6 placeholder:text-slate-600/50 font-light focus:border-aqua/50 transition-all"
                                                value={userInput}
                                                onChange={(e) => setUserInput(e.target.value)}
                                                placeholder="輸入您的 ESG 需求，靈魂精靈將為您織稿... (例如：今年減量目標與策略)"
                                            />
                                            <button 
                                                onClick={handleStartWeave}
                                                disabled={!userInput.trim()}
                                                className="w-full py-4 bg-aqua text-slate-900 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-teal-400 transition-all disabled:opacity-50 disabled:cursor-not-allowed group"
                                            >
                                                <Sparkles className="w-5 h-5 group-hover:animate-pulse" />
                                                開始靈魂織稿 (Manifest Draft)
                                            </button>
                                        </div>
                                    ) : (
                                        <textarea
                                    ...
                                    {generatedDraft && (
                                        <div className="absolute top-0 right-0 flex flex-col items-end gap-2">
                                            ...
                                            <div className="px-3 py-1 rounded-full bg-slate-950/80 border border-emerald-500/30 text-emerald-400 text-[10px] font-mono tracking-tighter uppercase">
                                                5T Trustworthy Certified
                                            </div>
                                        </div>
                                    )}
                                </div>
