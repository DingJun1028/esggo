...
export default function OmniWarRoomPage() {
    ...
    const baseCore: IComponentCore = {
        ...
        hash_lock: "INITIAL_WAR_ROOM_HASH",
        status: "Trustworthy",
        isFrozen: false
    };
    ...
    return (
        <div className="min-h-screen bg-[#070b14] flex font-sans text-slate-300 relative overflow-hidden">
            ...
            <div className="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] bg-[#63a6b0]/15 blur-[200px] rounded-full pointer-events-none mix-blend-screen mix-blend-lighten"></div>
            ...
            <main className="flex-1 w-full max-w-7xl mx-auto p-6 md:p-8 flex flex-col z-10 h-screen">
                ...
                <header className="flex justify-between items-end mb-8 shrink-0">
                    <div>
                        ...
                        {heartNetwork.connected && (
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#63a6b0]/10 border border-[#63a6b0]/20 text-[#63a6b0] text-[10px] font-black font-mono tracking-widest uppercase mb-3 ml-3">
                                <Heart size={10} fill="currentColor" className="animate-pulse" />
                                Heart Synced: {heartNetwork.count}
                            </div>
                        )}
                ...
                <div className="flex-1 min-h-0 flex gap-6">
                            ...
                            <div className="absolute inset-0 flex flex-col p-6 overflow-hidden rounded-[2.5rem]">
                                ...
                                <div className="flex justify-between items-center border-b border-white/10 pb-4 mb-6 shrink-0">
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 bg-[#63a6b0]/10 rounded-lg border border-[#63a6b0]/30 shadow-[0_0_15px_rgba(99,166,176,0.2)]">
                                            <ShieldCheck className="w-5 h-5 text-[#63a6b0]" />
                                        </div>
                                        <div>
                                            <h2 className="text-xl font-bold text-white tracking-wide">CH 3.2 溫室氣體盤查與減量</h2>
                                            <p className="text-xs text-slate-400 font-mono mt-1">Row-Level Security (RLS): <span className="text-[#63a6b0]">Environment Dept. Only</span></p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="w-80 flex flex-col gap-6 shrink-0">
                        {/* Omni Insight Box */}
                        <LiquidGlassContainer glowColor="aqua" intensity="high" className="shrink-0" coreContext={{ ...baseCore, uuid: "omni-insight" }}>
                            <div className="p-6">
                                <h3 className="text-sm font-bold text-[#63a6b0] uppercase tracking-widest mb-4 flex items-center gap-2">
                                    <Zap className="w-4 h-4" /> Omni Insight
                                </h3>
                                <div className="space-y-3">
                                    <div className="bg-[#63a6b0]/10 border border-[#63a6b0]/20 p-3 rounded-xl">
                                        <p className="text-xs text-[#b0d8dd] leading-relaxed">
                        ...
                        {/* Live Log / Chat */}
                        <LiquidGlassContainer glowColor="aqua" intensity="low" className="flex-1 min-h-0 flex flex-col" coreContext={{ ...baseCore, uuid: "live-ops" }}>
                                    ...
                                    <div key={log.id} className="flex gap-3">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${log.user === 'Alina' ? 'bg-emerald-500 text-white' :
                                            log.user === 'Marcus' ? 'bg-blue-500 text-white' :
                                                'bg-[#63a6b0] text-slate-950 font-black'
                                            }`}>
                                        ...
                                        <div>
                                            ...
                                            <div className={`mt-1 text-sm ${log.type === 'chat' ? 'bg-slate-800 text-slate-200 p-2.5 rounded-r-xl rounded-bl-xl border border-slate-700/50' : 'text-[#63a6b0]/70'}`}>
                                                {log.text}
                                            </div>
                                        </div>
                                    </div>
