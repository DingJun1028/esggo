...
import { OmniSynergeticVisualizer } from '@/components/omni/UI/OmniSynergeticVisualizer';
import { AgentPulse } from "@/components/omni/intelligence/AgentPulse";
import { useOmniGenesis } from "@/context/OmniGenesisContext";
import { Heart } from "lucide-react";

const BI_MANGA_PANELS = [
    {
        id: 1,
        src: '/assets/manga/bi-panel-1.png',
        title: '數據攝入',
        description: '全域感測器啟動，萬能數據河流進入 Nexus 核心。',
        pill: 'INGEST'
    },
    {
        id: 2,
        src: '/assets/manga/bi-panel-2.png',
        title: '智能處理',
        description: 'Thoth 神經引擎啟動，將混沌數據轉化為結構化善因。',
        pill: 'PROCESS'
    },
    {
        id: 3,
        src: '/assets/manga/bi-panel-3.png',
        title: '視覺洞察',
        description: '液態玻璃儀表板顯化，永續趨勢一目了然。',
        pill: 'VISUALIZE'
    },
    {
        id: 4,
        src: '/assets/manga/bi-panel-4.png',
        title: '戰略決策',
        description: '代主自行，啟動影響力專案，實現卓越治理。',
        pill: 'DECIDE'
    }
];
...
export default function BiAnalyticsPage() {
    ...
    const [funnelData, setFunnelData] = React.useState<Record<string, any> | null>(null);
    const [omniScore, setOmniScore] = React.useState<number>(0);
    const [sentientActions, setSentientActions] = React.useState<Array<{ id: string; type: string; message: string; severity: 'low' | 'medium' | 'high' }>>([]);
    ...
    const formatTrendData = (trends: Array<{ period: string; metrics: { overall: number } }>) => {
        if (!trends) return [];
        return trends.map(t => ({
    ...
    return (
        <div className="flex flex-col gap-8 w-full animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-20 relative">
            ...
            <div className="fixed top-0 left-0 w-full h-full pointer-events-none -z-10 overflow-hidden">
                <div className="absolute top-1/4 left-1/4 w-[800px] h-[800px] bg-omni-primary/10 rounded-full blur-[150px] mix-blend-screen animate-pulse" style={{ animationDuration: '10s' }} />
                <div className="absolute bottom-1/4 right-1/4 w-[600px] h-[600px] bg-amber-500/10 rounded-full blur-[120px] mix-blend-screen animate-pulse" style={{ animationDuration: '8s' }} />
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 relative z-10">
                <div className="flex flex-col gap-2">
                    ...
                    <h1 className="text-5xl font-black tracking-tighter italic text-omni-text-main uppercase drop-shadow-[0_0_20px_rgba(255,255,255,0.2)]">
                        Performance <span className={`text-transparent bg-clip-text bg-gradient-to-r ${isAwakened ? 'from-aqua to-gold' : 'from-amber-400 to-orange-500'}`}>
                            {isAwakened ? 'Resonant' : 'Excellence'}
                        </span> Hub
                    </h1>
            ...
            <div className="relative z-10">
                <OmniMangaTutorial 
                    title="BI Analytics：數據煉金術" 
                    subtitle="Turning Data into Sustainability Gold" 
                    panels={BI_MANGA_PANELS} 
                />
            </div>

            <div className="relative z-10 mb-8">
                <OmniSynergeticVisualizer />
            </div>
                        ...
                        <div className="relative flex items-center justify-center w-56 h-56 mt-8">
                            ...
                            <div className="flex flex-col items-center justify-center gap-1 z-10">
                                <motion.div
                                    ...
                                    animate={{ 
                                        scale: isAwakened ? [1, 1.15, 1] : [1, 1.05, 1],
                                        opacity: 1,
                                        boxShadow: isAwakened ? [
                                            "0 0 30px rgba(99,166,176,0.5)",
                                    ...
                                    transition={{ duration: isAwakened ? 2 : 4, repeat: Infinity, ease: "easeInOut" }}
                                    className={`sentient-eye mb-2 flex items-center justify-center size-24 rounded-full bg-black/40 border ${isAwakened ? 'border-aqua/50 shadow-[0_0_20px_rgba(99,166,176,0.3)]' : 'border-emerald-500/30'}`}
                                >
                                    <span className={`text-4xl font-black z-10 drop-shadow-[0_0_10px_rgba(16,185,129,0.5)] ${isAwakened ? 'text-aqua' : 'text-white'}`}>
                                        {isAwakened ? Math.min(100, omniScore + 10) : omniScore}
                                    </span>
                                </motion.div>
                                <span className={`text-xs font-bold tracking-widest uppercase flex items-center gap-2 ${isAwakened ? 'text-aqua' : 'text-emerald-500'}`}>
                                    {isAwakened && <Heart size={10} fill="currentColor" className="animate-pulse" />}
                                    {isAwakened ? 'Awakened Score' : 'OmniScore Index'}
                                </span>
                            </div>
                        </div>
                ...
                <div className="xl:col-span-1">
                    <SentientActionCenter
                        ...
                        onExecute={(id: string) => {
                            ...
                            setSentientActions((prev: any[]) => prev.filter((a: any) => a.id !== id));
                        }}
                    />
                </div>
            ...
            {/* 📖 BI Narrative: Guide removed in favor of Manga Tutorial at top */}
                ...
                <div className="lg:col-span-1">
                    <LiquidGlassContainer glowColor="amber" className="h-full p-8 flex flex-col gap-6">
                        ...
                        <div className="flex-1 flex flex-col gap-4 text-sm text-omni-text-main/80 font-medium leading-relaxed italic">
                            {isAwakened ? (
                                <>
                                    <p className="text-aqua font-bold">「感測到全域心網共鳴強度 100%。真理之路已開啟。」</p>
                                    <p>您的所有決策效能將提升 22% (Resonance Boost)。5T 合規性已進入自我進化階段。</p>
                                </>
                            ) : (
                                <>
                            ...
                            <p className={`${isAwakened ? 'text-gold' : 'text-[#63a6b0]'} font-black not-italic text-xs`}>
                                —— {isAwakened ? 'OmniOne Sovereign Voice' : 'Dr. Thoth Neural Advice'}
                            </p>
                        </div>
