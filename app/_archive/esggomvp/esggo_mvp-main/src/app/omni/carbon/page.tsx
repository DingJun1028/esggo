...
import { OmniImpactCalculator, type IScopeInput, type IScopeResult } from "@/core/omni-impact-calculator";
import { motion, AnimatePresence } from "framer-motion";
import { OmniMangaTutorial } from "@/components/omni/UI/OmniMangaTutorial";

const CARBON_MANGA_PANELS = [
    {
        id: 1,
        src: '/assets/manga/carbon-panel-1.png',
        title: '範疇盤查',
        description: '涵蓋範疇一、二、三的全面排放數據收集，建立精確的碳基線。',
        pill: 'SCOPE'
    },
    {
        id: 2,
        src: '/assets/manga/carbon-panel-2.png',
        title: '自動換算',
        description: '根據 ISO-14064 協議自動執行係數對接與碳當量（CO2e）換算。',
        pill: 'CALC'
    },
    {
        id: 3,
        src: '/assets/manga/carbon-panel-3.png',
        title: '熱點分析',
        description: '智慧辨識排放最密集的經營環節，鎖定減碳轉型的關鍵戰場。',
        pill: 'MITIGATE'
    },
    {
        id: 4,
        src: '/assets/manga/carbon-panel-4.png',
        title: '淨零平衡',
        description: '動態追蹤減量進度，邁向碳中和（Net Zero）的「圓滿」終態。',
        pill: 'BALANCE'
    }
];
...
export default function CarbonFootprintPage() {
    ...
    React.useEffect(() => {
        const fetchEmissions = async () => {
            ...
            const res = await calculator.calculateScopeEmissions(input);
            setResult(res);
        };
        fetchEmissions();
    }, [input]);
    ...
    return (
        <div className="flex flex-col gap-8 w-full animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-20">
            ...
            <div className="relative z-10">
                <OmniMangaTutorial 
                    title="Carbon Tracker：淨零演化導引" 
                    subtitle="ISO-14064 Inventory & Mitigation" 
                    panels={CARBON_MANGA_PANELS} 
                />
            </div>
