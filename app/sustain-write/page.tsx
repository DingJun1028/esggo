...
import { DocumentStagingModal } from '@/components/vault/DocumentStagingModal';
...
export default function SustainWritePage() {
  ...
  const [docSources, setDocSources] = useState<Record<string, string>>({}); // 紀錄來源
  ...
  const handleStagingLock = async (docId: string, source: string) => {
    setUploadedDocs((prev) => {
      const next = new Set(prev);
      next.add(docId);
      return next;
    });
    setDocSources((prev) => ({ ...prev, [docId]: source }));
    setIsStagingOpen(false);

    // [5T Trackable] Log to Audit DB
    try {
      await fetch('/api/audit', {
        ...
        body: JSON.stringify({
          ...
          document_id: docId,
          metric_name: 'Document ZKP Lock',
          metric_value: docId,
          unit: 'File',
          source_origin: source,
          hash_lock: `ZKP-SHA256-${Math.random().toString(36).substring(2, 10).toUpperCase()}`,
          status: 'Trustworthy',
          details: { message: `Document locked via ${source}` },
        }),
      });
    } catch (err) {
  ...
  const completionStats = useMemo(() => {
    ...
    const stats = cats.map((cat) => {
      ...
      return {
        ...
        total: docs.length,
        completed: Math.min(docs.length, baseCompleted + userUploadedCat),
        required: required.length,
        requiredCompleted: Math.min(required.length, baseRequiredCompleted + userUploadedReqCat),
      };
    });
    ...
    <div className="min-h-screen bg-neutral-50">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-8 space-y-6">
                          ...
                          <table className="w-full text-xs">
                            <thead>
                              <tr className="border-b border-neutral-100">
                                ...
                                <th className="text-right py-2 px-2 font-medium text-neutral-500">
                                  操作
                                </th>
                              </tr>
                            </thead>
                                ...
                                >
                                  ...
                                  <td className="py-2 px-2 text-center">
                                    {uploadedDocs.has(doc.id) ? (
                                      <Badge variant="success" size="sm">
                                        已入庫 ({docSources[doc.id] || '上傳單據'})
                                      </Badge>
                                    ) : (
                                      <Badge variant={doc.required ? 'error' : 'neutral'} size="sm">
                                        {doc.required ? '待填報' : '選填'}
                                      </Badge>
                                    )}
                                  </td>
                                  <td className="py-2 px-2 text-right">
                                    ...
                                    >
                                      {uploadedDocs.has(doc.id) ? '檢視' : '上傳單據'}
                                    </Button>
                                  </td>
                                </tr>
                              ))}
        ...
        <button
          ...
          className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-cyan-600 text-white shadow-sm flex items-center justify-center hover:bg-cyan-700 transition-all hover:scale-105"
        >
