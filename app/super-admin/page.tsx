...
export default function SuperAdminPanel() {
  ...
  const isAuthorized =
    user &&
    (user.role === 'superadmin' || user.email === 'dev@esggo.com' || user.id === 'dev_user');
          ...
          <CardContent className="pt-6 space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-400">
                監控協同代理群組（Antigravity, Jules, OmniNexus）的狀態
              </p>
            </div>
              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-slate-100 text-slate-700">
                  <tr>
                    ...
                    <td className="p-3">
                      <span className="px-2 py-0.5 rounded-full bg-green-100 text-green-800 font-semibold text-[10px]">
                        ACTIVE
                      </span>
                    </td>
                    <td className="p-3">監控系統平台導覽全景</td>
                    <td className="p-3">
                      <Button size="sm" variant="ghost" className="text-xs text-[#63a6b0]">
                        診斷
                      </Button>
                    </td>
                  </tr>
            ...
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-2">
              <h4 className="text-xs font-bold text-slate-800 flex items-center gap-1">
                <Terminal size={14} /> Swarm 集中意圖宣告門檻
              </h4>
              <p className="text-[10px] text-slate-500">
                目前配置：單一意圖變更需經過 3 個節點共識驗算，在 API Setup 發送信號前，將鎖定 5T
                雜湊鏈以防止篡改。
              </p>
            </div>
          </CardContent>
          ...
          <CardContent className="pt-6 space-y-4">
            <div>
              <h3 className="text-base font-black text-slate-800">
                PostgreSQL & Supabase RLS 管理
              </h3>
              <p className="text-xs text-slate-400">
                審查資料表安全性與行級安全政策 (Row Level Security)
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-[#63a6b0]/5 rounded-xl border border-[#63a6b0]/20 space-y-2">
                <h4 className="text-xs font-bold text-[#63a6b0] flex items-center gap-2">
                  <Key size={14} /> API 用戶驗證憑證
                </h4>
                <p className="text-[10px] text-slate-500">
                  目前平台後端不使用 Prisma 用戶端，改以 `supabaseAdmin` 直接對接資料庫。強制檢查
                  RLS 政策。
                </p>
      ...
      {activeTab === 'audit' && (
        <Card className={CARD_CLASS}>
          <CardContent className="pt-6 space-y-4">
            <div>
              <h3 className="text-base font-black text-slate-800">
                5T 協議合規性稽核帳本 (5T Audit Ledger)
              </h3>
              <p className="text-xs text-slate-400">
                監控 Tangible, Traceable, Trackable, Transparent, Trustworthy 核心落實度
              </p>
            </div>
            ...
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100 space-y-2">
              ...
              <p className="text-[10px] text-slate-500">
                所有 ESG 資料節點均套用 `SHA-256` 簽章防護鎖。任何未經授權的欄位更新將自動被 RLS
                觸發器拒絕並發布警告至安全主控台。
              </p>
            </div>
          </CardContent>
