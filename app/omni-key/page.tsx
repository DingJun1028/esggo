...
import {
  Key, Shield, Zap, Brain, Globe, Lock, CheckCircle2,
  AlertCircle, RefreshCw, Plus, Trash2, Eye, EyeOff,
  Sparkles, Crown, Search, Filter
} from 'lucide-react';
...
const LEVEL_CONFIG = {
  basic: { name: '探索者', icon: '🔍', color: 'neutral', gradient: 'from-neutral-500 to-neutral-600' },
  pro: { name: '創造者', icon: '⚡', color: 'info', gradient: 'from-blue-500 to-cyan-500' },
  enterprise: { name: '領導者', icon: '👑', color: 'warning', gradient: 'from-amber-500 to-orange-500' },
  omniverse: { name: '無限', icon: '🌌', color: 'success', gradient: 'from-violet-500 to-purple-500' },
};
...
export default function OmniKeyPage() {
  ...
  const [selectedLevel, setSelectedLevel] = useState<'basic' | 'pro' | 'enterprise' | 'omniverse'>('basic');
                ...
                <Card key={key.id} variant="default" padding="md" hover>
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className={cn('w-10 h-10 rounded-lg flex items-center justify-center text-lg bg-gradient-to-br', levelConfig.gradient)}>
                        {levelConfig.icon}
                      </div>
                      <div>
                        <h3 className="text-sm font-bold text-neutral-900">{levelConfig.name}</h3>
                        <p className="text-[10px] text-neutral-400 font-mono">{key.id.substring(0, 16)}...</p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    {['真', '善', '美', '信', '通'].map((t, i) => (
                      <div key={t} className={cn(
                        ...
                        key.fiveTStatus[i] ? 'bg-emerald-100 text-emerald-700' : 'bg-neutral-100 text-neutral-400'
                      )}>
                        {t}
                      </div>
                    ))}
                    ...
                    {key.capabilities?.slice(0, 4).map(cap => {
                      ...
                      return (
                        <div key={cap.id} className={cn(
                          ...
                          cap.enabled ? 'bg-neutral-100 text-neutral-600' : 'bg-neutral-50 text-neutral-400'
                        )}>
                          <Icon size={8} />
                          {cap.name}
                        </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      className="flex-1"
                    >
                      {isExpanded ? <><EyeOff size={12} />收起</> : <><Eye size={12} />詳情</>}
                    </Button>
                    <Button
                      variant="ghost"
                      size="sm"
                    ...
                    <div className="mt-3 pt-3 border-t border-neutral-100 space-y-2">
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div>
                          ...
                          <p className="text-neutral-700 font-mono">{key.metadata?.issuer || '—'}</p>
                        </div>
                        <div>
        ...
        <Card variant="outlined" padding="md">
          <div className="text-center space-y-3 py-4">
            ...
            <p className="text-sm text-neutral-500 max-w-lg mx-auto">
              萬能元鑰不僅是身份驗證機制，更是通往所有知識、所有功能、所有可能性的鑰匙。
              遵循 5T 協議（真、善、美、信、通），確保每一次開啟都是正向的。
            </p>
            <div className="flex items-center justify-center gap-4 pt-2">
              {['🔑 身份', '🌐 存取', '🧠 知識', '⚡ 執行', '🔮 預測', '🌍 互操作'].map(item => (
                <span key={item} className="text-xs text-neutral-400">{item}</span>
              ))}
            </div>
          </div>
        </Card>
