...
export function OmniNoteCRUD({ notes, onChange }: Props) {
  ...
  const save = useCallback(() => {
    ...
    if (editing) {
      ...
      onChange([...notes, {id:`ON-${rnd()}`,title:draft.title,content:draft.content,tags,fiveTGate:draft.fiveTGate||undefined,createdAt:Date.now()}]);
    }
    cancel();
  }, [draft, editing, notes, onChange]);

  const remove = useCallback((id:string) => onChange(notes.filter(n=>n.id!==id)), [notes, onChange]);
  ...
  const renderMd = (s:string) => xss(s)
    ...
    .replace(/`(.+?)`/g,'<code style="background:#252530;padding:1px 5px;border-radius:3px;font-family:monospace;font-size:12px;color:#22D3EE">$1</code>')
    .replace(/\n/g,'<br/>');

  const inputStyle = { width:'100%', background:'#252530', border:`1px solid ${C.border}`, borderRadius:8, padding:'8px 10px', color:C.text, fontSize:13, outline:'none', fontFamily:"'Noto Sans TC',sans-serif" };
  const btnStyle   = (c:string) => ({ border:'none', borderRadius:8, padding:'6px 14px', fontSize:12, cursor:'pointer', fontWeight:600, background:c, color:'#000' });
  ...
  return (
    <div>
      ...
      {/* Editor */}
      {(creating||editing) && (
        <div style={{background:'rgba(20,20,24,0.95)',border:`1px solid ${C.teal}`,borderRadius:12,padding:16,marginBottom:14}}>
          ...
          <div style={{display:'flex',gap:6,marginBottom:8}}>
            <button onClick={()=>setPreview(null)} style={{...btnStyle(preview===null?C.teal:'#252530'),color:preview===null?'#000':C.muted,flex:1}}>✏️ 編輯</button>
            <button onClick={()=>setPreview(draft.content)} style={{...btnStyle(preview!==null?C.gold:'#252530'),color:preview!==null?'#000':C.muted,flex:1}}>👁 預覽</button>
          </div>
          {preview!==null
            ? <div style={{minHeight:100,background:'#252530',borderRadius:8,padding:'10px 12px',fontSize:13,lineHeight:1.8,color:C.text}} dangerouslySetInnerHTML={{__html:renderMd(preview)}} />
          ...
          <div style={{display:'flex',gap:8,marginTop:12}}>
            ...
            <button onClick={cancel} style={{...btnStyle('#252530'),color:C.muted}}>取消</button>
          </div>
        </div>
      )}
      ...
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        ...
        {notes.map(n=>(
          <div key={n.id} style={{background:'#1A1A1F',borderRadius:10,padding:'12px 14px',border:`1px solid ${gateColor(n.fiveTGate)}30`}}>
