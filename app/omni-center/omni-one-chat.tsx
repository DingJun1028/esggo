...
const RESPONSES: Record<CaseType, string[]> = {
  ...
  ui_design: ['Liquid Glass UI 方案完成。採用 `backdrop-filter: blur(12px)`，符合 WCAG 2.1 AA 對比標準。','設計令牌已更新：主色 var(--accent-teal)，輔色 var(--accent-gold)，字體 Noto Sans TC + Fira Code。'],
        ...
        {busy && (
          <div className="flex items-start">
            <div className="bg-primary border border-borderColor rounded-xl rounded-tl-sm px-3.5 py-2.5 flex gap-1">
              {[0,1,2].map(i=><div key={i} className="w-1.5 h-1.5 rounded-full bg-accentTeal" style={{animation:`bounce .8s ${i*0.15}s infinite`}}/>)}
            </div>
          </div>
        )}
        <div ref={bottomRef}/>
