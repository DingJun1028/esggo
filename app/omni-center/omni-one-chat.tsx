...
const RESPONSES: Record<CaseType, string[]> = {
  ...
  documentation: ['已生成結構化文檔：**概覽** → **API 參考** → **使用範例**。請確認後發布。','文檔草稿完成，包含 TypeScript 類型聲明和 JSDoc 注釋。'],
...
export function OmniOneChat() {
  ...
  const send = async () => {
    ...
    const userMsg: Message = {id:Date.now()+'u', role:'user', text:input, time:now()};
    setMsgs(m=>[...m,userMsg]);
    setInput('');
    setBusy(true);
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      ...
      <div style={{flex:1,overflowY:'auto',display:'flex',flexDirection:'column',gap:8,paddingRight:4,minHeight:200,maxHeight:320}}>
        {msgs.map(m=>(
          <div key={m.id} style={{display:'flex',flexDirection:'column',alignItems:m.role==='user'?'flex-end':'flex-start'}}>
            <div style={{maxWidth:'85%',background:m.role==='user'?`${C.teal}25`:'#252530',border:`1px solid ${m.role==='user'?C.teal:C.border}`,
              borderRadius:m.role==='user'?'12px 12px 2px 12px':'12px 12px 12px 2px',padding:'8px 12px'}}>
              {m.caseType && <div style={{marginBottom:4}}>
                <span style={{fontSize:10,background:ctColor(m.caseType),color:'#000',borderRadius:4,padding:'1px 6px',fontFamily:'monospace',fontWeight:700}}>[{ctLabel(m.caseType)}]</span>
        ...
        {busy && (
          <div style={{display:'flex',alignItems:'flex-start'}}>
            <div style={{background:'#252530',border:`1px solid ${C.border}`,borderRadius:'12px 12px 12px 2px',padding:'10px 14px',display:'flex',gap:4}}>
      ...
      <div style={{display:'flex',gap:8}}>
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==='Enter'&&!e.shiftKey&&send()}
          ...
          style={{flex:1,background:'#252530',border:`1px solid ${C.border}`,borderRadius:8,padding:'8px 12px',color:C.text,fontSize:13,outline:'none',fontFamily:"'Noto Sans TC',sans-serif"}}/>
        <button onClick={send} disabled={busy||!input.trim()} style={{background:busy||!input.trim()?C.muted:C.teal,border:'none',borderRadius:8,padding:'8px 14px',color:'#000',fontWeight:700,fontSize:13,cursor:busy?'wait':'pointer'}}>
          {busy?'…':'發送'}
        </button>
      </div>
