...
const INITIAL_TASKS: OmniTask[] = [
  { id: 'T1', title: '完成 2025 年 ESG 碳排盤查報告', priority: 'High', dueDate: Date.now() + 86400000 * 2, status: 'Pending', tags: ['環境面'] },
  { id: 'T2', title: '更新 OmniOne 覺醒核心引擎 MemorySystem', priority: 'High', dueDate: Date.now() - 86400000 * 1, status: 'Pending', tags: ['AI', '架構'] },
  { id: 'T3', title: '審核第四季員工福利政策修改', priority: 'Medium', dueDate: Date.now() + 86400000 * 5, status: 'Pending', tags: ['社會面'] },
  { id: 'T4', title: '例行性 ZKP 零知識證明金鑰輪替', priority: 'Medium', dueDate: Date.now() + 86400000 * 10, status: 'Pending', tags: ['安全'] },
  { id: 'T5', title: '發佈萬能筆記 (OmniNote) 的前端 UI 更新', priority: 'Low', dueDate: Date.now() + 86400000 * 1, status: 'Pending', tags: ['UI/UX'] },
];
...
export function WuzuoNoteView() {
  ...
  const [filterDueDate, setFilterDueDate] = useState<'All' | 'Upcoming' | 'Overdue'>('All');
  ...
  const toggleStatus = async (task: OmniTask) => {
    ...
    try {
      ...
      if (newStatus === 'Completed') {
        const res = await omniOne.process(`任務已完成: ${task.title}`, { language: 'zh-TW', autonomous: false });
        setLearningFeedback(`🧠 OmniOne 學習回饋: 信心度提昇至 ${(res.confidence * 100).toFixed(0)}%`);
  ...
  const getPriorityColor = (p: Priority) => {
    switch(p) {
      case 'High': return 'text-[#F5222D] bg-[#F5222D]/10 border-[#F5222D]/20';
      case 'Medium': return 'text-[#FA8C16] bg-[#FA8C16]/10 border-[#FA8C16]/20';
      case 'Low': return 'text-[#52C41A] bg-[#52C41A]/10 border-[#52C41A]/20';
    }
  };
    ...
    <div className="flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {learningFeedback && (
        <div className="fixed top-20 right-8 z-50 animate-in fade-in slide-in-from-right-8 duration-300">
          <div style={liquidGlassCard} className="px-5 py-3 border border-accentPurple/30 flex items-center gap-3">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-2">
        <div>
          ...
          <p className="text-[13px] text-textSecondary mt-1">無作妙德版：已連線至 Firebase NCBDB 且具備 OmniOne 自主學習</p>
        </div>
      ...
      <div style={liquidGlassCard} className="p-3.5 border border-borderColor/50 dark:border-white/10 dark:bg-slate-800/40">
      ...
      <div className="flex flex-col gap-3">
        {processedTasks.length === 0 ? (
          ...
          processedTasks.map(task => {
            ...
            const dateStr = new Date(task.dueDate).toLocaleDateString('zh-TW', { month:'short', day:'numeric' });
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  ...
                  <div className="flex-1">
                    <div className={`font-semibold text-[14px] transition-all duration-300 
                      ${isCompleted ? 'text-textSecondary line-through' : 'text-textPrimary'}`}
                    >
                      {task.title}
                    </div>
                    <div className="flex gap-1.5 mt-1.5 flex-wrap">
                      <span className={`text-[10px] px-2 py-0.5 rounded-full border ${getPriorityColor(task.priority)} font-bold tracking-wide`}>
                        {task.priority}
                      </span>
                      {task.tags.map(t => (
                        <span key={t} className="text-[10px] px-2 py-0.5 rounded-full bg-black/5 dark:bg-white/10 text-textSecondary">
                          {t}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="shrink-0 flex items-center gap-1.5 text-[12px] font-medium">
                    {isCompleted ? (
                      ...
                      <>
                        <span className={isOverdue ? 'text-[#F5222D]' : 'text-textSecondary'}>
