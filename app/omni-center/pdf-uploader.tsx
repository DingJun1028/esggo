...
export function PdfUploader() {
  ...
  const [result, setResult] = useState<{ success: boolean; message: string; totalChunks?: number; pageCount?: number } | null>(null);
  ...
  const handleUpload = async () => {
    ...
    formData.append('userId', 'user_' + Math.random().toString(36).substr(2, 9)); // Simulate logged-in user
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.6 }}>
        上傳永續報告書 (PDF)，Omni-Core 將自動解析並進行切片 (Chunking)，寫入 <code style={{color: C.gold}}>54686_esg_go_userdb</code> 的 knowledge_chunks 中。
      </div>
      ...
      <div 
        ...
        onMouseOver={e => e.currentTarget.style.background = 'rgba(0,158,176,0.1)'}
        onMouseOut={e => e.currentTarget.style.background = 'rgba(0,158,176,0.05)'}
      >
        <input 
          type="file" 
      ...
      {result && (
        <div style={{
          padding: 16,
          borderRadius: 8,
          background: result.success ? 'rgba(34,197,94,0.1)' : 'rgba(255,77,109,0.1)',
