...
export default function SwarmPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const agents: ('Antigravity' | 'OmniJules' | 'OmniNexus')[] = [
        'Antigravity',
        'OmniJules',
        'OmniNexus',
      ];
  ...
  const triggerStressTest = () => {
    // 模擬 16 法則壓力測試
    const events: {
      type: 'HEAL' | 'STRATEGIZE' | 'EXECUTE' | 'OBSERVE';
      source: string;
      msg: string;
    }[] = [
      ...
      { type: 'STRATEGIZE', source: 'Antigravity', msg: '動態重構路由路徑 (Dynamic Routing)' },
      {
        type: 'HEAL',
        source: 'OmniJules',
        msg: 'Karma Protocol 啟動 - 記憶體修復 (Memory Healed)',
      },
    ...
    <div className="p-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="mb-10 flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            即時監控 Antigravity、OmniJules 與 OmniNexus 的協作狀態與 5T 驗證流。
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
          <div className="relative bg-[#020617]/60 border border-white/10 p-6 rounded-2xl overflow-hidden group">
        ...
        <div className="lg:col-span-2 relative bg-[#020617]/40 border border-white/10 rounded-2xl p-6 flex flex-col h-full min-h-[600px]">
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      <span
                        className={`text-sm font-bold ${getAgentColor(log.agent).split(' ')[0]}`}
                      >
                        {log.agent}
                      </span>
                    ...
                    <div className="text-slate-300 font-medium text-base mb-1">{log.action}</div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs px-2 py-0.5 rounded text-slate-400 border border-white/10 font-mono">
                        Target: {log.target}
                      </span>
                      {log.status === 'healing' && (
