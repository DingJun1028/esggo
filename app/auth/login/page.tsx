...
export default function LoginPage() {
  ...
  async function handleLogin(e?: React.FormEvent) {
    ...
    setError(null);
    try {
      if (!email || !password) throw new Error('Ë´ãËº∏?•ÈõªÂ≠êÈÉµ‰ª∂Ë?ÂØÜÁ¢º (Email & Password required)');
      ...
      if (!res.ok) {
        throw new Error(data.message || data.error || '?ªÂÖ•Â§±Ê? (Login Failed)');
      }
      ...
      const message = err instanceof Error ? err.message : '????ØË™§ (Connection Error)';
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      setError('Google ?ªÂÖ•Â∞öÊú™ÂÆåÂÖ®?çÁΩÆÔºåË??ØÁµ°ÁÆ°Á???(Google Auth not fully configured)');
    } catch (err: unknown) {
      setError('Google ?ªÂÖ•Â§±Ê?');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="relative z-10 w-full max-w-[440px] px-6 py-12 fade-in">
        ...
        <BrandCard padding="lg" className="bg-white/90 backdrop-blur-[40px] border-white shadow-extreme rounded-[48px] p-10">
           <div className="mb-10 text-center">
              <h2 className="text-2xl font-black text-[#003262] tracking-tight">Ë∫´Â?È©óË?‰∏≠Â?</h2>
             ...
             <div className="mb-8 p-5 bg-blue-50 border border-blue-200 rounded-[32px] flex flex-col gap-4">
                <div className="flex items-start gap-3">
                   ...
                   <div>
                      ...
                      <p className="text-blue-700/70 text-[10px] font-bold leading-relaxed mt-1">?ãÁôº?ÖÊ∏¨Ë©¶Ê®°ÂºèÂ∑≤?üÂ??ÇÊÇ®?Ø‰ª•?¥Êé•‰ΩøÁî®Âø´ÈÄüÂ??ñÈÄ≤ÂÖ•Âπ≥Âè∞ÁÆ°Á?‰ªãÈù¢??/p>
                   </div>
                </div>
                <BrandButton 
                  variant="primary" 
                  ...
                  loading={loading}
                >
                   <Zap size={14} className="mr-2" /> Âø´ÈÄüÈÄ≤ÂÖ•?ãÁôº?ÖÊéß?∂Âè∞
                </BrandButton>
             </div>
           )}
                 ...
                 <form onSubmit={handleLogin} className="space-y-6">
                    ...
                    <div className="pt-4 space-y-4">
                       <BrandButton 
                         ...
                         loading={loading}
                       >
                          ?üÂ?‰∏ªÊ???? <ArrowUpRight size={18} className="ml-2 group-hover:translate-x-1 transition-transform" />
                       </BrandButton>
                       
                       <div className="text-center">
                          <p className="text-[11px] font-bold text-slate-400">
                             Â∞öÊú™?ÅÊ?Â∏≥Ë?Ôº?<Link href="#" className="text-[#3B7EA1] font-black hover:text-[#003262] transition-colors underline underline-offset-4">Á´ãÂç≥Ë®ªÂ??êÁÇ∫?êÂì°</Link>
                          </p>
                       </div>
                    </div>
                 </form>
