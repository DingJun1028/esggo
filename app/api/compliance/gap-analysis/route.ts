...
export async function POST(req: NextRequest) {
  try {
    ...
    const lowCompleteness = matrix.filter((n: unknown) => n.completeness < 70);
    const sealedCount = matrix.filter((n: unknown) => n.isSealed).length;
...
2. 低完成度指標：${lowCompleteness.map((n: unknown) => `${n.code} (${n.completeness}%)`).join(', ')}
