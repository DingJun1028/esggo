...
const STEP_ICONS = { analyze: Target, plan: Layers, execute: Zap, verify: CheckCircle2, synthesize: Brain };
const STEP_LABELS = { analyze: '分析問題', plan: '制定計劃', execute: '執行任務', verify: '驗證結果', synthesize: '綜合回答' };
...
export default function OmniAgentPage() {
  ...
  useEffect(() => { fetchShards(); fetchNotes(); }, [fetchShards]);

  const fetchHealth = async () => {
    try { const r = await fetch('/api/system/health'); if (r.ok) setHealth(await r.json()); } catch { /* ignore */ }
  };

  const fetchNotes = async () => {
    try { const r = await fetch('/api/omni-agent/notes'); if (r.ok) setNotes(await r.json()); } catch { /* ignore */ }
  };
  ...
  const handleSend = async (text?: string, isVoice = false) => {
    ...
    const thinkingSteps: ThinkingStep[] = [
      { id: 's1', type: 'analyze', title: '分析問題', content: '', status: 'active', timestamp: Date.now() },
      { id: 's2', type: 'plan', title: '制定計劃', content: '', status: 'pending', timestamp: Date.now() },
      { id: 's3', type: 'execute', title: '執行任務', content: '', status: 'pending', timestamp: Date.now() },
      { id: 's4', type: 'verify', title: '驗證結果', content: '', status: 'pending', timestamp: Date.now() },
      { id: 's5', type: 'synthesize', title: '綜合回答', content: '', status: 'pending', timestamp: Date.now() },
    ];
    ...
    try {
      ...
      let analysis = '';
      if (ar.ok) { const d = await ar.json(); analysis = d.result || ''; updateStep(agentMsg.id, 0, 'done', analysis || '分析完成'); }
      ...
      let plan = '';
      if (pr.ok) { const d = await pr.json(); plan = d.result || ''; updateStep(agentMsg.id, 1, 'done', plan || '計劃完成'); }
      ...
      const res = await fetch('/api/omni-agent/chat', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          messages: [...messages, userMsg].map(m => ({ role: m.role === 'agent' ? 'assistant' : m.role, content: m.content })),
      ...
      while (true) {
        ...
        fullContent += chunk;
        setMessages(prev => prev.map(m => m.id === agentMsg.id ? { ...m, content: fullContent, status: 'streaming' } : m));
      }
      ...
      setMessages(prev => prev.map(m =>
        m.id === agentMsg.id ? { ...m, content: fullContent, status: 'sent', currentStep: 5, imageUrl } : m
      ));
  ...
  const updateStep = (msgId: string, idx: number, status: string, content: string) => {
    setMessages(prev => prev.map(m => {
      ...
      for (let i = 0; i < idx; i++) { if (steps[i]?.status !== 'done') steps[i] = { ...steps[i], status: 'done' }; }
  ...
  const handleSaveNote = async () => {
    if (messages.length < 2) { setError('沒有足夠的聊天內容來建立筆記'); return; }
  ...
  const handleTerminalCommand = async (cmd: string) => {
    ...
    switch (command) {
      case 'help':
        setTerminalOutput(prev => [...prev, '可用指令：', ...Object.entries(TERMINAL_COMMANDS).map(([k, v]) => `  ${k.padEnd(12)} — ${v}`), '']);
        break;
      case 'status':
      case 'health':
        try {
          ...
          setTerminalOutput(prev => [...prev, `資料庫: ${d.dbStatus} (${d.dbLatency}ms)`, `活躍代理: ${d.activeAgents}`, `記憶條目: ${d.codexEntries}`, '']);
        } catch { setTerminalOutput(prev => [...prev, '❌ 無法取得系統狀態', '']); }
        break;
      case 'agents':
        setTerminalOutput(prev => [...prev, '已註冊子代理：', '  1. SustainWrite Agent — 永續報告撰寫 [active]', '  2. Intelligence Agent — 商情分析 [active]', '  3. Audit Agent — 稽核驗證 [idle]', '  4. Data Agent — 數據管理 [active]', '  5. Vault Agent — 證據保管 [idle]', '  6. Notes Agent — 筆記整理 [active]', '  7. OWL Agent — 情報洞察 [active]', '']);
        break;
      case 'memory':
        setTerminalOutput(prev => [...prev, `記憶碎片: ${shards.length} 筆`, `來源: Supabase + NCBDB`, '']);
        break;
      case 'swarm':
        setTerminalOutput(prev => [...prev, `連線狀態: ${connectionStatus}`, `事件數量: ${events.length}`, '']);
        break;
      case 'sync':
        setTerminalOutput(prev => [...prev, '🔄 正在同步 NCBDB...', '']);
        try { await syncWithNCB(); setTerminalOutput(prev => [...prev, '✅ 同步完成', '']); }
        catch { setTerminalOutput(prev => [...prev, '❌ 同步失敗', '']); }
        break;
      case 'clear':
        setTerminalOutput(['╔══════════════════════════════════════════════════════════════╗', '║  OmniAgent Terminal v2.0                                      ║', '╚══════════════════════════════════════════════════════════════╝', '']);
        break;
      case 'nav':
        // 語言導航
        const target = args[0];
        if (target && ['chat', 'terminal', 'notes', 'memory', 'swarm', 'system'].includes(target)) {
          ...
          setTerminalOutput(prev => [...prev, '可用頁面: chat, terminal, notes, memory, swarm, system', '']);
        }
        break;
      default:
        setTerminalOutput(prev => [...prev, `❌ 未知指令: ${command}，輸入 "help" 查看可用指令`, '']);
    }
    setTerminalInput('');
  };

  const handleTerminalKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') { e.preventDefault(); handleTerminalCommand(terminalInput); }
  };
      ...
      <div className="max-w-7xl mx-auto px-6 py-6 space-y-4">
        ...
        <Card variant="default" padding="sm">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {health && <Badge variant={health.dbStatus === 'connected' ? 'success' : 'error'} size="sm">DB {health.dbLatency}ms</Badge>}
              <Badge variant={connectionStatus === 'connected' ? 'success' : 'warning'} size="sm">Swarm {connectionStatus}</Badge>
            </div>
          </div>
        </Card>
          ...
          {[
            ...
            <button
              ...
              className={cn(
                ...
                viewMode === tab.id ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'
              )}
            >
              <tab.icon size={14} />
              {tab.label}
              {tab.count !== undefined && tab.count > 0 && (
                <span className="text-[9px] px-1 py-0.5 rounded-full bg-neutral-200">{tab.count}</span>
              )}
            </button>
          ))}
        </div>
        ...
        {viewMode === 'chat' && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
            <div className="lg:col-span-3">
              <Card variant="default" padding="none" className="flex flex-col h-[calc(100vh-220px)]">
                <div className="px-4 py-2 border-b border-neutral-100 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="sm" onClick={handleSaveNote} disabled={messages.length < 2 || isSavingNote}>
                      <Save size={12} />
                      {isSavingNote ? '儲存中...' : '存筆記'}
                    </Button>
                    <Button variant="ghost" size="sm" onClick={handleClearChat}><Trash2 size={12} /></Button>
                  </div>
                </div>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2 justify-center">
                        {['分析 ESG 數據', '生成永續報告', '檢查合規狀態'].map(q => (
                          <button key={q} onClick={() => { setInput(q); handleSend(q); }}
                            className="text-xs px-3 py-1.5 rounded-full bg-neutral-100 text-neutral-600 hover:bg-neutral-200 transition-colors">{q}</button>
                        ))}
                      </div>
                    </div>
                  )}
                        ...
                        <div className="flex justify-end">
                          <div className="max-w-[80%] bg-neutral-900 text-white rounded-xl px-4 py-2.5">
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-[9px] text-white/50">{new Date(msg.timestamp).toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' })}</span>
                            </div>
                            ...
                            <div className="bg-neutral-50 rounded-lg border border-neutral-100 overflow-hidden">
                              <button onClick={() => toggleThinking(msg.id)}
                                ...
                                <span className="flex items-center gap-1.5">
                                  <Brain size={10} />
                                  思考過程 ({msg.thinkingSteps.filter(s => s.status === 'done').length}/{msg.thinkingSteps.length})
                                  {isLoading && msg.status === 'streaming' && <Loader2 size={8} className="animate-spin" />}
                                </span>
                                {expandedThinking.has(msg.id) ? <ChevronUp size={10} /> : <ChevronDown size={10} />}
                              </button>
                              {expandedThinking.has(msg.id) && (
                                <div className="px-3 pb-2 space-y-1">
                                  {msg.thinkingSteps.map((step, i) => {
                                    ...
                                    return (
                                      <div key={step.id} className={cn('flex items-start gap-2 p-1.5 rounded text-[10px]',
                                        step.status === 'active' ? 'bg-blue-50' : step.status === 'done' ? 'bg-emerald-50' : 'bg-white')}>
                                        <div className={cn('w-4 h-4 rounded flex items-center justify-center shrink-0',
                                          step.status === 'active' ? 'bg-blue-100' : step.status === 'done' ? 'bg-emerald-100' : 'bg-neutral-100')}>
                                          {step.status === 'active' ? <Loader2 size={8} className="animate-spin text-blue-600" /> :
                                           step.status === 'done' ? <CheckCircle2 size={8} className="text-emerald-600" /> :
                                           <Icon size={8} className="text-neutral-400" />}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                          <span className={cn('font-medium',
                                            step.status === 'active' ? 'text-blue-700' : step.status === 'done' ? 'text-emerald-700' : 'text-neutral-500')}>
                                            {step.title}
                                          </span>
                                          {step.content && <p className="mt-0.5 text-neutral-500 leading-relaxed">{step.content}</p>}
                                        </div>
                                      </div>
                                    );
                                  })}
                          ...
                          <div className="bg-white rounded-xl border border-neutral-100 px-4 py-3">
                            {msg.content && (
                              <div className="prose prose-sm max-w-none">
                                <p className="text-sm leading-relaxed whitespace-pre-wrap text-neutral-700">{msg.content.replace(/\[IMAGE:[^\]]+\]/g, '')}</p>
                              </div>
                            )}
                            {/* Generated Image */}
                            {msg.imageUrl && (
                              <div className="mt-3">
                                <img src={msg.imageUrl} alt="AI 生成圖片" className="rounded-lg max-w-full border border-neutral-100" />
                              </div>
                            )}
                            ...
                            <div className="flex items-center justify-between mt-2 pt-2 border-t border-neutral-50">
                              ...
                              <div className="flex items-center gap-1">
                                {msg.content && (
                                  <>
                                    <button onClick={() => speakText(msg.content)} className="p-1 text-neutral-400 hover:text-neutral-600">
                                      {isSpeaking ? <VolumeX size={12} /> : <Volume2 size={12} />}
                                    </button>
                                    <button onClick={() => handleCopy(msg.id, msg.content)} className="p-1 text-neutral-400 hover:text-neutral-600">
                ...
                <div className="px-4 py-3 border-t border-neutral-100">
                  <div className="flex items-end gap-2">
                    ...
                    {isLoading ? (
                      <Button variant="danger" size="md" onClick={handleStop}><Loader2 size={16} className="animate-spin" /></Button>
                    ) : (
                  ...
                  <p className="text-[9px] text-neutral-400 mt-1">Enter 發送 · Shift+Enter 換行 · 點擊麥克風語音輸入 · 指令 "nav chat" 切換頁面</p>
                </div>
              </Card>
            </div>
              ...
              <Card variant="default" padding="sm">
                ...
                <div className="space-y-1">
                  {['分析 ESG 數據', '生成永續報告', '檢查合規狀態', '同步記憶碎片'].map(cmd => (
                    <button key={cmd} onClick={() => { setInput(cmd); handleSend(cmd); }}
                      className="w-full text-left px-2.5 py-1.5 rounded text-[11px] text-neutral-600 hover:bg-neutral-50 transition-colors">{cmd}</button>
                  ))}
                </div>
              </Card>
              ...
              <div className="p-4 h-[calc(100vh-260px)] overflow-y-auto space-y-0.5">
                {terminalOutput.map((line, i) => (
                  <div key={i} className={cn(
                    ...
                    line.startsWith('╔') || line.startsWith('║') || line.startsWith('╚') ? 'text-neutral-500' :
                    'text-neutral-300'
                  )}>
                    {line || '\u00A0'}
                  </div>
            ...
            <div className="lg:col-span-1">
              <Card variant="default" padding="sm">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button variant="primary" size="sm" onClick={handleSaveNote} disabled={messages.length < 2 || isSavingNote}>
                    <Save size={12} />
                  ...
                  <div className="space-y-1">
                    {notes.map(note => (
                      ...
                      >
                        ...
                        <p className="text-[10px] text-neutral-400 mt-0.5">{note.messageCount} 則訊息 · {new Date(note.createdAt).toLocaleDateString('zh-TW')}</p>
                      </button>
                    ))}
                  </div>
                )}
            ...
            <div className="lg:col-span-2">
              {selectedNote ? (
                <Card variant="default" padding="md">
                  ...
                  <div className="prose prose-sm max-w-none">
                    <p className="text-sm text-neutral-700 leading-relaxed whitespace-pre-wrap">{selectedNote.content}</p>
                  </div>
                </Card>
              ) : (
        ...
        {viewMode === 'memory' && (
          <div className="space-y-4">
            <SectionHeader title="共享記憶層" subtitle={`${shards.length} 個碎片`}
              action={<div className="flex gap-2">
                <Button variant="ghost" size="sm" onClick={fetchShards} disabled={memoryLoading}><RefreshCw size={14} className={memoryLoading ? 'animate-spin' : ''} /></Button>
                <Button variant="primary" size="sm" onClick={syncWithNCB} disabled={memoryLoading}><DatabaseZap size={14} />同步 NCBDB</Button>
              </div>}
            />
            {shards.length === 0 && !memoryLoading ? (
              <Card variant="default" padding="lg"><div className="text-center space-y-2"><DatabaseZap size={24} className="text-neutral-200 mx-auto" /><p className="text-sm text-neutral-500">尚無記憶碎片</p></div></Card>
            ) : (
                ...
                {shards.map(shard => (
                  <Card key={shard.id} variant="default" padding="sm" hover>
                    <div className="flex items-start justify-between mb-1">
                      ...
                      <Badge variant={shard.source_origin === 'ncb' ? 'info' : 'neutral'} size="sm">{shard.source_origin === 'ncb' ? 'NCB' : 'Local'}</Badge>
                    </div>
                    <p className="text-[10px] text-neutral-500 line-clamp-2 mb-1">{shard.description}</p>
                    <div className="flex items-center gap-2 text-[9px] text-neutral-400">
                      <span>熵:{shard.entropy_level}</span><span>·</span><span>權重:{shard.importance_score?.toFixed(2)}</span><span>·</span><span>使用:{shard.usage_count}次</span>
                    </div>
                  </Card>
                ))}
              </div>
        ...
        {viewMode === 'swarm' && (
          <div className="space-y-4">
            <SectionHeader title="群蜂戰情室" subtitle={`${events.length} 個事件`}
              action={<div className="flex gap-2">
                <Badge variant={connectionStatus === 'connected' ? 'success' : 'warning'} size="sm">{connectionStatus}</Badge>
                <Button variant="ghost" size="sm" onClick={clearEvents}><Trash2 size={14} />清除</Button>
              </div>}
            />
            <Card variant="default" padding="none">
              <div className="bg-neutral-900 rounded-xl p-4 font-mono text-xs h-[500px] overflow-y-auto space-y-1">
                {events.length === 0 ? <div className="text-neutral-600 text-center mt-20">等待 Swarm 事件...</div> : events.map(ev => (
                  <div key={ev.id} className="border-l-2 border-neutral-700 pl-3 py-0.5">
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className={cn('px-1.5 rounded text-[9px] font-medium',
                        ev.source === 'OmniGateway' ? 'bg-emerald-900/50 text-emerald-400' :
                        ev.source === 'SharedMemory' ? 'bg-cyan-900/50 text-cyan-400' : 'bg-neutral-800 text-neutral-300')}>{ev.source}</span>
                      <span className="text-amber-400/80">[{ev.type}]</span>
                    </div>
                    <div className="text-neutral-400 pl-4 break-words">{typeof ev.payload === 'string' ? ev.payload : JSON.stringify(ev.payload)}</div>
                  </div>
                ))}
              </div>
            </Card>
                ...
                <Card key={item.label} variant="default" padding="sm">
                  <div className="flex items-center gap-2">
                    <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center',
                      item.color === 'success' ? 'bg-emerald-50' : item.color === 'error' ? 'bg-red-50' : 'bg-blue-50')}>
                      <item.icon size={14} className={cn(item.color === 'success' ? 'text-emerald-600' : item.color === 'error' ? 'text-red-600' : 'text-blue-600')} />
                    </div>
                    <div><p className="text-[10px] text-neutral-500">{item.label}</p><p className="text-sm font-bold text-neutral-900">{item.value}</p></div>
                  </div>
                </Card>
              ))}
            </div>
