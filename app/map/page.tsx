...
interface MapNode {
  ...
  category: string;
  description: string;
  r: number; // radius for circular layout
...
const mapNodesData: MapNode[] = [
  // 1. Perception Ring (r = 110)
  {
    id: 'ui-001',
    name: 'OmniBaseCard',
    category: 'Perception',
  ...
  },
  {
    ...
    painPointsSolved: '解決複雜數據依賴關係難以理解、數據血緣 (Data Lineage) 混亂不可考的難題。',
    updateDate: '2026-06-14',
    fiveTStatus: [true, true, true, false, false],
  },
  {
    ...
    businessLogic: '串接安全資料金庫 (Vault)，欄位層級實時解密渲染，支援零洩漏模糊檢索與權限過濾。',
    uiux: '密碼字元漸次解碼、欄位發光鎖頭圖示、高安全性黑色毛玻璃底色。',
    customerJourney: '高機密數據編輯與審查旅程，符合最嚴格的資安標準。',
    painPointsSolved: '解決資料庫明文儲存易洩密、後台管理員越權查看用戶隱私資料的重大資安漏洞。',
    updateDate: '2026-06-14',
  ...
  },
  {
    ...
    businessLogic: '展示資料庫表結構、連線池 (Connection Pool) 佔用率與 PostgreSQL 唯讀複本狀態。',
  ...
  },
  {
    ...
    businessLogic: '檢視與熱插拔代理技能包，即時向 Agent 注入新的 PDF 說明書或 Python 運算模組。',
  ...
  // 4. Security Ring (r = 290)
  {
    ...
    painPointsSolved: '解決 ESG 報告和歷史數據易被篡改、缺乏不可否認性技術保障的痛點。',
    updateDate: '2026-06-14',
    fiveTStatus: [true, true, true, true, true],
  },
  {
    ...
    businessLogic: '本地載入 ZK-SNARKs 證書，在不洩露具體數值的情況下驗證碳排或財務指標是否合規。',
  ...
  },
  {
    ...
    businessLogic: '匯整異常存取、暴力破解、越權注入等安全事件，支援一鍵封鎖惡意 IP 和自動限流。',
  ...
  },
  {
    ...
    painPointsSolved: '解決系統組件散亂、版本衝突難以盤點、代碼資產缺乏統一視覺展示介面的空缺。',
    updateDate: '2026-06-14',
    fiveTStatus: [true, true, true, true, true],
  },
  {
  ...
  // 6. Healing Ring (r = 380)
  {
    id: 'hea-001',
    name: 'HealingGuardian',
    category: 'Healing',
    description: '自主防禦與 5T 誠信修復守護者。',
    r: 380,
    angle: Math.PI / 8,
    registered: true,
    deliverables: ['lib/healing-guardian.ts', 'app/api/metrics/heal/route.ts'],
    businessLogic: '秒級攔截異常數據篡改，對齊絕對真理金庫，調用結晶協定重置故障狀態。',
    uiux: '極簡波紋擴散脈衝、3D 圓柱自癒進度、霓虹字元日誌終端機。',
    customerJourney: '系統主權與自適應演進旅程，全系統防衛之核心大腦。',
    painPointsSolved: '解決人為蓄意篡改或非授權邏輯漂移導致的 5T 誠信崩潰。',
    updateDate: '2026-06-14',
    fiveTStatus: [true, true, true, true, true],
  },
  {
    ...
    name: 'JESMonitor',
    category: 'Healing',
    description: '全域能耗與系統狀態的健康監控感測。',
    r: 380,
    angle: (9 * Math.PI) / 8,
    registered: false,
    deliverables: ['lib/ncbdb.ts', 'NCBDB_PROTOCOL.md'],
    businessLogic: '持續追蹤全系統能耗與 Webhook 連線脈搏，將異常訊號即時傳輸至自癒中樞。',
    uiux: '心電圖波形動效、呼吸式綠黃紅狀態指示。',
    customerJourney: '全站合規探測旅程，為自癒引擎提供第一手感知神經。',
    painPointsSolved: '解決系統隱蔽性故障與斷線無法實時探測、異常響應不及時的缺憾。',
    updateDate: '2026-06-14',
    fiveTStatus: [true, true, true, false, false],
  },
];
...
// Ring Colors & Names
const RINGS = [
  { name: '感知。UI 基礎 (Perception)', r: 110, color: 'stroke-cyan-500/20' },
  { name: '全域。數據與整合 (Data)', r: 170, color: 'stroke-indigo-500/20' },
  { name: '指揮。代理協作 (Command)', r: 230, color: 'stroke-emerald-500/20' },
  { name: '全知。防禦與安全 (Security)', r: 290, color: 'stroke-rose-500/20' },
  { name: '全息。永續與報告 (Hologram)', r: 340, color: 'stroke-amber-500/20' },
  { name: '自癒。自主防禦 (Healing)', r: 380, color: 'stroke-pink-500/20' },
];

export default function OmniMapPage() {
  const [selectedNode, setSelectedNode] = useState<MapNode>(mapNodesData[0]);
  const [healingLogs, setHealingLogs] = useState<string[]>([
    '>> SYSTEM ORBITS INITIALIZED',
    '>> 5T TOPOLOGY SENSORS: ACTIVE',
  ]);
  const [isHealing, setIsHealing] = useState(false);
  const [laserScan, setLaserScan] = useState(false);

  // SVG Center is (400, 400)
  const cx = 400;
  const cy = 400;
  ...
  const handleDriftScan = () => {
    setLaserScan(true);
    setHealingLogs((prev) => [
      `>> INITIATING HOLISTIC 5T DRIFT SCAN ON #${selectedNode.id}...`,
      ...prev.slice(0, 6),
    ]);
    setTimeout(() => {
      setLaserScan(false);
      const integrity = selectedNode.registered ? '100% VALID' : 'WARNING: COMPONENT PENDING';
      setHealingLogs((prev) => [
        `>> SCAN COMPLETED FOR ${selectedNode.name}`,
        `>> STATUS: ${integrity}`,
  ...
  const handleSelfHeal = async () => {
    setIsHealing(true);
    setHealingLogs((prev) => [
      `>> INITIATING OMNICORE HEALING FOR ${selectedNode.name}...`,
      ...prev.slice(0, 6),
    ]);
    try {
      const response = await fetch('/api/metrics/heal', {
        ...
        body: JSON.stringify({ targetId: selectedNode.id, table: 'map-topology' }),
      });
      const resData = await response.json();
      if (resData.success && resData.report) {
        const report = resData.report;
        setHealingLogs((prev) => [
          `>> HEALING SUCCESSFUL: ${selectedNode.name} RESTORED`,
          `>> NEW RECOVERY HASH LOCKED: ${report.recoveryHash.substring(0, 16)}`,
          `>> ENTROPY FORGE COMPLIANCE ATTAINED`,
          ...prev.slice(0, 4),
        ]);
        // Force state updates to show registered in UI
        if (!selectedNode.registered) {
          setSelectedNode((prev) => ({ ...prev, registered: true }));
        }
      } else {
        setHealingLogs((prev) => [
          `>> HEALING ABORTED: ${selectedNode.name} already in optimal state.`,
          ...prev.slice(0, 6),
        ]);
      }
    } catch (e) {
      setHealingLogs((prev) => [
        `>> SYNC FAILED: Unable to reach recovery vault node.`,
        ...prev.slice(0, 6),
      ]);
    } finally {
      setIsHealing(false);
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 w-full bg-[#020617] text-slate-700 relative overflow-hidden selection:bg-cyan-500/30">
      ...
      <div className="max-w-7xl mx-auto space-y-8 relative z-10 animate-in fade-in duration-700">
        <OmniHeader
          title="全端全通因果律拓樸圖 (OmniMap 2.0)"
          subtitle="六大星環因果鏈路 · 全域元件自癒可視化終端"
          ...
          <div className="lg:col-span-8 bg-slate-950/40 -md border border-slate-200 rounded-3xl p-6 relative overflow-hidden flex flex-col items-center justify-center min-h-[500px] lg:min-h-[650px] shadow-[inset_0_0_20px_rgba(255,255,255,0.01)]">
            ...
            <svg viewBox="0 0 800 800" className="w-full max-w-[620px] aspect-square relative z-0">
              ...
              {laserScan && (
                ...
                >
                  <animate attributeName="r" values="0;390" dur="1.5s" repeatCount="indefinite" />
                  <animate
              ...
              {isHealing && (
                ...
                >
                  <animate attributeName="r" values="390;0" dur="1s" repeatCount="indefinite" />
                  <animate
                    attributeName="opacity"
                    values="0.1;1"
                    dur="1s"
              ...
              {RINGS.map((ring, idx) => (
                <g key={idx}>
                  <circle
                    ...
                    r={ring.r}
                    fill="none"
                    className={cn('stroke-[1px] stroke-dasharray-[4_8]', ring.color)}
                  />
                  {/* Orbit Name labels lightly in the background */}
                  <text
                    x={cx}
                    y={cy - ring.r - 4}
                    className="fill-slate-600/70 text-[7px] font-mono text-center select-none"
                    textAnchor="middle"
                  >
                    {ring.name}
                  </text>
              ...
              {/* 2. Concentric cross lines to show connection causality */}
              {Array.from({ length: 8 }).map((_, idx) => {
                const angle = (idx * Math.PI) / 4;
                const x2 = cx + 390 * Math.cos(angle);
                const y2 = cy + 390 * Math.sin(angle);
                return (
                  <line
                    key={idx}
                    x1={cx}
                    ...
                    x2={x2}
                    y2={y2}
                    className="stroke-slate-800/15 stroke-[1px] stroke-dasharray-[2_10]"
                  />
                );
              })}
                ...
                return (
                  ...
                  >
                    ...
                    <circle
                      ...
                      className={cn(
                        ...
                        isSelected
                          ...
                          : 'fill-slate-900/80 stroke-amber-500/40 hover:stroke-amber-400 hover:fill-amber-950'
                      )}
                      filter={isSelected ? 'url(#glow-cyan)' : undefined}
                    />

                    {/* Small inner node status dot */}
                    <circle
                      cx={nx}
                      cy={ny}
                      r="2.5"
                      className={cn(node.registered ? 'fill-cyan-100' : 'fill-amber-100')}
                    />
                    ...
                    <text
                      ...
                      y={ny - 12}
                      className={cn(
                        'text-[8px] font-bold font-mono transition-opacity duration-300 select-none pointer-events-none',
                        isSelected
                          ? 'fill-white opacity-100 text-[9px]'
                          : 'fill-slate-400 opacity-0 group-hover:opacity-100'
                      )}
                      textAnchor="middle"
                    >
                      {node.name}
              ...
              {/* The Absolute Core Orb (Trinity JunAiKey Unified Hub) */}
              <circle
                cx={cx}
                cy={cy}
                r="18"
            ...
            {/* Quick Map Legend */}
            <div className="absolute bottom-4 left-4 z-20 flex gap-4 text-[10px] font-mono text-slate-500">
              ...
              <div className="space-y-4 flex-1">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button
                    ...
                    disabled={isHealing}
                    className="py-2.5 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/25 rounded-xl text-xs font-bold transition-all text-emerald-400 flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                  >
                    {isHealing ? (
                ...
                <div className="space-y-2 pt-2 border-t border-slate-200">
                  ...
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-bold text-slate-500 flex items-center gap-1">
                      <Cpu size={10} className="text-cyan-400" /> 業務邏輯 (Business Logic)
                    </span>
                  ...
                  <div className="flex flex-col gap-1">
                    <span className="text-[10px] font-bold text-slate-500 flex items-center gap-1">
                      <Palette size={10} className="text-cyan-400" /> UIUX 設計
                    </span>
                    <p className="text-[11px] text-slate-400 leading-normal">{selectedNode.uiux}</p>
                  </div>

                  <div className="grid grid-cols-2 gap-3 pt-1">
                    <div className="flex flex-col gap-1">
                      <span className="text-[9px] font-bold text-slate-500 flex items-center gap-1">
                        <Compass size={8} className="text-cyan-400" /> 客戶旅程
                      </span>
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-[9px] font-bold text-slate-500 flex items-center gap-1">
                        <Heart size={8} className="text-cyan-400" /> 痛點解決
                      </span>
