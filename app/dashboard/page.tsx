...
export default function SovereignDashboard() {
  ...
  // 實時連動：模擬接收來自 OmniCore 的 WebSocket 蜂群廣播日誌
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (Math.random() > 0.75) {
        setHiveMindLogs(prev => {
          const newLogs = [...prev, `[${new Date().toISOString().substring(11, 19)}] Swarm Event: OmniCore Resonance heartbeat check... OK.`];
          return newLogs.slice(-20); // 保持最新 20 筆
        });
      }
    }, 4500);
      ...
      <div className="w-80 h-full flex flex-col p-4 bg-black/20">
        ...
        <div className="flex-1 border border-white/10 rounded-lg bg-[#020617] p-3 overflow-y-auto text-[10px] text-slate-400 font-mono space-y-1.5 custom-scrollbar shadow-inner">
          {hiveMindLogs.map((log, i) => (
              <div key={i} className={`break-words ${log.includes('OmniCore') || log.includes('🌐') ? 'text-[#06b6d4]' :
