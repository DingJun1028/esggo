...
export default function GRITrackerPage() {
  ...
  const filtered = useMemo(() => {
    return matrix.filter((item) => {
      ...
      const matchSearch =
        item.code.toLowerCase().includes(search.toLowerCase()) || item.titleZh.includes(search);
  ...
  const pageConfig: UniversalPageConfig = {
    ...
    kpis: [
      ...
      },
      {
        key: 'done',
        label: '已封印指標',
        value: matrix.filter((i) => i.isSealed).length,
      ...
      },
      {
        key: 'pending',
        label: '待處理項',
        value: matrix.length - stats.completed,
        icon: <Clock size={18} />,
        color: '#FDB515',
      },
      {
    ...
    sections: [
            ...
            {Object.entries(CATEGORY_META).map(([k, meta]) => {
              ...
              return (
                <Card
                  key={k}
                  padding="lg"
                  className="glass-panel border-none shadow-sm hover:shadow-lg transition-all group"
                >
                  <div className="flex items-center gap-3 mb-4">
                    <div
                      className="w-10 h-10 rounded-xl flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform"
      ...
      },
      {
        ...
        component: (
          <Card
            padding="lg"
            className="bg-slate-900 border-none text-white overflow-hidden relative"
          >
            ...
            <div className="relative z-10 flex flex-col md:flex-row gap-8 items-start">
              <div className="flex-1 space-y-4">
                ...
                {gapAdvice ? (
                  <div className="p-6 bg-white/5 rounded-2xl border border-slate-200 font-medium text-sm leading-relaxed text-cyan-50/80">
                    {gapAdvice}
                  </div>
                ) : (
          ...
          <div className="space-y-8">
            <div className="flex flex-col md:flex-row gap-6">
              <div className="flex-1 relative group">
                <Search
                  size={18}
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#003262] transition-colors"
                />
                <input
                  ...
                  <button
                    ...
                    className={`px-5 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all ${
                      categoryFilter === cat
                        ...
                        : 'text-slate-400 hover:text-[#003262]'
                    }`}
                  >
                ...
                data={filtered.map((i) => ({
                  status: (
                    <Badge
                      ...
                      pulse={i.status === 'in_progress'}
                    />
                  ),
                  code: (
                    <span className="font-mono text-xs font-black text-[#003262]">{i.code}</span>
                  ),
                  name: (
                  ...
                  progress: (
                    <div className="flex items-center gap-3 w-40">
                      <BrandProgress
                        value={i.completeness}
                        size="xs"
                        color={i.completeness === 100 ? 'green' : 'blue'}
                        className="flex-1"
                      />
                      <span className="font-mono text-[10px] font-black w-8 text-right">
                        {i.completeness}%
                      </span>
                    </div>
                  ),
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-6 lg:p-12">
          ...
          <div className="relative bg-white/95 backdrop-blur-2xl rounded-[40px] border border-white shadow-extreme p-10 lg:p-14 max-w-2xl w-full overflow-hidden">
            ...
            <header className="flex justify-between items-start mb-12 relative z-10">
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-3xl bg-blue-50 flex items-center justify-center text-[#003262] shadow-sm">
                    <FileText size={28} />
                  </div>
                  <div>
                    <h3 className="text-3xl font-black text-[#003262] tracking-tighter">
                      {selected.code}
                    </h3>
              ...
              <button
                onClick={() => setSelected(null)}
                className="w-12 h-12 rounded-2xl bg-slate-50 hover:bg-slate-100 flex items-center justify-center text-slate-400 transition-all hover:rotate-90"
              >
                <X size={24} />
              </button>
            </header>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-3 p-4 bg-slate-50/50 rounded-2xl border border-transparent hover:border-slate-100 transition-all">
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${
                          selected.hasEvidence ? 'bg-emerald-500' : 'bg-slate-300'
                        }`}
                      />
                    ...
                    <div className="flex items-center gap-3 p-4 bg-slate-50/50 rounded-2xl border border-transparent hover:border-slate-100 transition-all">
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${
                          selected.tasksCount > 0 ? 'bg-blue-500' : 'bg-slate-300'
                        }`}
                      />
                      <span className="text-xs font-bold text-slate-600">
                ...
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <p className="text-[10px] opacity-70 font-medium leading-relaxed">
                      {selected.isSealed
                        ? '此指標已完成 T5 數位封印，具備最高治理主權與誠信驗證。'
                        : '目前正在收集中，待啟動 Hash Lock 封印以確保數據不可篡改。'}
                    </p>
                  </div>
                </div>

                <div className="p-6 bg-[#003262] rounded-[28px] text-white">
                  <p className="text-[10px] font-black text-blue-200/50 uppercase tracking-[0.3em] mb-3">
                    T5 TAGS
                  </p>
                  <div className="flex gap-2">
                    {['T1', 'T2', 'T3'].map((t) => (
                      <Badge
                        ...
                        variant="info"
                        size="xs"
                        className="bg-white/10 border-none text-blue-100 px-3"
                      >
                        {t}
                      </Badge>
                    ))}
                    {selected.isSealed && (
                      <Badge variant="gold" size="xs" className="px-3">
                        T5
                      </Badge>
                    )}
                  </div>
            ...
            <footer className="mt-auto pt-8 border-t border-slate-100 flex items-center justify-between relative z-10">
              <Button variant="ghost" className="rounded-xl h-12" onClick={() => setSelected(null)}>
                關閉視窗
              </Button>
              <Button
                variant="primary"
              ...
              >
                前往撰寫編輯器 <ArrowUpRight size={16} className="ml-2" />
              </Button>
            </footer>
          </div>
        </div>
