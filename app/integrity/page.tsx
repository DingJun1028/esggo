...
export default function IntegrityPage() {
  ...
  const handleGenerateZKP = async () => {
    ...
    setTimeout(
      () => addLog('擷取原始憑證: 碳排數值區間檢查 (Claim: emission < 1000 tCO2e)...'),
      800
    );
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-gray-100 p-8 font-sans selection:bg-[#003262] selection:text-white">
      ...
      <header className="mb-12 border-b border-gray-800 pb-6">
        ...
        <p className="mt-2 text-gray-400 max-w-2xl text-sm leading-relaxed">
          基於 Berkeley Sovereign v10.0 架構，透過密碼學技術確保 ESG 數據的不可篡改性 (Trustworthy)
          與可追溯性 (Traceable)。實作數值區間證明，保障企業隱私的同時提供透明的合規宣稱。
        </p>
      </header>
          ...
          <div className="bg-[#111111] border border-gray-800 rounded-xl p-6 shadow-2xl relative overflow-hidden group">
            ...
            <div className="space-y-4 mb-8">
              <div className="bg-[#1A1A1A] p-4 rounded-lg border border-gray-800">
                ...
                <p className="font-mono text-sm mt-1 text-[#FDB515]">
                  Scope 1 & 2 Carbon Emission &lt; 1000 tCO2e
                </p>
              </div>
              <div className="bg-[#1A1A1A] p-4 rounded-lg border border-gray-800">
                <span className="text-xs text-gray-500 uppercase tracking-wider font-bold">
                ...
                <p className="font-mono text-sm mt-1 text-gray-300">
                  ESGGO Dataform Pipeline (GCP)
                </p>
              </div>
            </div>
            ...
            <div className="bg-neutral-100 from-[#002014] to-[#111111] border border-[#52C41A]/30 rounded-xl p-6 shadow-2xl relative overflow-hidden">
              ...
              <div className="space-y-3 font-mono text-xs text-gray-400">
                <p>
                  ...
                  <span className="text-[#52C41A] font-bold">Verified (Immutable)</span>
                </p>
                <p className="break-all">
                  <span className="text-gray-500">Circuit Hash:</span> <br />
                  {proofData.circuit_hash}
                </p>
                <p>
                  <span className="text-gray-500">Public Signal:</span> <br />
                  {JSON.stringify(proofData.public_signal)}
                </p>
                <p>
                  <span className="text-gray-500">Proof [pi_a]:</span> <br />
                  {proofData.zkp_proof.pi_a.join(', ')}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="bg-[#1E1E1E] border border-gray-800 rounded-xl flex flex-col overflow-hidden shadow-2xl h-[600px]">
          ...
          <div className="flex-1 p-4 font-mono text-sm overflow-y-auto custom-scrollbar bg-[#0C0C0C]">
            <div className="space-y-2">
              ...
              {isGenerating && <div className="text-[#FDB515]">_</div>}
              <div ref={logEndRef} />
            </div>
          </div>
        </div>
