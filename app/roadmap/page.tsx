...
import { Map, Search, Plus, ShieldCheck, Activity, Brain, Lock, Loader2, X } from 'lucide-react';
...
export default function RoadmapPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/metrics/roadmap', { cache: 'no-store' });
      if (res.ok) {
  ...
  const handleSeal = async (id: number) => {
    ...
    try {
      const response = await fetch('/api/vault/seal', {
        ...
        body: JSON.stringify({ 
          evidence: { table: 'roadmap', recordId: id, timestamp: Date.now() }, 
          type: '5t-seal' 
        })
      });
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <span className="text-xs font-mono text-slate-500 uppercase tracking-widest">ROADMAP</span>
              </div>
              <h1 className="text-4xl font-black text-white tracking-tight">ROADMAP</h1>
              <p className="text-slate-400 font-mono text-sm tracking-widest uppercase mt-2">roadmap dashboard</p>
            </div>
          </div>
