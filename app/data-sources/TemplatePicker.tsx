...
import { ESG_TEMPLATES, type ESGTemplate } from '@/lib/oa-table/templates';
...
export function TemplatePickerModal({
  ...
  open: boolean;
  onClose: () => void;
  onConfirm: (t: ESGTemplate) => Promise<void>;
  creating: boolean;
}) {
  const [selected, setSelected] = useState<string | null>(null);
  const [filter, setFilter] = useState<'all' | 'E' | 'S' | 'G'>('all');
  const templates =
    filter === 'all' ? ESG_TEMPLATES : ESG_TEMPLATES.filter((t) => t.category === filter);
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title="Create ESG Datasheet from Template"
      width="max-w-2xl"
    >
      {/* Category Tabs */}
          ...
          return (
            <button
              ...
              className={`text-left p-4 rounded-xl border-2 transition-all ${
                selected === t.id
                  ? 'border-berkeley-blue bg-blue-50/30'
                  : 'border-slate-100 hover:border-slate-200'
              }`}
            >
              <div className="flex items-center gap-2 mb-1.5">
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-bold border ${meta.color}`}
                >
      ...
      <div className="flex items-center justify-between pt-3 border-t border-slate-100">
        <p className="text-[10px] text-slate-400">
          {selected ? `Selected: ${tmpl?.nameZh}` : 'Select a template'}
        </p>
        <button
          disabled={!selected || creating}
          onClick={() => tmpl && onConfirm(tmpl)}
