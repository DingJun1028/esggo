...
import { GraduationCap, Search, Plus, ShieldCheck, Activity, Brain, Lock, Loader2, X } from 'lucide-react';
...
export default function AcademyPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/metrics/academy', { cache: 'no-store' });
      if (res.ok) {
  ...
  const handleSeal = async (id: number) => {
    ...
    try {
      const response = await fetch('/api/vault/seal', {
        ...
        body: JSON.stringify({ 
          evidence: { table: 'academy', recordId: id, timestamp: Date.now() }, 
          type: '5t-seal' 
        })
      });
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pb-6 border-b border-white/5">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-600/20 flex items-center justify-center border border-cyan-500/30 shadow-[0_0_30px_rgba(6,182,212,0.15)] relative group">
              ...
              <GraduationCap className="text-cyan-400 relative z-10" size={28} />
            </div>
            <div>
