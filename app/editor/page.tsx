...
const REPORT_CHAPTERS = [
  ...
  {
    id: 'chap-04',
    name: '重大性議題分析 (Materiality)',
    order: 4,
    gri: ['GRI 3-1', 'GRI 3-2', 'GRI 3-3'],
    words: 15600,
    pages: 13.0,
  },
  {
...
export default function EditorPage() {
  ...
  const editorRef = useRef<OmniSustainWriteEditorRef>(null);
  ...
  const handleAIExpand = async () => {
    await expandContentWithAI(
      ...
      activeChapter.gri,
      customPrompt
        ? `請針對「${activeChapter.name}」進行深度展開，需符合 ${activeChapter.gri.join(
            ', '
          )} 規範，使用專業語氣，強調具體行動與數據量化。\n【附加指示】：${customPrompt}`
        : `請針對「${activeChapter.name}」進行深度展開，需符合 ${activeChapter.gri.join(
            ', '
          )} 規範，使用專業語氣，強調具體行動與數據量化。`
    );
        ...
        >
          {REPORT_CHAPTERS.map((chapter) => (
            ...
            >
              ...
              {activeChapter.id === chapter.id && (
                <ChevronRight size={14} className="shrink-0 text-cyan-600 dark:text-cyan-400" />
              )}
            </button>
          ))}
        </OmniBaseCard>
        ...
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 dark:bg-slate-900/40 shadow-[0_8px_32px_rgba(0,0,0,0.05)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.3)] p-5 rounded-2xl border border-slate-200 dark:border-white/10 relative overflow-hidden">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-3">
              {activeChapter.name}
              {isGenerating && (
                <span className="flex items-center gap-1 text-[10px] font-mono tracking-widest text-cyan-400 px-2 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20">
                  <Activity size={10} className="animate-pulse" /> GENERATING
                </span>
              )}
            </h1>
            <div className="flex flex-wrap items-center gap-3 mt-2">
              <div className="flex flex-wrap gap-2">
                {activeChapter.gri.map((g) => (
                  <span
                    key={g}
                    className="text-[10px] font-mono tracking-widest px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/20"
                  >
                    {g}
                  </span>
                ))}
              ...
              <div className="text-xs text-slate-400 font-mono flex items-center gap-2">
                ...
                <span className="text-cyan-400 font-bold">
                  {activeChapter.words.toLocaleString()} 字
                </span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <OmniButton variant="outline"  onClick={handleSave}>
              保存草稿
            </OmniButton>
            <OmniButton
              variant="primary"
            ...
            >
              <span className="flex items-center gap-1">{isGenerating ? <Loader2 className="animate-spin" size={14} /> : <Wand2 size={14} />}專家 AI 展開 (Scribe)</span>
            </OmniButton>
          </div>
        </header>
