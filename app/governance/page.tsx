...
const MetricCard = React.memo(({ title, value, unit, icon: Icon, trend, colorClass }: any) => (
  ...
  >
    ...
    <div className="mt-2 flex items-baseline gap-2">
      <span className="text-3xl font-black text-slate-800 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">
        {value}
      </span>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-[#0f172a] border border-cyan-500/30 rounded-2xl w-full max-w-2xl shadow-[0_0_50px_rgba(6,182,212,0.15)] overflow-hidden flex flex-col relative">
        ...
        <div className="p-6 border-b border-slate-200 flex justify-between items-center">
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white transition-colors p-2 rounded-full hover:"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <div className="flex justify-between relative">
            ...
            <div
              className="absolute top-1/2 left-0 h-0.5 bg-cyan-500 -z-10 -translate-y-1/2 transition-all duration-500"
              style={{ width: `${(step / sequenceLength) * 100}%` }}
            ></div>
        ...
        <div className="p-6 flex flex-col gap-6 max-h-[70vh] overflow-y-auto">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
              ...
              <p className="text-emerald-400 font-mono animate-pulse tracking-widest text-sm">
                  ...
                  {result.gaps?.map((gap: any, i: number) => (
                    ...
                    >
                      <span
                        className={
                          gap.severity === 'HIGH'
                            ? 'px-2 py-0.5 rounded text-[10px] font-bold bg-red-500/20 text-red-400'
                            : 'px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500/20 text-amber-400'
                        }
                      >
                        {gap.severity}
                      </span>
...
export default function GovernanceDashboard() {
  ...
  const filteredData = useMemo(() => {
    return activeTab === 'All'
      ? governanceData
      : governanceData.filter((d) => d.category === activeTab);
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-700 overflow-hidden flex flex-col font-mono relative selection:bg-cyan-500/30 selection:text-white">
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        <div
          className="absolute top-[-20%] left-[10%] w-[50%] h-[50%] bg-amber-500/10 blur-[120px] rounded-full animate-pulse"
          style={{ animationDuration: '8s' }}
        />
        <div
          className="absolute bottom-[-10%] right-[-10%] w-[60%] h-[60%] bg-cyan-500/10 blur-[130px] rounded-full animate-pulse"
          style={{ animationDuration: '12s' }}
        />
        <div
          className="absolute top-[40%] left-[-10%] w-[40%] h-[40%] bg-indigo-500/10 blur-[100px] rounded-full animate-pulse"
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="px-2 py-0.5 bg-amber-950 border border-amber-500/30 text-amber-400 text-[10px] font-bold uppercase tracking-widest rounded">
                    TCFD / SASB
                  </span>
                  <span className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">
                    Corporate Governance
                  </span>
                  <span className="text-[10px] text-slate-500 font-bold ml-2 flex items-center gap-1">
                    <ShieldCheck size={12} className="text-emerald-400" /> 5T-VERIFIED
                  </span>
                </div>
                <h1
                  ...
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-white/5 bg-transparent">
                      {filteredData.map((row) => (
                        <tr key={row.id} className="hover: transition-colors group">
                          ...
                          <td className="px-6 py-4 text-xs text-slate-500 font-mono text-right">
                            {row.target}
                          </td>
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="space-y-4 text-sm text-slate-300">
                      <p className="leading-relaxed">
                        偵測到{' '}
                        <strong className="text-red-400 font-bold px-1 bg-red-500/10 rounded">
                          高風險供應商稽核率 (85%)
                        </strong>{' '}
                        低於年度目標設定 (90%)。5T Trackable 協定判定存在斷鏈風險。
                      </p>
                      <div className="p-4 bg-red-950/30 rounded-xl border border-red-500/20 mt-4">
                        ...
                        <ul className="list-none space-y-3 text-xs">
                          <li className="flex items-start gap-2">
                            ...
                            <span className="text-slate-300">
                              啟動 OmniAgent 供應商自動化追查程序。
                            </span>
                          </li>
                          <li className="flex items-start gap-2">
                            <div className="w-1.5 h-1.5 bg-red-400 rounded-full mt-1.5 shrink-0"></div>
                            <span className="text-slate-300">
                              發送合規聲明書至未稽核廠商，並套用 ZKP 簽章。
                            </span>
                          </li>
                        </ul>
                      </div>
