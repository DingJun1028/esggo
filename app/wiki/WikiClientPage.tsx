...
${page.acceptance_5t || ''}
  ...
  const activeSection =
    filteredSections.length > 0
      ? filteredSections.includes(sections[activeIndex])
    ...
    return text.split('\n\n').map((block, i) => {
      ...
      if (block.startsWith('### ')) {
        return (
          <h3
            key={i}
            className="text-xl font-black text-cyan-400 mt-8 mb-4 tracking-wider flex items-center gap-2"
          >
            <div className="w-1.5 h-6 bg-cyan-500 rounded-full" /> {block.replace('### ', '')}
          </h3>
        );
      }
      ...
      if (block.startsWith('|')) {
        ...
        return (
          <div
            key={i}
            className="overflow-x-auto my-6 border border-white/10 rounded-xl bg-black/20 shadow-lg"
          >
              ...
              <thead>
                <tr className="bg-white/5 text-cyan-300 border-b border-white/10">
                  {rows[0]
                    ...
                    .filter(Boolean)
                    .map((cell, j) => (
                      <th key={j} className="p-4 font-bold tracking-widest uppercase text-xs">
                        {cell.trim()}
                      </th>
                    ))}
                </tr>
                    ...
                    {row
                      ...
                      .map((cell, j) => {
                        const formattedCell = cell
                          ...
                          .replace(
                            /`(.*?)`/g,
                            '<code class="text-indigo-300 bg-indigo-500/10 px-1.5 py-0.5 rounded border border-indigo-500/20">$1</code>'
                          );
                        return (
                          <td
                            key={j}
                            className="p-4 text-slate-300"
                            dangerouslySetInnerHTML={{ __html: myXss.process(formattedCell) }}
                          />
                        );
                      })}
                  </tr>
      ...
      if (block.startsWith('- ')) {
        return (
          <ul
            key={i}
            className="list-disc list-inside space-y-2 my-4 text-slate-300 marker:text-cyan-500 leading-relaxed"
          >
            {block
              .split('\n')
              ...
              .map((item, j) => {
                ...
                return (
                  <li key={j} dangerouslySetInnerHTML={{ __html: myXss.process(formattedItem) }} />
                );
              })}
          </ul>
        );
      ...
      return (
        <p
          key={i}
          className="text-slate-300 leading-relaxed mb-4 text-[15px]"
          dangerouslySetInnerHTML={{ __html: myXss.process(formattedText) }}
        />
      );
    });
  };
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 overflow-hidden">
                  ...
                  />
                  <span
                    className={`text-sm truncate font-bold tracking-wide ${isActive ? 'text-cyan-300' : 'text-slate-400 group-hover:text-slate-200 transition-colors'}`}
                  >
                    {sec.title}
                  </span>
                </div>
            ...
            >
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-xs text-slate-500 font-mono uppercase tracking-widest">
                  OmniCore Architecture Registry
                </span>
              </div>
