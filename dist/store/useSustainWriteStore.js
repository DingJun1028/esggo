...
import { saveSustainWriteSection, loadSustainWriteSections } from '../lib/dataconnect-memory';
let autoSaveTimeout;
...
export const useSustainWriteStore = create()(persist((set, get) => ({
    companyId: 'default',
    sections: {},
    fieldValues: {},
    notes: {},
    ...
    initData: async (companyId) => {
        ...
        try {
            ...
            savedSections.forEach(s => {
                ...
                if (s.field_values)
                    fvMap[s.chapter_id] = s.field_values;
                if (s.notes)
                    notesMap[s.chapter_id] = s.notes;
                if (s.documents_state)
                    Object.assign(docMap, s.documents_state);
                if (s.status)
                    statusMap[s.chapter_id] = s.status;
                if (s.content)
                    contentMap[s.chapter_id] = s.content;
            });
            set({
                sections: sectionMap,
                fieldValues: fvMap,
    ...
    triggerAutoSave: (chapterId, chapterName, chapterOrder, griRefs) => {
        ...
        const draftPayload = {
            ...
            status: (chapterStatuses[chapterId] || 'draft'),
    ...
    expandContentWithAI: async (chapterId, chapterName, chapterOrder, griRefs, customPrompt) => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error('AI Expansion API Failed');
...
//# sourceMappingURL=useSustainWriteStore.js.map