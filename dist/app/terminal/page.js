...
import { TerminalSquare, Search, Plus, ShieldCheck, Activity, Brain, Lock, Loader2 } from 'lucide-react';
export default function TerminalPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const res = await fetch('/api/metrics/terminal', { cache: 'no-store' });
            if (res.ok) {
    ...
    const handleSeal = async (id) => {
        ...
        try {
            const response = await fetch('/api/vault/seal', {
                ...
                body: JSON.stringify({
                    evidence: { table: 'terminal', recordId: id, timestamp: Date.now() },
                    type: '5t-seal'
                })
            });
