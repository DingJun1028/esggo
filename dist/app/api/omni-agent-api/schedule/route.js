...
export async function POST(req) {
    try {
        ...
        const bodyMission = body ? body.mission : undefined;
        const searchParams = req.nextUrl.searchParams;
        const missionParam = searchParams.get('mission');
        const mission = bodyMission || missionParam;
        const context = body ? body.context : {};
        const cronSecret = body ? body.cronSecret : undefined;
        ...
        if (!mission || !VALID_MISSIONS.includes(mission)) {
...
${result.message || ''}`;
        ...
        return NextResponse.json({ error: errorMessage, scheduled: false }, { status: 500 });
    }
}
