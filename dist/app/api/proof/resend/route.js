...
export async function POST(req) {
    try {
        ...
        if (!transactionId || !email) {
            return NextResponse.json({ id: Date.now().toString(), status: 'error', content: 'Missing transactionId or email' }, { status: 400 });
        }
        ...
        return NextResponse.json({ id: Date.now().toString(), status: 'error', content: error.message }, { status: 500 });
    }
}
