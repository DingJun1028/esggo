...
export async function POST(request) {
    try {
        ...
        if (!documentId || !zkpProof) {
            return NextResponse.json({ error: 'Missing documentId or zkpProof in payload.' }, { status: 400 });
        }
        ...
        return NextResponse.json({ error: 'Failed to process ZKP payload.' }, { status: 500 });
    }
}
