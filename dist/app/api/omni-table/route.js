...
// =========================================================================
async function executeDatabaseQuery(action, tenant_id, recordData) {
    ...
    if (SUPABASE_SERVICE_KEY && SUPABASE_URL) {
        try {
            ...
            if (action === 'insert') {
                ...
                if (!response.ok)
                    throw new Error(`Supabase REST Insert Failed: ${response.statusText}`);
                ...
                if (!response.ok)
                    throw new Error(`Supabase REST Select Failed: ${response.statusText}`);
                return await response.json();
            }
        }
        catch (dbError) {
