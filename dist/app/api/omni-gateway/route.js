...
// 模擬資料庫查詢輔助
const mockDatabaseQuery = async (sql, params) => {
...
// =========================================================================
function generateZkpSeal(previousHash, data, source, timestamp) {
...
// =========================================================================
async function runBrowserbaseEsggoAutomation(targetUrl, actionType) {
...
export async function GET(req) {
    try {
            ...
            const stream = new ReadableStream({
                async start(controller) {
                    ...
                    const simulatedEvents = [
                        {
                            event_type: 'system:flow:optimized',
        ...
        // 標準資料庫 RLS 拉取模式
        // ==========================================
        await mockDatabaseQuery(`SELECT set_config('request.jwt.claims', $1, true)`, [JSON.stringify({ tenant_id: tenantIdFromToken })]);
...
export async function POST(req) {
    try {
        ...
        if (body.action === 'browserbase_automation') {
            ...
            const finalZkpHash = generateZkpSeal(mockPreviousHash, automationResult.extractedData, "Browserbase_browse_sh", timestamp);
      ...
      `;
        ...
        if (!parseResult.success) {
            return NextResponse.json({ error: '數據契約驗證失敗', details: parseResult.error.format() }, { status: 400 });
        }
        ...
        if (validData.tenant_id !== tenantIdFromToken) {
            return NextResponse.json({ error: 'Forbidden: RLS 邊界違規，禁止跨租戶注入數據' }, { status: 403 });
        }
        // 4. ZKP Seal 零知識證明雜湊鏈封印
        const mockPreviousHash = "0x8fa0715e2d1d2d50b98fa071588c94cf";
        const timestamp = Date.now();
        const finalZkpHash = generateZkpSeal(mockPreviousHash, validData.payload.metrics, validData.source_origin, timestamp);
        // 5. 注入 Postgres RLS Session 上下文，強制喚醒底層 RLS 安全 Policy 
        const dbTransaction = async () => {
            await mockDatabaseQuery(`SELECT set_config('request.jwt.claims', $1, true)`, [JSON.stringify({ tenant_id: validData.tenant_id })]);
            const sql = `
        INSERT INTO "AuditRecord" (tenant_id, event_type, payload, source_origin, last_modified_by, timestamp)
        VALUES ($1, $2, $3, $4, $5, $6)
      `;
        ...
        return NextResponse.json({ error: 'Internal Server Error', message: error instanceof Error ? error.message : '未知錯誤' }, { status: 500 });
    }
}
/**
