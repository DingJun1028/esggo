...
// ── Components ──────────────────────────────────────────────
export async function getBrandComponents(category, search) {
...
export async function upsertBrandComponent(component) {
    const supabase = getSupabaseClient();
    if (!supabase)
        return null;
...
export async function toggleFavoriteComponent(id, isFavorite) {
    const supabase = getSupabaseClient();
    if (!supabase)
        return false;
    try {
...
export async function deleteBrandComponent(id) {
    const supabase = getSupabaseClient();
    if (!supabase)
        return false;
    try {
...
// ── Tokens ──────────────────────────────────────────────────
export async function getBrandTokens(category) {
    const supabase = getSupabaseClient();
    if (!supabase)
        return [];
    try {
...
export async function upsertBrandToken(token) {
    const supabase = getSupabaseClient();
    if (!supabase)
        return null;
    try {
        const { data, error } = await supabase
            .from('brand_tokens')
            .upsert({ ...token, company_id: 'default', updated_at: new Date().toISOString() }, { onConflict: 'company_id,token_key' })
            .select()
            .single();
        if (error)
            throw error;
...
// ── Usage Logs ───────────────────────────────────────────────
export async function logComponentUsage(log) {
    const supabase = getSupabaseClient();
    if (!supabase)
        return;
    try {
...
export async function getBrandStats() {
    ...
    try {
        const [{ count: total }, { data: cats }, { count: favorites }, { count: tokens }] = await Promise.all([
...
//# sourceMappingURL=brand-db.js.map