...
export function policyGuard(input) {
    ...
    const highRiskTypes = ['compliance_review'];
...
export function createExecution(task) {
    return {
        id: genId('exec'),
        taskId: task.id,
...
export function buildPromptPolicy(task, dataScope) {
...
import { createHashLock, generatePedersenCommitment, verifyCommitmentSum, applyDataMasking } from '../crypto-proof';
import { updateArtifact, getArtifact } from './store';
import { addToKnowledgeBase } from './rag-engine';
/**
 * 提升草稿至信任層 (5T 實證封印)
 * 實現「深貫廣通」：從 Agent 產出無縫對接至 5T 誠信協議
 */
export async function promoteToTrustLayer(artifactId, actorId) {
    // 1. 獲取草稿內容
    const artifact = getArtifact(artifactId);
    if (!artifact)
        throw new Error('找不到指定的產出物資料');
 ...
 */
export async function triggerEffortlessVirtue(vibeSignal, currentContext) {
 ...
 * 執行蜂群任務 (具備自癒與鏈路能力)
 */
export async function executeSwarmTask(taskId, parentArtifactId) {
    ...
    const broadcast = (stage, node = 'Agent') => {
        try {
 ...
 */
export async function invokeHealingGuardian(sourceTaskId, failureReason) {
 ...
 */
export async function dispatchSwarmHandoff(sourceTaskId, targetSkillKey, reason) {
 ...
 * 評估是否需要自動委派 (智慧觸發器)
 */
export async function evaluateAutonomousDelegation(taskId, content) {
...
export function generateMockArtifact(task, execution) {
    const skill = getSkill(task.skillKey);
    const artifactType = (skill?.outputArtifactType ?? 'report_section_draft');
    const targetRoleMatch = task.description?.match(/\[Target Audience: (.*?)\]/);
    const targetRole = targetRoleMatch ? targetRoleMatch[1] : 'public';
    const contentMap = {
