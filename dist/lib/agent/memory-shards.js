 ...
 */
export async function extractMemoryShard(conversationLog) {
    const prompt = `
 ...
 */
export async function synthesizeSkillUltimate(shards) {
...
// 核心機制：將記憶碎片存入資料庫
export async function storeMemoryShard(shard) {
    // 只包含資料庫中存在的欄位
    const insertData = {
        id: shard.id,
        title: shard.title,
...
// 核心機制：將技能奧義存入資料庫
export async function storeSkillUltimate(ultimate) {
    // 只包含資料庫中存在的欄位
    const insertData = {
        skill_name: ultimate.skillName,
        mastery_level: ultimate.masteryLevel,
...
//# sourceMappingURL=memory-shards.js.map