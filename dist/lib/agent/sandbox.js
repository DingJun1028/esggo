 ...
 */
export async function runInSandbox(code, options = { language: 'python' }) {
 ...
 */
async function runProductionE2B(code, options) {
 ...
 */
async function runSimulatedSandbox(code, options) {
    ...
    if (options.timeoutMs) {
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error(`Sandbox execution timed out after ${options.timeoutMs}ms`)), options.timeoutMs));
