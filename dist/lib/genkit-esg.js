...
export async function runGRIContentFlow(input) {
...
輸出格式（JSON）：{"content": "...", "indicators": []}`;
        ...
        fullContent += `\n\n## ${section.title}\n\n${secData.content}`;
        if (secData.indicators)
            allIndicators.push(...secData.indicators);
...
export const EXPERT_SACRED_TEMPLATES = {
...
export async function runESGAnalysisFlow(input) {
...
輸出格式：{"summary":"...","compliance":{"score":0,"gaps":[],"recommendations":[]},"greenwashingRisks":[],"griAlignment":[],"formula":"..."}`;
    ...
    const jsonMatch = rawResponse.match(/\{[\s\S]*\}/);
    const result = JSON.parse(jsonMatch?.[0] ?? '{"summary":""}');
...
export async function scanGreenwashing(text) {
    const prompt = `請掃描綠漂風險：\n${text}\n輸出 JSON 格式：{"risks":[],"overallRisk":"low"}`;
    const rawResponse = await callGemini(prompt);
    const jsonMatch = rawResponse.match(/\{[\s\S]*\}/);
    const result = JSON.parse(jsonMatch?.[0] ?? '{"risks":[]}');
...
export async function runOCRAnalysisFlow(input) {
    const prompt = `你是一個專業的 OCR 與文件分析模型。請分析這份 ${input.documentType} 文件。\n圖片連結: ${input.imageUrl}\n請提取文字並將關鍵數據結構化。\n輸出 JSON 格式：{"extractedText":"...","structuredData":{},"confidenceScore":95}`;
    const rawResponse = await callGemini(prompt);
    const jsonMatch = rawResponse.match(/\{[\s\S]*\}/);
    const result = JSON.parse(jsonMatch?.[0] ?? '{"extractedText":"","structuredData":{},"confidenceScore":0}');
    const hashLock = computeHashLock({ input, result });
    return { ...result, hashLock };
}
export async function runEvidenceValidationFlow(evidenceText, griReference) {
    const prompt = `作為審計專家，請自動驗證以下證據是否符合 GRI 標準 ${griReference}。\n證據內容：\n${evidenceText}\n輸出 JSON 格式：{"isValid":true,"validationDetails":"..."}`;
    const rawResponse = await callGemini(prompt);
    const jsonMatch = rawResponse.match(/\{[\s\S]*\}/);
    const result = JSON.parse(jsonMatch?.[0] ?? '{"isValid":false,"validationDetails":"Validation failed."}');
    const hashLock = computeHashLock({ evidenceText, griReference, result });
    return { ...result, hashLock };
}
//# sourceMappingURL=genkit-esg.js.map