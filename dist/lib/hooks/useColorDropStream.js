...
// =========================================================================
export function useColorDropStream(token = 'taiwan-jwt-token') {
    ...
    const getVisualState = useCallback((eventType) => {
        switch (eventType) {
    ...
    const triggerForensicReplay = useCallback(async (speed = 2) => {
        try {
            ...
            if (!response.ok)
                throw new Error('Replay trigger failed');
    ...
    useEffect(() => {
        ...
        eventSource.addEventListener('state:hydration', (e) => {
            const data = JSON.parse(e.data);
        ...
        eventSource.addEventListener('color:drop:live', (e) => {
            ...
            setEvents(prev => {
                // 防止重複的歷史回水
                if (prev.some(evt => evt.id === newRecord.id))
                    return prev;
    ...
    const metrics = useMemo(() => {
        ...
        const verifiedCount = events.filter(e => e.event_type === 'color:drop:verified' ||
            e.event_type === 'vault:seal:medical_zkp_ready').length;
...
//# sourceMappingURL=useColorDropStream.js.map