 ...
 */
export async function push5TReport(payload) {
    const channel = payload.channel || SLACK_HOME_CHANNEL;
    if (!channel)
        throw new Error('[SlackGateway] 未指定推播頻道，請設定 SLACK_HOME_CHANNEL。');
 ...
 * 推播代理蜂群任務執行摘要
 */
export async function pushAgentSummary(payload) {
    const channel = payload.channel || SLACK_HOME_CHANNEL;
    if (!channel)
        throw new Error('[SlackGateway] 未指定推播頻道');
 ...
 * 驗證使用者是否在允許名單中
 */
export function isSlackUserAllowed(userId) {
    if (SLACK_ALLOWED_USERS.length === 0)
        return false; // 預設拒絕
