export class ESGDataService {
    ...
    async createContract(input) {
        ...
        if (error)
            throw error;
        // Sync to OmniTable
        if (this.omniTableClient && typeof this.omniTableClient.createRecords === 'function') {
            await this.omniTableClient.createRecords('esg_contracts', [contract]);
        }
        return data;
    }
    async getContracts() {
    ...
    async addAgentStep(taskId, step) {
        ...
        // Log to OmniTable
        if (this.omniTableClient && typeof this.omniTableClient.createRecords === 'function') {
            await this.omniTableClient.createRecords('esg_agent_logs', [{
                    taskId,
    ...
    async createEvidenceRecord(input) {
        ...
        // Sync to OmniTable
        if (this.omniTableClient && typeof this.omniTableClient.createRecords === 'function') {
            await this.omniTableClient.createRecords('esg_evidence', [evidence]);
        }
        return data;
    }
    ...
    async getLatestAgentTasks(limit = 10) {
        const { data, error } = await this.supabase
            .from('agent_tasks')
            .select('*')
    ...
    async searchByCompliance(framework) {
        const { data, error } = await this.supabase
            .from('agent_tasks')
            .select('*')
