...
export const resolvers = {
    Query: {
        ...
        esgMetrics: async (_, { category, year }) => {
            try {
                const supabase = getSupabaseClient();
                if (!supabase)
                    return [];
        ...
        evidenceFiles: async (_, { status, category }) => {
            try {
                const supabase = getSupabaseClient();
                if (!supabase)
                    return [];
        ...
        auditLogs: async (_, { limit = 50, offset = 0 }) => {
            try {
                const supabase = getSupabaseClient();
                if (!supabase)
                    return [];
        ...
        griDisclosures: async (_, { status }) => {
            ...
            if (status)
                return disclosures.filter((d) => d.status === status);
            return disclosures;
        },
        advisorySessions: async (_, { userId = 'default' }) => {
            try {
                const supabase = getSupabaseClient();
                if (!supabase)
                    return [];
        ...
        environmentalData: async (_, { category, year }) => {
            try {
                const supabase = getSupabaseClient();
                if (!supabase)
                    return [];
        ...
        roadmapMilestones: async (_, { status }) => {
            try {
                const supabase = getSupabaseClient();
                if (!supabase)
                    return [];
    ...
    Mutation: {
        ...
        sealEvidence: async (_, { id, sealType }) => {
            const supabase = getSupabaseClient();
            if (!supabase)
                throw new Error('Database not available');
        ...
        verifyEvidence: async (_, { id, hashLock }) => {
            const supabase = getSupabaseClient();
            if (!supabase) {
        ...
        updateCompanyProfile: async (_, { input }) => {
            const supabase = getSupabaseClient();
            if (!supabase)
                throw new Error('Database not available');
            const payload = {
