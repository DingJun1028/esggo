import { EternalMemoryType, } from '../src/shared/types/index.ts';
import { integrityModule } from './omni-core/integrity.ts';
import { sha256, generatePedersenCommitment, verifyCommitmentSum, } from './crypto-proof.ts';
...
export class OmniCore {
    ...
    validateT5Gate(evidence) {
        const tangible = Boolean((evidence.finalEffect && evidence.finalEffect.length > 0) ||
            (evidence.tangible_metric && evidence.tangible_metric.length > 0));
        const traceable = Boolean((evidence.originCause && (evidence.originCause.startsWith('/') || evidence.originCause === 'Manual Execution')) ||
            (evidence.source_origin && (evidence.source_origin.startsWith('/') || evidence.source_origin === 'Manual Execution')));
        const trackable = Boolean((evidence.processTrace && evidence.processTrace.length > 0) ||
            (evidence.lifecycle_hooks && evidence.lifecycle_hooks.length > 0));
        const transparent = Boolean(evidence.formula_ref &&
    ...
    async sealComponent(metric, source, formula, trigger = 'Manual', policyId) {
        let validation;
        if (policyId) {
     ...
     * 當偵測到數據偏差或亂碼時觸發
     */
    async restoreComponent(faultyData) {
    ...
    async storeMemory(content, type = EternalMemoryType.EPISODIC) {
    ...
    async consolidateMemories(type) {
        const memories = await this.getMemories();
    ...
    async generatePrivacyProof(metric, value, min, max, blindingFactorOverride) {
        ...
        const { proof, publicSignals } = await import('./crypto-proof.ts').then(m => m.generateSnarkJSRangeProof(value, min, max, commitment.blindingFactor));
        return {
            commitment,
            min,
            max,
    ...
    async storeZKPProof(proof, metricName, dataType = "ESG Metric", source = "OmniCore") {
