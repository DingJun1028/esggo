 ...
 */
export async function generatePedersenCommitment(value) {
    // r: 32 bytes 隨機盲化因子
 ...
 */
export async function verifyPedersenCommitment(commitment, value, blindingFactor) {
    try {
        const vG = G_POINT.multiply(BigInt(value));
 ...
 */
export function verifyCommitmentSum(commitments, totalCommitment) {
    try {
        // C_total = C1 + C2 + C3 ...
...
export async function createHashLock(data) {
...
export async function verifyHashLock(data, originalHash, originalTimestamp) {
    const content = JSON.stringify(data) + originalTimestamp;
    const newHash = createHash('sha256').update(content).digest('hex');
    return newHash === originalHash;
}
export async function create5TAttestation(artifactId) {
 ...
 */
export async function generateSnarkJSRangeProof(value, min, max, blindingFactor) {
 ...
 */
export async function verifySnarkJSProof(vKey, publicSignals, proof) {
    try {
 ...
 */
export function applyDataMasking(value, level, options) {
 ...
 */
export function unmaskL2Data(maskedValue, secretKey) {
...
export async function verifyZKPProof(proof, blindingFactor) {
