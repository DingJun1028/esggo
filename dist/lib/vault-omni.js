 ...
 */
export function computeHashLock(data) {
    // Deterministic JSON stringify to ensure consistent hashing
    const str = typeof data === 'string' ? data : JSON.stringify(data, (key, value) => value instanceof Object && !Array.isArray(value)
 ...
 */
export function flattenToRecord(component, dimension = 'CORE') {
 ...
 */
export async function readFromVault(uuid) {
 ...
 */
export async function readFromVaultByHashLock(hashLock) {
    const supabase = getServiceClient();
    if (!supabase)
        return null;
    const { data, error } = await supabase
 ...
 */
export async function verifyRecord(uuid) {
 ...
 */
export async function engraveToSingleTable(component) {
    const supabase = getServiceClient();
    if (!supabase)
        return { success: true, id: component.uuid };
...
//# sourceMappingURL=vault-omni.js.map