...
class CollaborativeSwarmAgent {
    constructor(agentConfig) {
        this.memorySummary = 'Initial swarm memory summary';
    ...
    async handleEvent(event, payload) {
        ...
        if (event === 'HEALING_PROTOCOL' && this.agentConfig.name === 'Agent0') {
            ...
            await HealingGuardian.executeUniversalHealing(payload.targetId, payload.table);
        }
...
export class CollaborativeADKSwarm {
    ...
    swarmNegotiate(results) {
        ...
        const successfulEntries = Object.fromEntries(Object.entries(results).filter(([_, val]) => val.success && !val.error));
...
//# sourceMappingURL=adk-swarm.js.map