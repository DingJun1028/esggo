...
const GlobalIntegration_1 = require("./GlobalIntegration");
...
async function main() {
    ...
    try {
        const isValid = await T5Protocol_1.T5Validator.validate(testEvidence);
    ...
    try {
        const evidence = await GlobalIntegration_1.GlobalEvidenceService.createEvidenceRecord({
            uuid: 'test-uuid-123',
            version: '1.0.0',
    ...
    try {
        const contract = await GlobalIntegration_1.ContractService.createContract({
    ...
    try {
        const manager = GlobalIntegration_1.GlobalStateManager.getInstance();
