...
const GlobalHealingServer_1 = require("./GlobalHealingServer");
(0, vitest_1.describe)('GlobalHealingService 整合與自癒測試集', () => {
    let service;
    (0, vitest_1.beforeEach)(() => {
        // 每次測試初始化全新服務與事件庫，確保測試完全隔離
        service = new GlobalHealingServer_1.GlobalHealingService();
    });
    (0, vitest_1.test)('場景一：healCard 動態建立內置記憶體節點並執行自癒', async () => {
        ...
        (0, vitest_1.expect)(laggingCard.name).toBe('碳減排核心指標 (真理版)');
        (0, vitest_1.expect)(laggingCard.status).toBe('done');
        (0, vitest_1.expect)(laggingCard.attributes).toEqual(['E', 'Carbon']);
        (0, vitest_1.expect)(laggingCard.abilities).toEqual(['Calculation', 'Reporting']);
        // 5. 驗證 GPL 事件流中是否正確寫入自癒成功紀錄
        const events = service.getEventStore().getEvents(cardUuid);
        (0, vitest_1.expect)(events.length).toBe(2);
        (0, vitest_1.expect)(events[1].event_type).toBe('HEALING_APPLIED');
    });
    (0, vitest_1.test)('場景二：手動註冊異質外部節點並透過 healAll 撫平差異', async () => {
        ...
        (0, vitest_1.expect)(result.status).toBe('SUCCESS');
        (0, vitest_1.expect)(result.reconciledCount).toBe(1);
        (0, vitest_1.expect)(mockExternalPayload.name).toBe('ISO-14064 碳中和確證書');
        (0, vitest_1.expect)(mockExternalPayload.status).toBe('done');
        (0, vitest_1.expect)(mockExternalPayload.attributes).toContain('Verified');
    });
    (0, vitest_1.test)('場景三：LV1_MONITOR 等級下，偵測偏離但僅作紀錄、不執行覆寫', async () => {
        const monitorService = new GlobalHealingServer_1.GlobalHealingService(undefined, 'LV1_MONITOR');
        ...
        (0, vitest_1.expect)(laggingCard.status).toBe('todo'); // 依舊是 todo
        // GPL 應寫入 HEALING_LOGGED 事件
        const events = monitorService.getEventStore().getEvents(cardUuid);
        (0, vitest_1.expect)(events.some(ev => ev.event_type === 'HEALING_LOGGED')).toBe(true);
    });
});
//# sourceMappingURL=GlobalHealingServer.test.js.map