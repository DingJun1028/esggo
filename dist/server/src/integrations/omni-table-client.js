 ...
 */
function generateHashLock(node) {
...
export async function syncLogicNodesToOmniTable(nodes) {
    try {
        const spaceId = process.env.OMNITABLE_SPACE_ID;
        // In OmniTable Fusion API, we can search for nodes by name
        const existingNodes = await withRetry(() => omniTable.searchNodes(spaceId, 'Logic Nodes'));
        ...
        if (!datasheetId) {
            const created = await withRetry(() => omniTable.createDatasheet(spaceId, {
                name: 'Logic Nodes',
                fields: [
                    { name: '節點名稱', type: 'text' },
        ...
        const existingRecords = await withRetry(() => omniTable.getRecords(datasheetId));
        if (existingRecords.records && existingRecords.records.length > 0) {
            await withRetry(() => omniTable.deleteRecords(datasheetId, existingRecords.records.map((r) => r.recordId)));
        }
        // Chunk array to avoid API limits (e.g., OmniTable batch limit is usually 10)
        const chunkSize = 10;
        for (let i = 0; i < records.length; i += chunkSize) {
            const chunk = records.slice(i, i + chunkSize);
            await withRetry(() => omniTable.createRecords(datasheetId, chunk));
        }
        return true;
    }
    catch (error) {
