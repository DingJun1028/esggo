...
const OmniAgentBus_1 = require("./services/OmniAgentBus");
...
async function simulateOmniAgentTask(userQuery, rawESGData) {
    console.log(`
OmniAgent: Received query - "${userQuery}"`);
    ...
    try {
        ...
        if (userQuery.includes("analyze ESG data")) {
            ...
            const analysisResult = await OmniAgentBus_1.omniAgentBus.invokeSkillAction('esg-data-advisor', 'analyzeESGData', analysisInput, currentTaskContext);
            ...
            const complianceResult = await OmniAgentBus_1.omniAgentBus.invokeSkillAction('esg-data-advisor', 'checkGRICompliance', complianceInput, currentTaskContext);
            ...
            const promptResult = await OmniAgentBus_1.omniAgentBus.invokeSkillAction('gemma-dev', 'generatePrompt', promptInput, currentTaskContext);
...
(async () => {
    ...
    await simulateOmniAgentTask("Please analyze ESG data for our latest report.", '{"emissions": 500, "water_usage": 1200, "diversity_score": 0.75}');
    await simulateOmniAgentTask("Can you check GRI compliance for carbon emissions under GRI 305-1?", "Our company reported 500 tons of CO2 emissions, following GRI 305-1 guidelines.");
    await simulateOmniAgentTask("Please generate a prompt for Gemma to summarize ESG risks.");
    await simulateOmniAgentTask("What is the weather today?"); // No matching skill
    console.log("--- OmniAgent Simulation Finished ---");
})();
//# sourceMappingURL=omniagent-runtime.js.map