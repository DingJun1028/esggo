...
exports.extendedValidators = exports.ALLOWED_MIME_TYPES = void 0;
...
async function validateContent(content, contentType, options = {}) {
    const errors = [];
    const warnings = [];
...
function getMimeTypeCategory(mimeType) {
    for (const [category, types] of Object.entries(exports.ALLOWED_MIME_TYPES)) {
...
// Zod 擴展驗證器
// ============================================
exports.extendedValidators = {
    contentHash: zod_1.z.string().regex(/^[a-f0-9]{64}$/i, 'Invalid SHA-256 hash'),
    ...
    mimeType: zod_1.z.string().regex(/^[a-z]+\/[a-z0-9\-\+\.]+$/i, 'Invalid MIME type format'),
    base64: zod_1.z.string().regex(/^[A-Za-z0-9+/]*={0,2}$/, 'Invalid Base64 format'),
    ...
    confidence: zod_1.z.number().min(0).max(1, 'Confidence score must be between 0 and 1'),
};
async function validateBatch(items, validator) {
    const valid = [];
    const invalid = [];
