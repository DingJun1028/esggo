 ...
 */
export const CausalTopologyGraph = ({ taskId = 'task_omni_01', executionId, agentStatus, zkpStatus, vaultStatus, healingStatus }) => {
    ...
    return (_jsxs("div", { className: "relative w-full p-8 rounded-2xl bg-slate-900/80 backdrop-blur-xl border border-white/10 overflow-hidden", children: [_jsx("div", { className: "absolute top-[-50%] left-[-50%] w-[200%] h-[200%] bg-[radial-gradient(circle_at_50%_50%,rgba(56,189,248,0.1),transparent_50%)] animate-spin-slow pointer-events-none" }), _jsxs("h3", { className: "text-xl font-bold text-white mb-8 relative z-10 tracking-wider", children: ["\uD83C\uDF0C \u56E0\u679C\u5F8B\u52D5\u614B\u93C8\u8DEF (Causal Link) ", _jsxs("span", { className: "text-xs font-mono text-cyan-400 bg-cyan-900/40 px-2 py-1 rounded ml-2", children: ["Task: ", taskId] })] }), _jsx("div", { className: "flex flex-col md:flex-row items-center justify-between gap-6 relative z-10", children: nodes.map((node, index) => (_jsxs(React.Fragment, { children: [_jsxs("div", { className: `flex flex-col items-center justify-center p-6 w-64 rounded-xl border backdrop-blur-md transition-all duration-700 ease-out ${getStatusColor(node.status)}`, children: [_jsx("span", { className: "text-xs font-mono font-semibold text-gray-400 uppercase tracking-widest mb-2", children: node.phase === 'cause' ? '因 (Cause)' : node.phase === 'process' ? '循 (Process)' : node.phase === 'heal' ? '修 (Heal)' : '果 (Effect)' }), _jsx("h4", { className: "text-lg font-bold text-gray-100 text-center", children: node.title }), _jsx("p", { className: "text-sm text-gray-400 text-center mt-2", children: node.subtitle })] }), index < nodes.length - 1 && (_jsx("div", { className: "flex-1 w-full md:w-auto h-8 md:h-1 flex items-center justify-center", children: _jsxs("div", { className: "relative h-full w-1 md:w-full md:h-1 rounded-full bg-gray-700/50 overflow-hidden", children: [_jsx("div", { className: `absolute inset-0 transition-opacity duration-1000 ${nodes[index].status === 'success' ? 'opacity-100 bg-gradient-to-r from-emerald-400 to-cyan-400 shadow-[0_0_10px_rgba(56,189,248,0.8)]' : 'opacity-0'}` }), _jsx("div", { className: `absolute inset-0 transition-opacity duration-1000 ${nodes[index].status === 'processing' ? 'opacity-100 bg-gradient-to-r from-blue-500/20 to-blue-400/80 animate-pulse' : 'opacity-0'}` })] }) }))] }, node.id))) })] }));
};
export default CausalTopologyGraph;
//# sourceMappingURL=CausalTopologyGraph.js.map