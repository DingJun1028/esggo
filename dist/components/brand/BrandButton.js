...
export default function BrandButton({ children, variant = 'primary', size = 'md', loading = false, icon, iconRight, fullWidth = false, className = '', disabled, hoverBgClassName, // Destructure new prop
...props }) {
    const finalHoverBgClass = hoverBgClassName || (variant === 'ghost' ? 'hover:bg-[#003262]/5' : ''); // Fallback for ghost if not provided
    return (_jsxs("button", { className: cn('relative inline-flex items-center justify-center gap-3 rounded-2xl font-black uppercase overflow-hidden', 'transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-blue-500/20', 'disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer active:scale-95', variantStyles[variant], sizeStyles[size], finalHoverBgClass, // Apply custom hoverBgClass or fallback
        fullWidth && 'w-full', className), disabled: disabled || loading, ...props, children: [_jsxs("div", { className: "relative z-10 flex items-center gap-3", children: [loading ? _jsx(Loader2, { size: 18, className: "animate-spin" }) : icon, children, !loading && iconRight] }), _jsx("div", { className: "absolute inset-0 bg-gradient-to-tr from-white/0 via-white/5 to-white/0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" })] }));
}
//# sourceMappingURL=BrandButton.js.map