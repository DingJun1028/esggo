...
Write-Host "======================================================================" -ForegroundColor Cyan
Write-Host "本腳本將在本地 Windows 電腦自動產生高安全度的 ED25519 金鑰對，" -ForegroundColor SlateGray
Write-Host "配置 SSH 快速連線捷徑，並提供 VPS 端誠信防禦與權限加固之整合指引。" -ForegroundColor SlateGray
Write-Host ""
...
if (-not (Test-Path $keyPath)) {
    ...
    # 執行 ssh-keygen 產生金鑰對
    ssh-keygen -t ed25519 -f $keyPath -N "" -C "esggo-vps-key"
...
if (Test-Path $configPath) {
    ...
    if ($existingConfig -match "Host esggo-vps") {
        $hasConfig = $true
    }
}
...
Write-Host "請登入 VPS 後，直接在遠端終端機執行下方一鍵貼上指令（將自動建立授權檔案並鎖定權限）：" -ForegroundColor SlateGray
Write-Host ""
...
Write-Host "======================================================================" -ForegroundColor Cyan
Write-Host "當您測試 'ssh esggo-vps' 免密碼成功連線後，建議執行以下安全防衛設定：" -ForegroundColor SlateGray
Write-Host ""
Write-Host "1. 編輯 VPS 上的 SSH 設定檔：" -ForegroundColor SlateGray
Write-Host "   sudo nano /etc/ssh/sshd_config" -ForegroundColor Cyan
Write-Host ""
Write-Host "2. 修改或確認以下參數（禁用密碼登入與允許金鑰登入）：" -ForegroundColor SlateGray
...
Write-Host ""
Write-Host "3. 重新載入遠端 SSH 服務生效：" -ForegroundColor SlateGray
