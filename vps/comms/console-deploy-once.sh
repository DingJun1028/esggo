...
echo "=== STEP 4 firewall ==="; ufw allow 22/tcp || true; ufw allow 80/tcp || true; ufw allow 443/tcp || true; ufw allow 8042/tcp || true; ufw allow 9999/tcp || true; ufw reload || true; ufw status verbose || true
echo "=== STEP 5 sshd ==="; systemctl restart sshd || true; ss -ltnp | grep ':22' || true; journalctl -u sshd -n 50 --no-pager || true
echo "=== STEP 6 local services ==="; (curl -s --max-time 3 http://127.0.0.1:8042/health || echo "health: unavailable") | sed 's/^/[gateway] /'; (ss -ltnp | grep ':22' >/dev/null 2>&1 && echo "ssh: listening" || echo "ssh: not listening") | sed 's/^/[ssh] /'
