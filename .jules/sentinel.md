...
## 2026-08-31 - [Prevent Stack Trace Leakage in Error Boundaries]
**Vulnerability:** The error boundaries in `src/App.jsx` and `src/main.jsx` unconditionally rendered the stack trace (`error?.stack`) to the UI, which leaks internal system details to end users in production.
**Learning:** Exposing stack traces in error messages provides attackers with valuable insights into the application's internal structure, dependencies, and potential vulnerabilities. Error boundaries should gracefully handle errors without exposing sensitive information.
**Prevention:** Ensure that stack traces and other internal error details are only logged securely (e.g., to the console or an error tracking service) and never rendered directly to the client interface in production environments.
