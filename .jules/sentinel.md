...
## 2026-07-07 - Fix hardcoded relay token in VPS scripts
**Vulnerability:** A hardcoded authentication token (`esggo-relay-20260707`) was found embedded in 12 different deployment and relay communication scripts (`.sh`, `.ps1`, `.bat`) within the `vps/comms/` and `vps/recovery/` directories. This is a critical security risk as it exposes the authorization key for the VPS relay endpoints to anyone who has read access to the repository.

**Learning:** This likely existed because the token was initially added for convenience during early development or debugging phases for the agent scripts and was forgotten when the scripts were committed to version control. The scripts were heavily duplicated and variations of the deploy scripts meant this vulnerability spread across many files.

**Prevention:** Always use environment variables to supply sensitive tokens, API keys, and passwords to scripts during deployment instead of hardcoding them into source files. Enforce pre-commit hooks to scan for hardcoded credentials. Use `$env:VAR` for PowerShell, `${VAR}` for Bash, and `%VAR%` for Batch scripts.
