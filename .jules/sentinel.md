...
## 2026-07-03 - [Command Injection via execSync with curl]
**Vulnerability:** In `src/impl/core.ts`, user input was serialized using `JSON.stringify()` and directly interpolated into a `curl` shell command executed via `child_process.execSync`. Since `JSON.stringify` does not escape single quotes (`'`), a malicious user could craft a payload with a single quote to break out of the string and execute arbitrary shell commands (Remote Code Execution).
**Learning:** Shell command execution tools like `execSync` are inherently dangerous when interacting with user data, even if the data appears structured (like JSON). Escaping data correctly for shell consumption is difficult and error-prone.
**Prevention:** Avoid shell commands for network requests entirely. Use native, secure APIs like `fetch` or well-maintained HTTP client libraries (e.g., `axios`), which securely handle data serialization and transmission without involving the shell.
