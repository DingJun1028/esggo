...
## 2024-08-01 - [Missing Accessibility in Custom Tab Components]
**Learning:** Found a pattern where custom Tab components (like `Tabs.tsx`) lack the proper structural ARIA roles (`role="tablist"` and `role="tab"`) and `aria-selected` state tracking. Additionally, these custom interactive elements were missing explicit keyboard focus states, hindering keyboard navigation.
**Action:** Always ensure custom tab patterns wrap their container in `role="tablist"`, apply `role="tab"` and `aria-selected` to individual tab buttons, and include explicit `focus-visible:ring-2` styles for accessible keyboard navigation.
