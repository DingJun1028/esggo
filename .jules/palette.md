...
## 2026-08-17 - [Dynamic Form Accessibility]
**Learning:** Dynamic form generators often loop through schema fields to create inputs and labels, but fail to programmatically link them. Without `htmlFor` on the label and a matching `id` on the input, screen readers cannot properly announce the input field's purpose. Additionally, error messages need `role="alert"` and inputs need `aria-invalid` and `aria-describedby` to communicate validation states effectively to assistive technologies.
**Action:** When auditing or modifying dynamic forms, always ensure generated inputs have unique `id`s tied to their respective `<label htmlFor="...">`. Also implement `aria-invalid`, `aria-describedby`, and `role="alert"` for robust error reporting, and use `aria-hidden="true"` on decorative icons inside error messages.
