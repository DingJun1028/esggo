...
## 2025-02-09 - OmniTable Array Filter Optimization
**Learning:** In `OmniTable.tsx`, executing `.filter` on a potentially large array of 5T registered data directly in the render function block forces the application to re-evaluate the array with complex `toLowerCase().includes()` comparisons for every state change (e.g. expanding a trace detail).
**Action:** Wrap such calculations in `useMemo` hooks and explicitly list filter inputs (data, search value) as dependencies to prevent O(N) recalculations on every React render cycle. Hoist operations like `filter.toLowerCase()` outside the filtering loop.
