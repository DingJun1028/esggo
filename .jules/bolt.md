...
## 2024-06-16 - Prevent filtering recalculation on row expansion
**Learning:** In components with tabular data like `OmniTable.tsx`, where row interactions (like expansion or selection) share the same component state as search/filtering logic, filtering the array directly during render causes unnecessary `O(N)` string operations on every unrelated interaction.
**Action:** Always wrap data filtering logic in `useMemo` with only the raw data and filter inputs as dependencies, to decouple it from local UI state changes.
