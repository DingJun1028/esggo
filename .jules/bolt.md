...
## 2025-02-09 - OmniCoreShell NavGroups Filter Optimization
**Learning:** In `OmniCoreShell.tsx`, filtering `NAV_GROUPS` dynamically using `.map` and inner `.filter` loops with inline `toLowerCase()` strings blocks the main thread during navigation state updates and layout renders on large sets.
**Action:** Always wrap data-intensive derivations (filtering/sorting) in `useMemo` so that they only recalculate when dependencies (e.g. `searchQuery`) change, and hoist string operations like `toLowerCase()` outside nested loops.
