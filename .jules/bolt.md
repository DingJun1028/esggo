...
## 2024-06-18 - UserList Array Filter Optimization
**Learning:** In `UserList.js`, performing a text search filter inside the render cycle by evaluating `searchTerm.toLowerCase()` for every single user `user.name.toLowerCase().includes(searchTerm.toLowerCase())` scales poorly. This leads to O(N*M) redundant string operations (re-evaluating the search term case transformation per list item) every time the input value changes or the parent component re-renders.
**Action:** Always wrap data filtering lists in `useMemo` hooks with strict dependencies (`[initialUsers, searchTerm]`), and extract invariant derivations like `searchTerm.toLowerCase()` *outside* the array traversal loop. This avoids redundant operations on every iteration.
