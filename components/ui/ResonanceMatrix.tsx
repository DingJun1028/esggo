...
export function ResonanceMatrix({ results }: Props) {
  return (
    <div className="space-y-6">
        ...
        {results.map((r) => {
          ...
          return (
            <BrandCard
              key={r.topicId}
              padding="md"
              className="border-slate-100 shadow-sm transition-all hover:border-blue-200 group"
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h4 className="text-xs font-black text-berkeley-blue uppercase tracking-tight">
                    {r.label}
                  </h4>
      ...
      <BrandCard
        padding="lg"
        className="bg-gradient-to-br from-slate-900 to-berkeley-blue text-white border-none shadow-premium overflow-hidden relative"
      >
        <div className="relative z-10 space-y-6">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="text-sm font-black uppercase tracking-[0.2em] text-primary-300">
                共鳴矩陣分析 (Resonance Map)
              </h3>
              <p className="text-xs text-primary-100 opacity-60">
                Visualizing Strategy vs. Expectations
              </p>
            </div>
            <div className="text-right">
              ...
              <p className="text-2xl font-black font-mono">
                {Math.round(
                  results.reduce((acc, r) => acc + r.resonance, 0) / (results.length || 1)
                )}
                %
              </p>
            </div>
          ...
          <div className="aspect-video rounded-3xl border border-white/10 relative p-8">
            ...
            <span className="absolute left-10 bottom-2 text-[8px] font-black text-white/30 uppercase">
              內部優先級 (Internal Priority)
            </span>
            <span className="absolute left-2 top-10 -rotate-90 text-[8px] font-black text-white/30 uppercase origin-left">
              外部期望 (External Exp.)
            </span>
