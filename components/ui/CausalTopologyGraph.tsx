...
export const CausalTopologyGraph: React.FC<CausalTopologyGraphProps> = ({
  ...
  const getStatusColor = (status: NodeStatus) => {
    switch (status) {
      ...
      case 'failed':
        return 'border-rose-400 bg-rose-500/20 shadow-[0_0_20px_rgba(244,63,94,0.6)] scale-[0.98] ring-2 ring-rose-500/50';
      default:
        return 'border-gray-500/50 bg-white/5 opacity-80';
    }
  };
  ...
  return (
    <div className="relative w-full p-8 rounded-xl bg-slate-900/80 border border-white/10 overflow-hidden">
      ...
      <h3 className="text-xl font-bold text-white mb-8 relative z-10 tracking-wider">
        🌌 因果律動態鏈路 (Causal Link){' '}
        <span className="text-xs font-mono text-cyan-400 bg-cyan-900/40 px-2 py-1 rounded ml-2">
          Task: {taskId}
        </span>
      </h3>
                  ...
                  <div
                    className={`absolute inset-0 transition-opacity duration-1000 ${
                      nodes[index].status === 'success'
                        ? 'opacity-100 bg-gradient-to-r from-emerald-400 to-cyan-400 shadow-[0_0_10px_rgba(56,189,248,0.8)]'
                        : 'opacity-0'
                    }`}
                  />
                  <div
