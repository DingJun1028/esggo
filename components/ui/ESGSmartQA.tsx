...
export const ESGSmartQA = () => {
  ...
  const [messages, setMessages] = useState<
    { id: string; role: 'user' | 'bot'; text: string; sources?: RagSource[] }[]
  >([]);
    ...
    try {
      ...
      const interval = setInterval(() => {
        ...
        if (i >= fullText.length) {
          ...
          dispatchBus(
            'MANIFEST',
            'ESGSmartQA',
            'RAG response stream completed and UI manifest achieved.'
          );
        }
      }, 20);
    } catch (e) {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: 'bot',
          text: '系統發生震盪，無法連線至 OmniAgent 知識庫。',
        },
      ]);
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: Date.now().toString(), role: 'bot', text: `正在解析文獻：${file.name}...` },
    ]);
      ...
      if (data.success) {
        setMessages((prev) => [
          ...prev,
          {
            id: Date.now().toString(),
            role: 'bot',
            text: `✅ 檔案 \`${data.fileName}\` 解析成功！已將文本切割為 ${data.chunksCount} 個向量區塊 (Chunks) 並寫入 OmniVector Store。現在您可以針對該文件進行問答。`,
          },
        ]);
      } else {
        setMessages((prev) => [
          ...prev,
          { id: Date.now().toString(), role: 'bot', text: `❌ 檔案上傳失敗：${data.error}` },
        ]);
      }
    } catch (err) {
      setMessages((prev) => [
        ...prev,
        { id: Date.now().toString(), role: 'bot', text: '系統發生震盪，無法上傳文獻。' },
      ]);
    } finally {
                ...
                ${messages
                  ...
                  .map(
                    ...
                    <div class="message ${msg.role}">
                        <div class="role">${
                          msg.role === 'user' ? '👤 稽核人員 (User)' : '🤖 OmniAgent (System)'
                        }</div>
                        <div>${msg.text.replace(/\n/g, '<br/>')}</div>
                        ${
                          msg.sources && msg.sources.length > 0
                            ? `<div class="sources">📎 溯源文獻：${msg.sources
                                .map((s) => `${s.title} (${(s.score * 100).toFixed(0)}%)`)
                                .join('、 ')}</div>`
                            : ''
                        }
                    </div>
  ...
  return (
    <div className="flex flex-col h-[400px] rounded-xl border border-white/5 overflow-hidden relative">
      {/* 神經形態背景 */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/5 blur-3xl pointer-events-none" />

      {/* 工具列 Header */}
      <div className="flex items-center justify-between px-4 py-2 border-b border-white/10 relative z-10">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-2 py-1 text-[10px] font-bold text-slate-300 border border-white/10 rounded hover: hover:text-white transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          title="匯出對話紀錄 (PDF)"
        >
          <Download size={12} />
          匯出稽核軌跡
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5 custom-scrollbar relative z-10">
        ...
        {messages.map((msg) => (
          ...
          >
            <div
              className={`p-2 rounded-full h-fit border ${
                msg.role === 'user'
              ...
              {msg.sources && msg.sources.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {msg.sources.map((src, idx) => (
                    <div
                      key={idx}
                      className="flex flex-col gap-1 text-[10px] p-2 rounded-md bg-slate-900 border border-slate-700 text-slate-300 shadow-sm relative overflow-hidden group"
                    >
                      <div className="flex items-center gap-1.5">
                        <BookOpen
                          size={10}
                          className={src.hashLock ? 'text-cyan-400' : 'text-emerald-400'}
                        />
              ...
              {msg.role === 'bot' && msg.id !== 'welcome' && (
                <div className="flex items-center gap-2 mt-1 opacity-60 hover:opacity-100 transition-opacity">
                  <button
                    ...
                    onClick={() =>
                      handleFeedback(
                        msg.id,
                        'good',
                        messages[messages.findIndex((m) => m.id === msg.id) - 1]?.text || '',
                        msg.text
                      )
                    }
                    className={`p-1 rounded hover:bg-emerald-500/20 hover:text-emerald-400 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 ${
                      feedbackStatus[msg.id] === 'good'
                  ...
                  <button
                    ...
                    className={`p-1 rounded hover:bg-rose-500/20 hover:text-rose-400 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-400 ${
                      feedbackStatus[msg.id] === 'bad'
