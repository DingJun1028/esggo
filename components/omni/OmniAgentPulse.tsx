...
// =========================================================================
// OmniAgent Pulse - Liquid Glass Floating UI (Optimized & Improved)
// =========================================================================
export function OmniAgentPulse() {
  ...
  // Clear execution result after 4 seconds automatically
  useEffect(() => {
    if (!executionResult) return;
    const timer = setTimeout(() => setExecutionResult(null), 4000);
  ...
  const handleCommandSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await executeCelestialCommand(commandInput.trim());
      setExecutionResult(res.message);
      setCommandInput('');
    } catch (err) {
      setExecutionResult('⚠️ Command execution failed');
    } finally {
          ...
          {isExpanded ? (
            ...
            >
              ...
              <div className="bg-gradient-to-r from-cyan-900/40 to-blue-900/40 p-3 border-b border-cyan-500/20 flex items-center justify-between cursor-grab active:cursor-grabbing group">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg bg-red-500/20 hover:bg-red-500/40 text-red-400 hover:text-white transition-colors cursor-pointer"
                    title="關閉"
                  >
                    <X size={14} />
                  </button>
              ...
              <div className="p-4 space-y-4">
                {/* Selected Model Mind Preference (G4) */}
                <div className="flex items-center justify-between bg-cyan-950/20 border border-cyan-500/10 rounded-xl px-3 py-1.5 text-xs font-mono text-cyan-400">
                  <div className="flex items-center gap-1.5">
                    <Cpu size={12} className="text-cyan-400" />
                    <span>Mind Core:</span>
                  </div>
                  <span className="font-bold text-white flex items-center gap-1">
                    Gemma 4 (G4) <Sparkles size={11} className="text-yellow-400" />
                  </span>
                </div>
                ...
                <form onSubmit={handleCommandSubmit} className="space-y-2">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] text-slate-500 uppercase tracking-widest">
                      Direct Celestial Command
                    </label>
                    <span className="text-[9px] text-slate-400 font-mono">G4 Active</span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="bg-white/5 border border-white/5 rounded-lg p-2 flex flex-col items-center text-center">
                    ...
                    <span className="text-sm font-black text-white">
                      {lastSyncRecord ? `${timeSinceLastSync}s ago` : '--'}
                    </span>
                  </div>
                </div>
            ...
            >
              ...
              <div className="flex flex-col items-start text-left">
                ...
                <span className="text-[10px] font-mono text-cyan-400 mt-1 drop-shadow-[0_0_5px_rgba(6,182,212,0.5)]">
                  {connectionStatus}
                </span>
              </div>
            </motion.div>
