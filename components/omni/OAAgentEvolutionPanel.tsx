...
export default function OAAgentEvolutionPanel() {
  ...
  return (
    <div className="rounded-xl border border-white/80 overflow-hidden">
      {/* ── Header ── */}
      <div className="relative bg-gradient-to-r from-slate-900 via-violet-950 to-slate-900 px-8 py-6 overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-between gap-6">
          ...
          <button
            ...
            disabled={isEvolving}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-violet-500 hover:bg-violet-400 disabled:bg-violet-800 text-white text-sm font-black transition-all shadow-sm disabled:cursor-not-allowed"
          >
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              {OA_LATEST_RELEASES.map((release) => (
                <div
                  key={release.version}
                  className="p-5 rounded-xl border border-slate-100 bg-white shadow-sm mb-3"
                >
            ...
            {evolveLog.length > 0 && (
              <div className="p-4 rounded-xl bg-slate-900 font-mono text-xs space-y-1">
                ...
                {isEvolving && <p className="text-violet-400">▌</p>}
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'skills' && (
          <div className="space-y-3">
            ...
            {OMNI_TO_SKILL_MAP.map((skill, i) => (
              <div
                key={i}
                className="p-4 rounded-xl border border-slate-100 bg-white hover:border-violet-200 hover:shadow-sm transition-all"
              >
