...
const DEFAULT_STEPS: ThinkingStep[] = [
  ...
  },
  {
    ...
    status: 'pending',
    label: '多路徑推理展開',
    detail: '同時評估 GRI / CSRD / TCFD 三條合規路徑',
    depth: 0,
  },
  {
    id: '2a',
  ...
  },
  {
    ...
    status: 'pending',
    label: '風險矩陣評估',
    detail: 'TCFD 氣候情境 1.5°C / 2°C 情境交叉分析',
    depth: 2,
  },
  {
    id: '3',
  ...
  },
  {
    ...
    status: 'pending',
    label: '5T 誠信驗算',
    detail: 'T1 溯源 · T2 透明 · T4 可信任 驗算閘門',
    depth: 0,
  },
  {
    id: '4a',
    ...
    status: 'pending',
    label: 'ZKP 零知識證明',
    detail: 'Pedersen Commitment 同態加法驗證',
    depth: 1,
  },
  {
    id: '5',
    phase: 'seal',
    status: 'pending',
    label: 'SHA-256 Hash Lock 封印',
    detail: '輸出結果寫入不可篡改的永恆記憶',
    depth: 0,
  },
];
...
const ThinkingConnector = ({ active, depth }: { active: boolean; depth: number }) => (
  <div className={`flex items-start ${depth > 0 ? 'ml-6' : ''}`}>
    <div className="flex flex-col items-center mr-3" style={{ marginLeft: depth * 16 }}>
      <div
        className={`w-px flex-1 min-h-[20px] transition-all duration-500 ${
          active ? 'bg-gradient-to-b from-violet-400/60 to-transparent' : 'bg-white/5'
        }`}
      />
    </div>
  </div>
  ...
  <div className="relative w-10 h-10 flex items-center justify-center">
    ...
    <div
      className={`w-10 h-10 rounded-full flex items-center justify-center border transition-all duration-500 ${
        active ? 'bg-violet-500/20 border-violet-400/40' : 'bg-white/5 border-white/10'
      }`}
    >
...
export function OmniThinkingChain({
  ...
  const [steps, setSteps] = useState<ThinkingStep[]>(
    (externalSteps || DEFAULT_STEPS).map((s) => ({ ...s, status: 'pending' }))
  );
  ...
  useEffect(() => {
    ...
    const list = (externalSteps || DEFAULT_STEPS).map((s) => ({
      ...s,
    ...
    const advance = () => {
      ...
      if (idx === 8 && !hasFailed && !externalSteps) {
        setSteps((prev) =>
          prev.map((s, i) =>
            i === idx
              ? { ...s, status: 'failed', detail: '驗算失敗：發現不一致的雜湊簽章 (Hash Mismatch)' }
              : s
          )
        );
        hasFailed = true;
        ...
        timerRef.current = setTimeout(() => {
          setSteps((prev) => {
            ...
            newSteps.splice(idx + 1, 0, {
              ...
              phase: 'verify',
              status: 'fallback',
              label: '啟動萬能果因協議 (Karma Protocol)',
              detail: '執行 HEAL 進行自動修復與重新封印...',
              depth: 1,
            });
            return newSteps;
          });
          ...
          timerRef.current = setTimeout(() => {
            setSteps((prev) =>
              prev.map((s, i) =>
                i === idx + 1 ? { ...s, status: 'done', detail: '修復完成，ZKP 已重新綁定' } : s
              )
            );
  ...
  const currentPhase =
    activeIdx >= 0 && activeIdx < steps.length ? steps[activeIdx]?.phase : 'seal';
        ...
        <div className="h-1 rounded-full overflow-hidden">
          <div
            ...
            style={{
              background: done
                ? '#34d399'
                : `linear-gradient(to right, #7c3aed, ${
                    cfg.color.includes('cyan')
                      ? '#22d3ee'
                      : cfg.color.includes('amber')
                      ? '#fbbf24'
                      : cfg.color.includes('emerald')
                      ? '#34d399'
                      : cfg.color.includes('rose')
                      ? '#fb7185'
                      : '#a78bfa'
                  })`,
            }}
