...
export default function VaultOmniTable({
  columns = [],
  records = [],
  className = '',
}: VaultOmniTableProps) {
  const [selectedRecord, setSelectedRecord] = useState<string | null>(null);
  const [liveRecords, setLiveRecords] = useState<VaultOmniTableRecord[]>(records);
    ...
    const fetchVaultData = async () => {
      ...
      try {
        ...
        if (!error && data && data.length > 0) {
          const mapped: VaultOmniTableRecord[] = data.map((item) => ({
            id: item.id,
            data: { ...item },
            timestamp: item.created_at,
            author: item.company_id || 'OmniSystem',
            zkpHash: item.hash_lock || '0000000000000000',
            fiveTStatus: [true, true, true, true, item.is_sealed || false],
          }));
        ...
        console.warn('Fallback to mock data due to Supabase connection issue:', err);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // If no columns are provided, derive them from the first record if possible
      ...
      <div className="p-4 border-b border-slate-800 bg-slate-900/50 flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-lg font-bold text-slate-200">
            Vault OmniTable{' '}
            <span className="text-xs font-mono text-cyan-500 ml-2 border border-cyan-900/50 bg-cyan-950/30 px-2 py-0.5 rounded">
              ZKP SEALED
            </span>
          </h3>
        </div>
          ...
          <tbody className="text-sm font-medium">
                ...
                >
                  ...
                  <td className="p-4 text-slate-400 flex items-center gap-2">
                    <div className="w-6 h-6 rounded-full bg-slate-700 flex items-center justify-center text-[10px] text-white">
                      {record.author ? record.author.charAt(0).toUpperCase() : '?'}
                    </div>
                      ...
                      {record.fiveTStatus.map((status, i) => (
                        <div
                          ...
                          className={`w-2 h-4 rounded-sm ${
                            status ? 'bg-cyan-500' : 'bg-slate-700'
                          }`}
              ...
              {liveRecords.length === 0 && !isLoading && (
                <tr>
                  ...
                  >
                    No verified records found in Evidence Vault.
                  </td>
                </tr>
              )}
