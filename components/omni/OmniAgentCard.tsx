    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 opacity-20 pointer-events-none">
          <div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] md:w-[400px] h-[300px] md:h-[400px] rounded-full border-2 animate-spin-slow"
            style={{ borderColor: `${colors.accent.cyan}30` }}
          ></div>
          <div
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] md:w-[460px] h-[340px] md:h-[460px] rounded-full border"
            style={{ borderColor: `${colors.accent.cyan}20`, animationDelay: '1s' }}
          ></div>
        </div>
        ...
        <div
          ...
          style={{ background: `linear-gradient(to top, ${colors.accent.cyan}10, transparent)` }}
        >
          <div className="grid grid-cols-8 h-full opacity-30">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className="border-r"
                style={{ borderColor: `${colors.accent.cyan}20` }}
              ></div>
            ))}
          </div>
        </div>
        ...
        <div className="relative z-10 h-full flex items-center justify-center py-8">
          ...
          >
            <div
              className="w-48 h-48 md:w-64 md:h-64 rounded-full flex items-center justify-center mb-4 border shadow-[0_0_50px_rgba(85,199,255,0.2)]"
              style={{
                background: `linear-gradient(to bottom right, ${colors.accent.cyan}20, ${colors.accent.purple}20)`,
        ...
        <div className="absolute bottom-6 left-6 z-20">
          <div className="text-[11px] font-mono uppercase" style={{ color: colors.text.muted }}>
            UNIT ID // {id} // {name}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-3 md:gap-5">
        ...
        <div className="space-y-3">
          <div className="flex flex-col sm:flex-row items-start justify-between gap-3">
            <div>
              <h1
                className="text-3xl sm:text-4xl md:text-[44px] font-black leading-tight tracking-tight drop-shadow-sm"
          ...
          >
            ...
            <div className="flex items-center justify-center h-40">
              <div className="relative w-36 h-36">
                <svg
                  viewBox="0 0 100 100"
                  className="w-full h-full drop-shadow-[0_0_10px_rgba(85,199,255,0.3)]"
                >
                ...
                {['感知', '解析', '推演', '執行', '協同', '進化'].map((label, idx) => (
                  <div
                    ...
                    style={{
                      ...
                      ...(idx === 1 && { top: '20%', right: 0, transform: 'translateX(1.5rem)' }),
                      ...(idx === 2 && {
                        bottom: '20%',
                        right: 0,
          ...
          >
            {stats.map((stat) => (
              <div key={stat.label} className="flex items-center gap-3 text-sm">
                <div className="w-14 font-bold" style={{ color: colors.text.secondary }}>
                  {stat.label}
                </div>
                <div
                  className="w-10 text-xs font-black text-center"
                  style={{
                    color: stat.rank.includes('EX') ? colors.accent.purple : colors.accent.gold,
                  }}
                >
                  {stat.rank}
                </div>
        ...
        <div className="space-y-3 mt-2">
            ...
            {skills.map((skill) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="text-[9px] font-bold" style={{ color: colors.text.muted }}>
                    {skill.type}
                  </div>
                </div>
                <div
                  className="text-xs font-black mb-0.5 truncate"
                  style={{ color: colors.text.primary }}
                >
                  {skill.nameEN}
                </div>
          ...
          >
            ...
            <div className="relative flex items-center gap-4">
              <div
                className="w-14 h-14 rounded-full flex items-center justify-center flex-shrink-0 shadow-[0_0_15px_rgba(155,124,255,0.5)]"
                style={{
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className="text-[10px] font-black px-2 py-0.5 rounded border border-white/10"
                ...
                >
                  {ultimate.nameEN}{' '}
                  <span className="text-sm font-bold opacity-70 ml-1">{ultimate.nameZH}</span>
                </div>
                <div
                  className="text-[11px] font-bold mt-1 opacity-80"
                  style={{ color: colors.text.secondary }}
                >
              ...
              <div
                className="text-2xl font-black italic tracking-tighter drop-shadow-md pr-2"
                style={{ color: colors.accent.gold }}
              >
                100%
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-2">
            ...
            <div className="flex gap-1.5 mt-2">
              {voiceLines.map((_, i) => (
                <div
                  ...
                  style={{
                    backgroundColor: i === currentVoiceLine ? colors.accent.cyan : colors.border,
                  }}
                />
              ))}
            </div>
          ...
          {/* Status */}
          <div
            className="rounded-xl p-4 border font-mono text-[10px]"
            style={{ backgroundColor: `${colors.bg.card}cc`, borderColor: `${colors.border}80` }}
          >
            <div
              className="font-bold uppercase tracking-widest mb-2 font-sans"
              style={{ color: colors.text.secondary }}
            >
              System Status
            </div>
            <div className="space-y-1 font-bold" style={{ color: colors.text.muted }}>
              {Object.entries(systemStatus).map(([key, value]) => (
                <div key={key} className="flex justify-between">
                  <span>{key}</span>{' '}
                  <span
                    style={{ color: key === 'MODE' ? colors.accent.purple : colors.accent.cyan }}
                  >
                    {value}
                  </span>
                </div>
