...
const FiveTBadge = ({ tag }: { tag: string }) => {
  ...
  return (
    <span
      className={`px-1.5 py-0.5 rounded text-xs font-black font-mono ${colors[tag] || 'bg-gray-100 text-gray-600'}`}
    >
      {tag}
    </span>
  );
...
const PulseRing = ({ active }: { active: boolean }) => (
  <span className="relative flex h-3 w-3">
    ...
    <span
      className={`relative inline-flex rounded-full h-3 w-3 ${active ? 'bg-violet-500' : 'bg-slate-300'}`}
    />
  </span>
);
    ...
    <div className="rounded-3xl border border-white/80 bg-white/60 backdrop-blur-xl shadow-glass overflow-hidden">
      ...
      <div className="px-8 py-4 bg-slate-50/80 border-b border-slate-100 flex items-center gap-4 flex-wrap">
        ...
        <span className="text-xs text-slate-500 font-medium">
          {HERMES_TO_OMNI_SKILL_MAP.filter((s: any) => s.absorptionStatus === 'transcended').length}{' '}
          個技能已超越 ·{' '}
          {HERMES_TO_OMNI_SKILL_MAP.filter((s: any) => s.absorptionStatus === 'absorbed').length}{' '}
          個已吸收 ·{' '}
          {HERMES_TO_OMNI_SKILL_MAP.filter((s: any) => s.absorptionStatus === 'pending').length}{' '}
          個待洗鍊
        </span>
      </div>
        ...
        {activeTab === 'skills' && (
          <div className="space-y-3">
            <p className="text-xs text-slate-500 mb-4">
              每一行代表一個 Hermes 通用技能如何被 ESGGO 洗鍊為 OmniAgent 專屬 ESG 能力
            </p>
