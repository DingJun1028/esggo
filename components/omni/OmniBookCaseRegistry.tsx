...
const MOCK_DATA: IOmniComponent[] = [
  {
    ...
    timestamp: Date.now(),
    evidence: { source_origin: 'mock', flow_path: [], timestamp: Date.now() },
    id: 'OMNI-001',
    type: 'Molecule',
    name: 'BrandKpiCard',
  ...
  },
  {
    ...
    type: 'Atom',
    name: 'AtomicButton',
    lifecycleStatus: RecordLifecycleStatus.Pending,
    attribution: '萬能元件',
    tangible: true,
    traceable: true,
    trackable: false,
  ...
  },
  {
    ...
    timestamp: Date.now(),
    evidence: { source_origin: 'legacy', flow_path: ['migration'], timestamp: Date.now() },
    id: 'OMNI-006',
    type: 'Molecule',
...
export default function OmniBookCaseRegistry() {
  ...
  const renderDimension = (active: boolean, isTrustworthy: boolean = false) => {
    if (active) {
      return isTrustworthy ? (
        <span className="text-red-500 font-bold" title="已封印 (Hash Lock)">
          🔴
        </span>
      ) : (
        <span className="text-green-500 font-bold" title="啟用">
          🟢
        </span>
      );
    }
    return (
      <span className="text-yellow-500 font-bold" title="待驗證">
        🟡
      </span>
    );
  };
  ...
  const registerNewComponent = async () => {
    const newComponent: IOmniComponent = {
      ...
      id: `OMNI-${Math.floor(Math.random() * 1000)
        .toString()
        .padStart(3, '0')}`,
      type: 'Atom',
      ...
      <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-gray-50/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              OmniBookCase元件總表 · 5T+1 維度控管矩陣 (MECE 分類)
            </p>
          </div>
          <div className="ml-4">
            <OmniZKPBadge />
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <CheckCircle
              size={16}
              className={isVerifying ? ' text-emerald-500' : 'text-emerald-500'}
            />
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider sticky top-0 z-10 shadow-sm">
              ...
              <th className="px-6 py-4 font-semibold border-b border-gray-200 text-center text-[#63a6b0]">
                元件歸屬
              </th>
              <th
                className="px-3 py-4 font-semibold border-b border-gray-200 text-center"
                title="Tangible"
              >
                感知
              </th>
              ...
              <th className="px-6 py-4 font-semibold text-gray-700 text-center">
                不可篡改
                <br />
                <span className="text-[10px] text-gray-400 normal-case">(Trustworthy)</span>
              </th>
              <th className="px-6 py-4 font-semibold border-b border-gray-200 text-right">
                按鈕 (Actions)
              </th>
            </tr>
          </thead>
            ...
            {isLoading ? (
              ...
              filteredComponents.map((comp) => (
                <tr key={comp.id} className="hover:bg-blue-50/30 transition-colors group">
                  ...
                  <td className="px-6 py-4">{renderStatusBadge(comp.lifecycleStatus)}</td>
                  ...
                  <td className="px-3 py-4 text-center">
                    {renderDimension(comp.trustworthy, true)}
                  </td>
      ...
      <div className="p-4 border-t border-gray-100 bg-gray-50/50 flex justify-between items-center text-xs text-gray-500">
        ...
        <div className="flex items-center gap-4">
          <span className="flex items-center gap-1">
            <span className="text-green-500 font-bold">🟢</span> 啟用
          </span>
          <span className="flex items-center gap-1">
            <span className="text-yellow-500 font-bold">🟡</span> 待驗證
          </span>
          <span className="flex items-center gap-1">
            <span className="text-red-500 font-bold">🔴</span> 已封印
          </span>
        </div>
      </div>
    </div>
