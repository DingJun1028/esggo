...
} from '@/lib/hooks/useOmniAgentStream';

/* ─── Mission Definitions ─── */
const MISSIONS = [
  {
    id: 'SYNC_OMNIBLUE_OMNITABLE',
    label: 'OmniBlue → OmniTable',
    labelZh: '藍碳同步',
    icon: Database,
    color: '#06b6d4',
  },
  ...
  {
    id: 'PILOT_REPORT',
    label: 'SustainWrite Pilot',
    labelZh: '自主撰寫',
    icon: Brain,
    color: '#8b5cf6',
  },
  {
    id: 'TRANSFER_TO_NCBDB',
    label: 'NCBDB Migration',
    labelZh: '資料遷移',
    icon: ArrowRight,
    color: '#f59e0b',
  },
...
function ConnectionBadge({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {!connected && (
        <button onClick={onReconnect} className="p-1 rounded hover: text-slate-400">
          <RefreshCw size={12} />
        </button>
      )}
    </div>
        ...
        <CardContent className="p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ background: `${mission.color}15`, border: `1px solid ${mission.color}30` }}
              >
          ...
          {progress?.totalProcessed !== undefined && status === 'complete' && (
            <p className="text-[10px] text-emerald-400 mb-3">
              ✓ Processed {progress.totalProcessed} items
            </p>
          )}
      ...
      <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/10">
        ...
        <div className="space-y-1.5 text-[10px]">
          <p className="text-slate-300">
            Gate: <span className="text-white font-bold">{String(lastSeal.payload.gate)}</span>
          </p>
          <p className="text-slate-300">
            Resource:{' '}
            <span className="text-white font-bold">
              {String(lastSeal.payload.chapter || lastSeal.payload.resource || 'N/A')}
            </span>
          </p>
...
export default function ThinkTankControl() {
  const { events, isConnected, connectionError, activeMissions, lastSeal, reconnect, clearEvents } =
    useOmniAgentStream();
      ...
      <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-black text-white tracking-tight flex items-center gap-2">
              ...
              <Badge
                variant="outline"
                className="bg-cyan-500/5 text-cyan-400 border-cyan-500/30 text-[8px]"
              >
                LIVE
              </Badge>
            </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            ...
            size="sm"
            onClick={clearEvents}
            className="text-[9px] font-bold uppercase border border-white/5 h-7"
          >
            Clear Log
          </Button>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3">
            {MISSIONS.map((m) => (
              <MissionCard
                key={m.id}
                mission={m}
                progress={activeMissions.find(
                  (p) =>
                    p.mission === m.label ||
                    p.mission ===
                      m.id
                        .replace(/_/g, ' ')
                        .replace('SYNC OMNIBLUE OMNITABLE', 'OmniBlue to OmniTable Sync')
                )}
              ...
              {[
                {
                  ...
                  value: activeMissions.filter((m) => m.status === 'complete').length,
                  color: 'text-emerald-400',
                },
                {
                  label: 'Active Missions',
                  value: activeMissions.filter((m) => m.status === 'running').length,
                  color: 'text-cyan-400',
                },
                { label: 'Total Events', value: events.length, color: 'text-slate-300' },
                {
                  label: '5T Seals',
                  value: events.filter((e) => e.event === '5T_SEAL').length,
