...
export default function OmniAuthOmni() {
  const [status, setStatus] = useState<'idle' | 'authenticating' | 'verifying_5T' | 'success'>(
    'idle'
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 bg-[#020617] rounded-3xl border border-cyan-500/20 shadow-[0_0_50px_rgba(6,182,212,0.15)] max-w-md w-full mx-auto relative overflow-hidden">
      ...
      <p className="text-slate-400 text-sm mb-8 text-center leading-relaxed">
        通過 5T 誠信協議驗證您的主權身份
        <br />
        <span className="text-[10px] text-cyan-500/70 uppercase tracking-widest font-mono mt-1 block">
      ...
      <button
        ...
        className="w-full relative group overflow-hidden rounded-xl border border-white/10 p-4 transition-all hover: hover:border-cyan-500/30 active:scale-95"
      >
        ...
        <div className="relative flex items-center justify-center gap-3">
          <Fingerprint
            size={20}
            className={status === 'idle' ? 'text-cyan-400' : 'text-slate-500'}
          />
