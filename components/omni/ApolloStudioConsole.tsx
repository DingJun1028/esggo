...
export default function ApolloStudioConsole() {
  ...
  const triggerSwarm = () => {
    ...
    setLogs((prev: string[]) => [...prev, '[Apollo] 啟動 OmniAgent Orchestrator...']);
    setTimeout(
      () => setLogs((prev: string[]) => [...prev, '[Nexus] 指令下達至 L-Hub 代理群集']),
      800
    );
    setTimeout(
      () => setLogs((prev: string[]) => [...prev, '[Swarm] 檢索 Vault Omni 進行數據溯源']),
      1500
    );
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-indigo-300/70 font-mono tracking-widest mt-1">
              OmniAgent Swarm Orchestration Center
            </p>
          </div>
        </div>
