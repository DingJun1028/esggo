...
export function ShieldOfAbsoluteTruth({
  ...
  const [status, setStatus] = useState<'analyzing' | 'secure' | 'drift-detected' | 'healing'>(
    'analyzing'
  );
  ...
  return (
    <Card
      className={cn(
        ...
        status === 'secure'
          ...
          : status === 'healing'
          ? 'border-aqua-cyan/50 bg-aqua-cyan/10'
    ...
    >
      <div className="p-4 flex items-center justify-between border-b border-slate-100/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-black text-sm text-text-primary flex items-center gap-2">
              絕對真實之盾{' '}
              <span className="text-[10px] font-mono text-slate-400 font-medium px-1.5 py-0.5 bg-slate-100 rounded">
                Omni-Core ZKP
              </span>
            </h3>
        ...
        <Badge
          status={
            status === 'secure' ? 'success' : status === 'drift-detected' ? 'warning' : 'info'
          }
        >
          {status.toUpperCase()}
        </Badge>
          ...
          <div className="flex items-start gap-3 bg-white p-3 rounded-xl border border-emerald-100">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[11px] text-slate-500 font-medium">
                已通過 5T 誠信協議驗證，ZKP 雙重雜化與 MD5 埋植比對無誤。無綠漂 (Greenwashing)
                跡象。
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 bg-white p-3 rounded-xl border border-amber-100">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-slate-500 font-medium">
                  部分 AI 生成內容缺乏客觀數據支撐，可能觸發綠漂風險。建議啟動 ADK 積極修復系統。
                </p>
              </div>
            </div>
