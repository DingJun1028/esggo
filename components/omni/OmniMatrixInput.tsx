    ...
    <div className="w-full bg-[#020617]/50 rounded-xl border border-cyan-500/20 overflow-hidden">
                ...
                <div key={form.id} className="flex flex-col items-center gap-1">
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center transition-all duration-300 ${
                      isActive
                        ? 'bg-cyan-500 text-white shadow-[0_0_10px_rgba(6,182,212,0.8)] scale-110'
                        : isPast
                  ...
                  <span
                    className={`text-[9px] font-bold ${
                      isActive ? 'text-cyan-400' : isPast ? 'text-cyan-600' : 'text-slate-600'
                    }`}
                  >
                    {form.label}
                  </span>
      ...
      <form onSubmit={handleSubmit} className="p-2 relative">
        <input
          ...
          className="w-full border border-white/10 rounded-lg py-2 pl-3 pr-10 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/50"
          ...
          >
            <div className="flex justify-between items-center mb-1">
              <span className="font-bold">
                {status === 'success' ? '✅ 矩陣執行完成' : '⚠️ 矩陣異常'}
              </span>
              <button onClick={() => setStatus('idle')} className="text-xs hover:text-white">
                清除
              </button>
            </div>
            {status === 'success' && result?.strategy && (
              <div className="mt-1 p-1.5 rounded border border-white/5 space-y-1">
                <div>
                  <span className="text-slate-500">代理:</span> {result.strategy.assignedAgent}
                </div>
                <div>
                  <span className="text-slate-500">信心:</span>{' '}
                  {(result.strategy.confidence * 100).toFixed(0)}%
                </div>
                <div>
                  <span className="text-slate-500">行動:</span>{' '}
                  {result.strategy.actionGraph?.length || 0} 步
                </div>
              </div>
            )}
