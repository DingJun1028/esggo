...
import {
  ...
  Lightbulb,
  Heart,
} from 'lucide-react'; // 新增更多 Lucide icons for AI styles
import { cn } from '../lib/utils';
import OmniSustainWriteEditor, { OmniSustainWriteEditorRef } from './omni/OmniSustainWriteEditor'; // 導入 TipTap 編輯器組件及其 Ref 類型
import {
  convertHtmlToPdf,
  convertHtmlToDocx,
  convertHtmlToMarkdown,
  convertHtmlToPlainText,
} from '../lib/utils/documentConverters'; // 導入轉換函數

const AI_STYLES_OPTIONS = [
  {
    key: 'professional',
    label: '嚴謹專業',
    description: '請以嚴謹、專業且符合 GRI 準則的口吻進行擴寫，確保語氣客觀中立。',
    icon: <Briefcase size={12} />,
  },
  {
    key: 'data_driven',
    label: '數據導向',
    description: '請以數據導向為主，在擴寫時強調具體指標、成效與量化數據的邏輯分析。',
    icon: <Database size={12} />,
  },
  {
    key: 'innovative',
    label: '前瞻創新',
    description: '請以創新、前瞻性的口吻進行擴寫，強調企業在永續發展上的突破與未來願景。',
    icon: <Lightbulb size={12} />,
  },
  {
    key: 'empathetic',
    label: '溫暖關懷',
    description: '請以溫暖、關懷社會的柔和口吻擴寫，強調企業對員工、社區與環境的正面影響力。',
    icon: <Heart size={12} />,
  },
];
...
export function ChapterEditor({
  ...
  const editorRef = useRef<OmniSustainWriteEditorRef>(null); // 新增 editorRef
  const {
    generatedContent,
    updateContent,
    commitHistory,
  ...
  const handleExport = async (format: string) => {
    ...
    const filenamePrefix = `${chapterName.replace(/\s/g, '_')}_${
    ...
    <div className="flex flex-col h-full bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
      ...
      <div className="h-12 px-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex gap-1">
            ...
            <button
              ...
              disabled={
                !(contentHistory[chapterId]?.future.length > 0) || isGeneratingAI[chapterId]
              }
              className="p-1.5 rounded-lg text-slate-400 hover:text-cyan-600 hover:bg-cyan-50 disabled:opacity-20 transition-all"
              title="重做"
            >
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() =>
              expandContentWithAI(chapterId, chapterName, chapterOrder, griRefs, AI_STYLES[aiStyle])
            }
            disabled={isGeneratingAI[chapterId]}
      ...
      <div className="flex-1 relative">
        <OmniSustainWriteEditor
          ...
          onChange={(htmlContent: string) =>
            updateContent(chapterId, htmlContent, chapterName, chapterOrder, griRefs)
          }
