    ...
    <div className="fixed bottom-10 right-10 z-[10000] flex flex-col items-end">
        ...
        {isOpen && (
          <div
            className={cn(
              ...
              isMaximized
                ? 'w-full max-w-[90%] sm:max-w-[640px] h-[60vh] sm:h-[80vh] max-h-[80vh]'
                : 'w-full max-w-[90%] sm:max-w-[400px] max-w-md h-[60vh] max-h-[60vh]'
            )}
          >
            <Card className="w-full h-full flex flex-col border-white/60 rounded-[3rem] p-0 overflow-hidden">
              {/* Agent Header */}
              <header className="p-8 bg-berkeley-blue text-white flex items-center justify-between relative overflow-hidden cursor-grab active:cursor-grabbing">
                <div className="absolute top-0 right-0 w-64 h-full blur-3xl -z-0" />
                <div className="flex items-center gap-5 relative z-10">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <Badge
                        variant="verified"
                        className="bg-emerald-400/20 text-emerald-300 border-none px-2 py-0.5 text-[9px]"
                      >
                        ACTIVE
                      </Badge>
                    </div>
                ...
                <div className="flex items-center gap-2 relative z-10">
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center hover: rounded-xl transition-all text-white/60 hover:text-white"
                  >
              ...
              <div className="flex-1 p-8 overflow-y-auto space-y-6 bg-slate-50/30 no-scrollbar">
                <div className="flex gap-4">
                  ...
                  <Card className="bg-white p-5 rounded-3xl rounded-tl-none border border-slate-100/50 shadow-sm text-[13px] text-slate-600 leading-relaxed font-medium">
                    您好，我是您的 **OmniAgent
                    數據大祭司**。我正在即時平衡系統的任督二脈能量流。當前共鳴算力為 **
                    {(rs * 100).toFixed(1)}%**，全域 5T 狀態穩定。
                  </Card>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <Card className="p-5 border-white/80 shadow-sm flex flex-col items-center text-center gap-3">
              ...
              <footer className="p-6 bg-white border-t border-slate-100/50">
                <div className="relative group">
                  ...
                  <Button
                    variant="primary"
                    className="absolute right-2 top-2 w-10 h-10 p-0 rounded-xl"
                    onClick={() => setInput('')}
                  >
                    <Send size={16} />
                  </Button>
                ...
                <div className="mt-4 flex items-center justify-center gap-6">
                  <button className="text-[10px] font-black text-slate-300 hover:text-berkeley-blue transition-colors uppercase tracking-widest">
                    5T_SEAL
                  </button>
      ...
      <button
        ...
        className={cn(
          ...
          isOpen
            ? 'bg-white text-berkeley-blue rotate-90 border border-white'
