    ...
    <>
      <div
        ...
        style={{
          ...
          background: sealed
            ? 'linear-gradient(135deg, rgba(240,253,244,0.4) 0%, rgba(255,255,255,0.8) 100%)'
            : undefined,
        }}
        onClick={() => {
      ...
      >
          ...
          <div className="flex flex-col items-end gap-1.5">
            {sealed ? (
              <BrandBadge
                ...
                size="sm"
                dot
                className="font-black tracking-tighter shadow-sm"
              >
                ZKP_SEALED
              </BrandBadge>
            ) : verified ? (
              <BrandBadge variant="success" size="sm" className="font-bold">
                5T_VERIFIED
              </BrandBadge>
            ) : (
              <BrandBadge variant="warning" size="sm" className="font-bold">
                PENDING
              </BrandBadge>
            )}
          </div>
        ...
        <div className="space-y-4 relative z-10">
          <div className="flex flex-col">
            <h3 className="text-[11px] text-slate-400 font-black uppercase tracking-[0.2em] mb-1.5 group-hover:text-slate-500 transition-colors">
              {label}
            </h3>
            {formula && (
          ...
          <div className="flex items-baseline gap-2.5">
            <span className="text-4xl font-black text-[#003262] tracking-tighter font-mono">
              {value}
            </span>
            {unit && (
              <span className="text-sm text-slate-400 font-black uppercase tracking-[0.2em]">
                {unit}
              </span>
            )}
          </div>
      ...
      {showDetail && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-6 lg:p-12 animate-in fade-in zoom-in duration-300">
          <div className="absolute inset-0 bg-slate-900/60" onClick={() => setShowDetail(false)} />
          <div className="relative rounded-[40px] border border-white shadow-extreme p-10 lg:p-14 max-w-2xl w-full overflow-hidden">
            ...
            <header className="flex items-start justify-between mb-12 relative z-10">
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-slate-400 font-black uppercase tracking-[0.3em] ml-13">
                  Governance Proof & Provenance
                </p>
              </div>
              <button
                  ...
                  >
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="text-[13px] font-black uppercase tracking-widest">
                        {sealed ? '5T Sealed & Immutable' : 'Live Synchronization'}
                      </span>
                    </div>
            ...
            <footer className="mt-16 pt-8 border-t border-slate-100 flex items-center justify-between relative z-10">
              <div className="flex flex-col">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-4xl font-black text-[#003262] font-mono tracking-tighter">
                    {value}
                  </span>
