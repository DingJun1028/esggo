      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-50 -md sticky top-0 z-10">
            ...
            <button
              onClick={onClose}
              className="w-10 h-10 rounded-xl bg-slate-50 hover:bg-slate-100 flex items-center justify-center text-slate-400 transition-all hover:rotate-90 border border-slate-100"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="px-6 py-3 bg-slate-50/30 border-b border-slate-50">
            <BrandTabs activeTab={activeTab} onTabChange={setActiveTab} tabs={tabs} />
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-8 no-scrollbar scroll-smooth">
                ...
                <BrandCard className="bg-[#003262] text-white p-8 rounded-[2.5rem] border-none shadow-2xl relative overflow-hidden">
                  <div className="relative z-10 space-y-6">
                    ...
                    <p className="text-[10px] text-blue-100/60 leading-relaxed font-medium italic">
                      「偵測到您正在編輯『環境績效』章節。Z0-Auditor 已自動載入 ISO 14064-1
                      實證標記。」
                    </p>
                  </div>
                  <Activity
                    size={180}
                    className="absolute -bottom-10 -right-10 text-white/5 opacity-20"
                  />
                </BrandCard>
                  ...
                  {[
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div
                          className={cn(
                            'w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center transition-colors group-hover:bg-[#003262] group-hover:text-white',
                            a.color
                          )}
                        >
                          {a.icon}
                      ...
                      <BrandBadge
                        variant={a.status === 'Idle' ? 'outline' : 'info'}
                        size="xs"
                        className="px-3"
                      >
                        {a.status}
            ...
            {activeTab === 'vault' && (
              <div className="space-y-6 fade-in">
                ...
                <section className="space-y-4">
                  <h4 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] ml-2">
                    5T Sealed Artifacts
                  </h4>
                  {[
                    {
                      name: 'GRI_305_GHG_Scan.json',
                      hash: 'sha256:ox_alc_8812',
                      type: 'ALCHEMY_RESULT',
                    },
                    {
                      name: 'Sovereign_Strategy_v1.pdf',
                      hash: 'sha256:ox_strat_442',
                      type: 'VAULT_PROOF',
                    },
                  ].map((file, i) => (
                    <BrandCard
                      key={i}
                      padding="md"
                      className="border-none shadow-sm hover:shadow-premium transition-all group cursor-pointer relative overflow-hidden"
                    >
                      ...
                      <div className="flex items-center justify-between relative z-10">
                        ...
                        <BrandBadge variant="outline" size="xs" className="scale-75 origin-right">
                          {file.type}
                        </BrandBadge>
                      </div>
                    </BrandCard>
            ...
            {activeTab === 'history' && (
              <div className="space-y-4 fade-in">
                <div className="flex items-center justify-between px-2 mb-4">
                  <h4 className="text-[10px] font-black text-slate-300 uppercase tracking-[0.25em]">
                    Autonomous Traces
                  </h4>
                  <BrandBadge variant="success" size="xs" dot>
                    Live_Sovereignty
                  </BrandBadge>
                </div>
                {[
                  {
                    time: '16:45:22',
                    msg: 'Z0-Auditor detected GHG deviation > 5%',
                    status: 'Warning',
                    detail: 'Triggering Swarm Handoff',
                  },
                  {
                    time: '16:45:01',
                    msg: 'Vault_Omni synchronizing with Alchemy...',
                    status: 'Syncing',
                  },
                  {
                    time: '16:42:12',
                    msg: 'Strategy_Lab proposal [OX-3] accepted.',
                    status: 'Sealed',
                  },
                  ...
                  >
                    <span className="text-[10px] font-black font-mono text-slate-300 w-14 shrink-0">
                      {trace.time}
                    </span>
                    <div className="flex-1 space-y-2">
                      <p className="text-[11px] font-black text-slate-700">{trace.msg}</p>
                      {trace.detail && (
                        <div className="p-2.5 rounded-xl border border-slate-100 text-[9px] font-bold text-slate-400 uppercase flex items-center gap-2">
          ...
          {/* Footer Action */}
          <div className="p-8 border-t border-slate-100 -md sticky bottom-0 z-10">
            <BrandButton
              variant="primary"
              fullWidth
