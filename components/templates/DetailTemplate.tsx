    ...
    <div className="flex flex-col gap-8 w-full fade-in pb-20">
          ...
          <nav className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-text-secondary/50">
            {breadcrumbs.map((b, i) => (
              <React.Fragment key={i}>
                <a href={b.href} className="hover:text-text-brand transition-colors">
                  {b.label}
                </a>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {summaryItems.map((item, i) => (
          <Card key={i} className="p-5 border-border-primary/50 shadow-sm">
            <p className="text-[9px] font-black text-text-secondary uppercase tracking-[0.2em] mb-2">
              {item.label}
            </p>
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        {sidebar && <aside className="col-span-4 space-y-6">{sidebar}</aside>}
      </div>
    </div>
  );
}
