...
export default function OmniCodexViewer({
  isVpsBound,
  toggleVpsBindingAction,
}: {
  isVpsBound: boolean;
  toggleVpsBindingAction: () => void;
}) {
  return (
    <div className="relative bg-[#020617]/60 border border-white/10 rounded-2xl p-6 mt-8 overflow-hidden group w-full">
      ...
      <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4">
        ...
        <span className="text-xs text-cyan-400 font-mono border border-cyan-400/30 px-2 py-1 rounded bg-cyan-400/10">
          TRANSCENDED
        </span>
      </div>

      <div className="flex flex-col md:flex-row gap-6 relative z-10">
        <div className="flex-1 border border-white/5 p-4 rounded-xl">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-300">
              <CheckCircle size={14} className="text-emerald-400" />{' '}
              <span>Karma Protocol 自動果因推演與 5T 封存</span>
            </div>
            <div className="flex items-center gap-2 text-xs text-slate-300">
              <CheckCircle size={14} className="text-emerald-400" />{' '}
              <span>Gap Analysis 短板自動化探測上線</span>
            </div>
          </div>
        </div>

        {/* 終始矩陣 (End-to-Start Matrix) */}
        <div className="flex-1 border border-white/5 p-4 rounded-xl relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mt-8 relative px-4">
            ...
            <div className="flex flex-col items-center gap-2 z-10">
              <div
                className={`w-16 h-16 rounded-full border-2 flex items-center justify-center text-2xl font-black transition-all duration-500 ${
                  isVpsBound
              ...
              <div className="text-center mt-2">
                <p
                  className={`text-xs font-bold uppercase tracking-wider ${
                    isVpsBound ? 'text-white' : 'text-slate-500'
                  }`}
                >
                  OmniAgent Gateway
                </p>
