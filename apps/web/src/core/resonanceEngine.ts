...
export class ResonanceEngine {
  ...
  private calculateTemporalResonance(card: IComponentCore, truth: IComponentCore): number {
    // 檢查 timestamp 匹配度
    const timeDiff = Math.abs(card.timestamp - truth.timestamp);
    return Math.max(0, 1 - timeDiff / 3600000); // 1小時內認為匹配
  }

  private calculateStructuralResonance(card: IComponentCore, truth: IComponentCore): number {
    // 檢查結構相似度 (UUID, version 等)
    return card.version === truth.version ? 1 : 0.5;
  }

  private calculateContentResonance(card: IComponentCore, truth: IComponentCore): number {
