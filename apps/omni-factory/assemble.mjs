...
//   6. 生成 RWD HTML 模組 (StandardPage + KpiCard + T5Strip + VaultTable)
//   7. 對接現有 CD: 印出 deploy 指令 (不自動 push, 避免越權)
//
...
function p6_qualityGate(spec, artifact, registryPath) {
  ...
  try {
    ...
    if (hit) {
      ...
      else { t5 = false; t5Reason = `hashLock 不一致 (${hit.hash} ≠ ${artifact.hashLock})`; }
    }
  } catch { /* 新模組 */ }
  gates.push(['T5通', t5]);
...
function p7_seal(spec, artifact, registryPath) {
  ...
  if (rows.some((r) => r.id === mdCell(spec.id))) {
    return { stage: 'P7', ok: true, note: '已存在註冊表', skipped: true };
  }
  fs.mkdirSync(path.dirname(registryPath), { recursive: true });
  fs.writeFileSync(registryPath, registry.trimEnd() + '\n' + entry + '\n', 'utf8');
...
function generateKpiCards(kpis) {
  ...
  return `<div class="bento-grid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: clamp(8px, 2vw, 16px); margin-bottom: 24px">
  ${kpis.map((k) => `<div class="solid-card" style="background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: 8px; padding: clamp(12px, 2vw, 20px)">
    ...
    <div class="t5-strip-mini" style="margin-top: 8px; font-size: 0.7rem">◈ 溯源 · ✦ 透明 · ❖ 量化</div>
  </div>`).join('\n  ')}
</div>`;
}

function generateT5Strip(t5) {
  ...
  return `<div class="t5-strip" style="display: flex; flex-wrap: wrap; gap: 4px; margin-bottom: 24px">
  ${dims.map((d) => {
    const c = FIVE_T_COLORS[d];
    const passed = t5[d];
    return `<span data-dim="${d}" data-passed="${passed}" style="background: ${c.bg}; color: ${c.text}; opacity: ${passed ? 1 : 0.3}; padding: 4px 10px; border-radius: 8px; font-size: 0.75rem">${icons[d]} ${labels[d]}</span>`;
  }).join('\n  ')}
</div>`;
}

function generateVaultTable(rows) {
  ...
  const trs = rows.map((r) => {
    ...
    return `<tr style="border-bottom: 1px solid var(--border-color)">
      ...
      <td style="padding: 8px"><span style="background: color-mix(in srgb, var(--accent-teal) 18%, transparent); color: var(--accent-teal); padding: 2px 8px; border-radius: 4px; font-size: 0.75rem">${escapeHtml(r.standard)}</span></td>
  ...
  return `<div class="vault-table" style="margin-bottom: 24px; overflow-x: auto">
...
function generateSectionsMd(sections) {
...
function generateModule(spec, artifact) {
  ...
  const kpiHtml = generateKpiCards(spec.kpis);
  const t5Html = generateT5Strip(t5);
  const vaultHtml = generateVaultTable(spec.vaultRows);
  const sectionsHtml = generateSectionsMd(spec.sections);
...
<body>
  <div class="container">
    <header class="page-header">
      ...
      <p>MEDCE: ${escapeHtml(spec.medce.primary)} · 5T 品質閘門通過</p>
    </header>
