...
import type { MemoryGraph, GraphNode, GraphEdge, NodeType } from '../../../../lib/memory-graph-engine';

interface TruthChainVisualizerProps {
...
const getNodeIcon = (type: NodeType) => {
  switch (type) {
    ...
    case 'POLICY': return <ShieldCheck className="w-5 h-5" />;
    case 'MEMORY': return <Database className="w-5 h-5" />;
    case 'STAKEHOLDER_EXPECTATION': return <Users className="w-5 h-5" />;
    case 'SIMULATION': return <Cpu className="w-5 h-5" />;
...
export function TruthChainVisualizer({ graph, className = '' }: TruthChainVisualizerProps) {
  if (!graph || graph.nodes.length === 0) {
    return (
      <div className={`p-8 flex items-center justify-center rounded-xl border border-white/10 bg-slate-950/40 backdrop-blur-md ${className}`}>
  ...
  return (
    <div className={`relative p-6 md:p-10 rounded-2xl border border-white/10 bg-[#020617]/80 backdrop-blur-xl overflow-hidden ${className}`}>
            ...
            >
              <div className={`flex flex-col items-center p-6 rounded-xl border-2 shadow-[0_0_30px_-5px_rgba(6,182,212,0.3)] backdrop-blur-md min-w-[280px] ${getNodeColor(evidenceNode.type)}`}>
                ...
                {evidenceNode.hash_lock && (
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-black/40 rounded-md border border-cyan-500/30 w-full justify-center">
