...
export class OmniCoreEcosystem {
  ...
  private static hasEvidence(obj: unknown): obj is { evidence: { originCause: string; processTrace: string[]; finalEffect: string; hash_lock?: string } } {
  ...
  public static lockAndFreeze<T extends object>(obj: T): T {
    if (!OmniCoreEcosystem.hasEvidence(obj)) {
      (obj as { evidence: any }).evidence = { originCause: 'SYSTEM', processTrace: [], finalEffect: 'LOCKED' };
    }
    (obj as { evidence: any }).evidence['hash_lock'] = `0xCELESTIAL_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
