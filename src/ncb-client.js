...
// 跨裝置共用的真·後端 (對齊根專案 src/lib/ncb-utils.ts 介面)。
//
// 設計:
...
const NCB_DB_INSTANCE =
  (typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_NCB_DB_INSTANCE) ||
  (typeof process !== 'undefined' && process.env && process.env.NCB_DB_INSTANCE) ||
  '54686_esg_go_userdb';
...
export const ncbSubmissions = {
  async list(appId) {
    return ncbQuery({ table: `platforms_${appId}_submissions`, method: 'GET' });
  },
  async set(id, payload) {
    return ncbQuery({ table: `platforms_${payload.APP_ID || 'lc'}_submissions`, method: 'POST', body: { id, ...payload } });
  },
  async delete(id, appId) {
    return ncbQuery({ table: `platforms_${appId}_submissions`, method: 'DELETE', params: { id } });
  },
};

export const ncbProfiles = {
  async get(uid, appId) {
    const rows = await ncbQuery({ table: `platforms_${appId}_profiles`, method: 'GET', params: { user_id: uid } });
    return Array.isArray(rows) && rows.length ? rows[0] : null;
  },
  async set(uid, appId, data) {
    return ncbQuery({ table: `platforms_${appId}_profiles`, method: 'POST', body: { user_id: uid, ...data } });
  },
};

export const ncbTAs = {
  async get(uid, appId) {
    const rows = await ncbQuery({ table: `platforms_${appId}_tas`, method: 'GET', params: { user_id: uid } });
    return Array.isArray(rows) && rows.length ? rows[0] : null;
  },
  async set(uid, appId, data) {
    return ncbQuery({ table: `platforms_${appId}_tas`, method: 'POST', body: { user_id: uid, ...data } });
  },
};

export const ncbPairings = {
  async list(appId) {
    return ncbQuery({ table: `platforms_${appId}_pairings`, method: 'GET' });
  },
  async set(id, appId, payload) {
    return ncbQuery({ table: `platforms_${appId}_pairings`, method: 'POST', body: { id, ...payload } });
  },
  async delete(id, appId) {
    return ncbQuery({ table: `platforms_${appId}_pairings`, method: 'DELETE', params: { id } });
  },
};
