...
// Oracle 回拉的原始信任帳本 entry (omni_trust.entry)
export interface OracleTrustEntry {
  seq: number;
  prev_hash: string;
  curr_hash: string;
  uuid: string | null;
...
// Python reconcile 回傳的單筆原始矩陣 row (snake_case)
interface RawMatrixRow {
  uuid: string;
  origin_seq: number;
  terminal_seq: number;
  status: SyncStatus;
  direction?: SyncMatrixRow['direction'];
}
...
export async function pullFromOracle(
  limit = 0,
): Promise<{ ok: boolean; count: number; entries: OracleTrustEntry[]; reason?: string }> {
  if (!hasOracleCreds()) {
  ...
  try {
    ...
    return {
      ...
      count: out.count ?? 0,
      entries: out.entries ?? [],
      reason: out.ok ? undefined : out.error,
    };
  } catch (e) {
    return {
      ok: false,
      count: 0,
      entries: [],
      reason: `oracle pull failed: ${(e as Error).message}`,
    };
  }
}
...
export async function reconcileBidirectional(
  ...
  try {
    ...
    const out = JSON.parse(stdout.trim());
    const toRow = (r: RawMatrixRow): SyncMatrixRow => ({
      uuid: r.uuid,
