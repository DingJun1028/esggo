...
export async function reconcileAll(): Promise<{
  ...
  for (const e of pulled.entries as Array<{ uuid?: string; seq?: number }>) {
    if (e.uuid && e.seq !== undefined) terminalByUuid.set(e.uuid, Math.max(terminalByUuid.get(e.uuid) ?? 0, e.seq));
  }
  // 合併所有出現過的 uuid
...
export async function pullBehindApp(rows: SyncMatrixRow[]): Promise<{ pulled: number; failed: number }> {
  ...
  const byUuid = new Map((res.entries as Array<{ uuid?: string; seq?: number }>).filter(e => e.uuid).map((e) => [e.uuid as string, e]));
  let pulled = 0;
  let failed = 0;
  for (const r of rows) {
