...
#   DEERFLOW_TIMEOUT      — Max seconds to wait for the streaming run (default: 600)
#
# Modes: flash, standard, pro (default), ultra

set -euo pipefail

command -v python3 >/dev/null 2>&1 || { echo "ERROR: python3 is required (chat.sh parses SSE with it)" >&2; exit 1; }

DEERFLOW_URL="${DEERFLOW_URL:-http://localhost:2026}"
GATEWAY_URL="${DEERFLOW_GATEWAY_URL:-$DEERFLOW_URL}"
LANGGRAPH_URL="${DEERFLOW_LANGGRAPH_URL:-$DEERFLOW_URL/api/langgraph}"
DEERFLOW_TIMEOUT="${DEERFLOW_TIMEOUT:-600}"
...
if [ -z "$THREAD_ID" ]; then
  THREAD_RESP=$(curl -s -m 30 -X POST "${LANGGRAPH_URL}/threads" \
...
HTTP_CODE=$(curl -s -N -m "$DEERFLOW_TIMEOUT" -o "$TMPFILE" -w "%{http_code}" \
  -X POST "${LANGGRAPH_URL}/threads/${THREAD_ID}/runs/stream" \
  -H "Content-Type: application/json" \
  -d "$BODY" 2>/dev/null || echo "000")

if [ -z "$HTTP_CODE" ] || [ "$HTTP_CODE" -lt 200 ] || [ "$HTTP_CODE" -ge 300 ]; then
  echo "ERROR: Stream request failed (HTTP ${HTTP_CODE})." >&2
