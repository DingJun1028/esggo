...
## 9. Boot Volume attach/detach syntax (verified 2026-08-25)
For rescuing an instance via its boot volume, the exact oci.exe argument forms matter:

### 9.1 Find the Boot Volume OCID
`boot-volume-attachment list` REQUIRES `--availability-domain` (the full AD string, e.g.
`xzUx:AP-SINGAPORE-1-AD-1`, NOT bare `AD-1`) AND `--compartment-id` (tenancy OCID):
```bash
oci compute boot-volume-attachment list --instance-id <vps-ocid> \
  --compartment-id <tenancy-ocid> --availability-domain "xzUx:AP-SINGAPORE-1-AD-1" \
  --region ap-singapore-1
# → 'boot-volume-id' = the Boot Volume OCID; 'id' field == the instance OCID (OCI quirk)
```

### 9.2 Detach (non-pty safe)
No `--compartment-id`/`--availability-domain` needed; pass the **instance OCID** as the
attachment id and `--force` (skips the y/N TTY prompt that hangs in non-pty shells):
```bash
oci compute boot-volume-attachment detach --boot-volume-attachment-id <vps-ocid> \
  --region ap-singapore-1 --force
```

### 9.3 Re-attach (no AD/compartment)
```bash
oci compute boot-volume-attachment attach --instance-id <vps-ocid> \
  --boot-volume-id <bv-ocid> --region ap-singapore-1
```

### 9.4 WARNING — eventual-consistency Conflict trap
`boot-volume-attachment list` may report `DETACHED` while `attach` to another instance
returns `Conflict: Volume currently attached`, and `instance action START` returns
`Conflict: Boot volume is not currently attached`. This is OCI eventual consistency, NOT
a real state. The lock persisted >3 min across retries. **Reliable recovery**: re-attach
to the ORIGINAL instance (9.3) and wait ~60s before START. Do NOT detach-and-attach to a
*different* (rescue) instance to edit `authorized_keys` — it will Conflict and leave you
worse off (STOPPED instance, boot volume stranded). Prefer the OCI web-console interactive
Serial Console terminal for any authorized_keys edit.

### 9.5 Launch a temp rescue instance (key injected at create)
`--ssh-authorized-keys-file` (NOT `--ssh-authorized-keys`) takes a **native Windows path**
(`C:/Users/dingj/.ssh/id_rsa_esggo.pub`, NOT `/c/Users/...`):
```bash
oci compute instance launch --compartment-id <tenancy-ocid> \
  --availability-domain "xzUx:AP-SINGAPORE-1-AD-1" \
  --shape "VM.Standard.A1.Flex" --shape-config '{"ocpus":1,"memoryInGBs":6}' \
  --image-id <ubuntu-24.04-arm-image-ocid> --subnet-id <subnet-ocid> \
  --ssh-authorized-keys-file "C:/Users/dingj/.ssh/id_rsa_esggo.pub" \
  --display-name "rescue-temp" --region ap-singapore-1
```
Get the public IP after RUNNING: `oci network vnic get --vnic-id <vnic-ocid> --region ...`
→ `public-ip` field. Terminate when done: `oci compute instance terminate --instance-id
<rescue-ocid> --region ap-singapore-1 --force` (free-tier quota is limited).
