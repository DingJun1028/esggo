...
// 用途：驗證遠端伺服器健康狀態
// 執行：ssh ubuntu@your-vps 'bash <(curl -s URL)'
// ============================================================
...
async function main() {
  console.log('🧪 Remote Validation - Health Check\n');

  const results = await Promise.all([
    check('/health', `${HOST}/health`),
    check('/translate (single)', `${HOST}/translate`, {
      method:'POST', headers:{'Content-Type':'application/json'},
      body:JSON.stringify({text:'hello world',from:'en',to:'zh'})
    }),
    check('/translate (multi)', `${HOST}/translate`, {
      method:'POST', headers:{'Content-Type':'application/json'},
      body:JSON.stringify({text:'thank you',targets:['zh-TW','ja','es']})
    }),
  ]);
