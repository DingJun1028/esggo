...
import { getOATableServerClient } from '../../../lib/omni-table';
...
export async function syncLogicNodesToOmniTable(nodes: LogicNode[]) {
  try {
    ...
    if (!datasheetId) {
      const created = await withRetry<any>(() =>
        omniTable.createDatasheet(spaceId, {
          name: 'Logic Nodes',
          fields: [
            { name: '節點名稱', type: 'text' },
    ...
    if (existingRecords.records && existingRecords.records.length > 0) {
      await withRetry<any>(() =>
        omniTable.deleteRecords(
          datasheetId,
          existingRecords.records.map((r: any) => r.recordId)
        )
      );
    }
