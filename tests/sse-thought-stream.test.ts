...
async function openStream(runId: string): Promise<{ reader: ReadableStreamDefaultReader<Uint8Array>; ac: AbortController; decoder: TextDecoder }> {
  const ac = new AbortController();
  const req = new Request(`http://localhost/api/agent/gemma4-local/thought/stream?runId=${runId}`, { signal: ac.signal });
...
describe('SSE thought stream (GET /api/agent/[id]/thought/stream)', () => {
  ...
  it('filters thoughts by runId', async () => {
    ...
    setTimeout(() => {
      publishThought({ agentId: 'gemma4-local', runId: otherRun, step: 1, content: 'should-be-filtered' });
