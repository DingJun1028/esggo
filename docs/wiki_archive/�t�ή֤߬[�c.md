...
*   **基礎設施層 (Infrastructure Layer)**：
    ...
    *   **技術**：Supabase PostgreSQL 作為核心關係型數據庫，提供數據儲存、認證 (Auth)、實時訂閱 (Realtime) 與文件儲存 (Storage) 等服務。BlueCC Hybrid Control Plane 作為混合雲控制平面，用於管理多雲或混合部署環境下的資源與服務。API Connectors 用於與第三方系統（如 ERP, IoT 平台）進行數據交換的標準化接口。Resend Email 用於系統通知、報告發送等郵件服務。Hermes Gateway 作為 API Gateway，負責請求路由、負載均衡、安全認證與流量管理。
