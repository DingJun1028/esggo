...
2. 客戶旅程與 UX 體驗 (Customer Journey & UX)
   ...
   操作軌跡：
3. 企業各部門將原始 ESG 數據（如電費單、員工培訓紀錄）透過前端介面或 API 輸入系統。
4. 系統自動進行數據結構化與初步驗證，並透過 AI 協作層提供撰寫建議。
5. 關鍵數據與報告內容自動關聯至「證據金庫」，並生成不可篡改的 Hash 鎖。
6. 永續專案經理利用 AI 賦能的 SustainWrite 編輯器高效產出報告草稿。
7. 內部稽核或外部查核員可透過 5T 標籤與 ZKP 驗證機制，快速追溯數據源頭與變更歷史。
8. 最終報告經確認後發佈，並確保其內容具備高度可信性與透明度。
...
- **AI 協作層 (AI Collaboration Layer)**：
  ...
  - **技術**：Mistral Small 3.1 24B (OpenRouter) 作為核心大語言模型 (LLM) 提供智能推理與生成能力。Genkit 流程 用於編排複雜的 AI 工作流，如 RAG (Retrieval-Augmented Generation) 或多步驟推理。OmniHermes 合規掃描 基於 LLM 進行文本分析，比對內部知識庫與外部法規，識別合規風險。SPIRIT 三大 AI 人格 透過 Prompt Engineering 與模型微調，定義不同角色（如數據分析師、合規專家、撰寫助手）的 AI 行為。Digital Twin RAG 結合企業內部數據與知識庫，為數位分身提供即時、精準的上下文信息。
