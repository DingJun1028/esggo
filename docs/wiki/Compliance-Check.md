...
- **資料流向**： 讀取 `SustainWrite` 編輯器草稿 -> 調用 `AI-Platform` (Mistral Small 3.1 24B Pro) -> 比對 `Library` 法律資料庫 -> 寫入 `ncbdb.compliance_records`。
- **5T 實踐點**：
...
- **AI 診斷引擎**： 使用 `Genkit` + `Mistral Small 3.1 24B` 進行快速語意比對。
