// OA-Team 60 代理暗光雙屬性共享記憶 smoke test
//
// 對齊 OA-TEAM-60-MEMORY-MAPPING.md + tencent-mem.ts:
//   - 60 代理分為 umbra（暗系蜂王 01-30）與 lumen（光系蜂后 31-60）
//   - 每陣列 6 代理，每代理寫入帶 alignment + archetype 標籤
//   - 召回時按 alignment 分組，驗證暗/光分區可查
//
// 環境:
//   TDAI_GATEWAY_URL / TDAI_GATEWAY_API_KEY / TDAI_SERVICE_ID
//   預設: http://127.0.0.1:8420 / 空 / oa-team-swarm
//
// 退出碼: 0 = PASS, 1 = FAIL, 2 = SKIP（無 core/key）
...
// 60 代理對應表（從 OA-TEAM-60-MEMORY-MAPPING.md 壓縮）
// [編號, 代理名, alignment, archetype, 陣列]
const AGENTS = [
  // 蜂王 01-30 (umbra 為主，智庫/5T 為 lumen)
  ['01', '萬能蜂后', 'lumen', 'Sovereign', '智庫聖所'],
  ['02', '萬能規劃蜂', 'lumen', 'Architect', '智庫聖所'],
  ['03', '萬能分析蜂', 'lumen', 'Oracle', '智庫聖所'],
  ['04', '萬能策効蜂', 'lumen', 'Muse', '智庫聖所'],
  ['05', '萬能風險蜂', 'lumen', 'Sentinel', '智庫聖所'],
  ['06', '萬能優化蜂', 'lumen', 'Alchemist', '智庫聖所'],
  ...
  ['13', '萬能圖像蜂', 'lumen', 'Painter', '光之羽翼'],
  ['14', '萬能動畫蜂', 'lumen', 'Flow', '光之羽翼'],
  ['15', '萬能文案蜂', 'lumen', 'Narrator', '光之羽翼'],
  ['16', '萬能音頻蜂', 'lumen', 'Resonator', '光之羽翼'],
  ['17', '萬能市場蜂', 'lumen', 'Herald', '光之羽翼'],
  ['18', '萬能社群蜂', 'lumen', 'Bridger', '光之羽翼'],
  ...
  ['25', '萬能測場蜂', 'lumen', 'Purifier', '5T 驗算'],
  ['26', '萬能追蹤蜂', 'lumen', 'Hawk', '5T 驗算'],
  ['27', '萬能安全蜂', 'lumen', 'Ward', '5T 驗算'],
  ['28', '萬能維護蜂', 'lumen', 'Curator', '5T 驗算'],
  ['29', '萬能支援蜂', 'lumen', 'Echo', '5T 驗算'],
  ['30', '萬能質控蜂', 'lumen', 'Seal', '5T 驗算'],
  // 蜂后 31-60 (lumen 為主，符文/煉金 為 umbra)
  ...
  ['37', '蜂后萬能編碼蜂', 'umbra', 'Smith', '符文契約'],
  ['38', '蜂后萬能算法蜂', 'umbra', 'Depth', '符文契約'],
  ['39', '蜂后萬能架構蜂', 'umbra', 'Architect', '符文契約'],
  ['40', '蜂后萬能數據蜂', 'umbra', 'Weaver', '符文契約'],
  ['41', '蜂后萬能測試蜂', 'umbra', 'Seer', '符文契約'],
  ['42', '蜂后萬能設計蜂', 'umbra', 'Shaper', '符文契約'],
  ...
  ['49', '蜂后萬能增長蜂', 'umbra', 'Erode', '煉金熵減'],
  ['50', '蜂后萬能運營蜂', 'umbra', 'Nexus', '煉金熵減'],
  ['51', '蜂后萬能商業分析蜂', 'umbra', 'Skinner', '煉金熵減'],
  ['52', '蜂后萬能探路蜂', 'umbra', 'Scout', '煉金熵減'],
  ['53', '蜂后萬能外交蜂', 'umbra', 'Wraith', '煉金熵減'],
  ['54', '蜂后萬能調研蜂', 'umbra', 'Lens', '煉金熵減'],
...
async function main() {
  console.log('# OA-Team 60 代理暗光雙屬性共享記憶 smoke test');
  ...
  // Phase 1: 60 代理寫入（分 umbra/lumen 兩 session）
  ...
  // Phase 3: 驗證
  const checks = [
    ['umbra 全寫入', umbraWrites === umbraAgents.length, `${umbraWrites}/${umbraAgents.length}`],
    ['lumen 全寫入', lumenWrites === lumenAgents.length, `${lumenWrites}/${lumenAgents.length}`],
    ['umbra 可召回', umbraRecall.length > 0, `recall=${umbraRecall.length}`],
    ['lumen 可召回', lumenRecall.length > 0, `recall=${lumenRecall.length}`],
    ['暗光分區隔离', umbraRecall.every((m) => m.content.includes('umbra')) && lumenRecall.every((m) => m.content.includes('lumen')), 'cross-talk=0'],
  ];

  const failed = checks.filter(([, ok]) => !ok).map(([name]) => name);
  const status = failed.length === 0 ? 'PASS' : 'FAIL';
  console.log(`\nRESULT: ${status}${failed.length ? ' ❌ ' + failed.join(', ') : ' ✅ 60 代理暗光雙屬性共享記憶成功'}`);
