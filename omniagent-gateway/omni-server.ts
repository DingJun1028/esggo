...
// ═══════════════════════════════════════════════════════════════
// SHARED MEMORY LAYER — NCBDB Bidirectional Sync
// ═══════════════════════════════════════════════════════════════
...
app.post('/memory/store', async (req, res) => {
  try {
    const { agent, type, content, tags, priority, ttl } = req.body;
    if (!agent || !type || !content) return res.status(400).json({ error: 'agent, type, content required' });
    const id = sharedMemory.store({ agent, type, content, tags: tags || [], priority: priority || 'normal', ttl });
    broadcastWS({ type: 'MEMORY_STORED', source: 'SharedMemory', payload: { id, agent, type } });
    res.json({ ok: true, id });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// GET /memory/get/:id
app.get('/memory/get/:id', (req, res) => {
  const entry = sharedMemory.get(req.params.id);
...
app.post('/memory/query', (req, res) => {
  const { agent, type, tags, since, until, limit, search } = req.body;
  const results = sharedMemory.query({ agent, type, tags, since, until, limit: limit || 50, search });
  res.json({ total: results.length, entries: results });
});

// GET /memory/stats
app.get('/memory/stats', (_req, res) => res.json(sharedMemory.getStats()));

// POST /memory/sync — Force NCBDB sync
app.post('/memory/sync', async (_req, res) => {
  const result = await sharedMemory.forceSync();
  res.json(result);
});

// POST /agents/message — Inter-agent messaging
app.post('/agents/message', (req, res) => {
  const { from, to, type, payload, priority } = req.body;
  if (!from || !type || !payload) return res.status(400).json({ error: 'from, type, payload required' });
  const messageId = 'msg_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8);
  broadcastWS({ type: 'AGENT_MESSAGE', messageId, from, to: to || 'broadcast', payload, priority: priority || 'normal', timestamp: Date.now() });
  res.json({ ok: true, messageId });
});

console.log('[OmniGateway] Shared Memory Layer (NCBDB sync) initialized');
// 404 + error handlers
