...
def get_video_materials(
    ...
    if params.video_source == "local":
        ...
        mats = list(params.video_materials or [])
        if not mats:
            import os as _os, glob as _glob
            _md = """"""
            try:
                _md = video.config.app.get("material_directory", "") if hasattr(video, "config") else ""
            except Exception:
                _md = ""
            if not _md or not _os.path.isdir(_md):
                _md = "/MoneyPrinterTurbo/storage/materials"
            mats = sorted(_glob.glob(_os.path.join(_md, "*.mp4")))
            logger.info(f"local fallback: found {len(mats)} mp4 in {_md}")
        materials = video.preprocess_video(
            materials=mats, clip_duration=params.video_clip_duration
        )
        if not materials:
            _mark_task_failed(
                task_id,
